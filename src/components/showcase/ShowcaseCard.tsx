...
export function ShowcaseCard({ example, locale, showEditButton = true }: ShowcaseCardProps) {
  const title = getLocalizedText(example.title, locale)
  const description = getLocalizedText(example.description, locale)
  const chartTypeLabel = chartTypeLabels[example.chartConfig.type]?.[locale] ?? example.chartConfig.type
    ...
    <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md overflow-hidden border border-gray-200 dark:border-gray-700 hover:shadow-lg transition-shadow">
      ...
      <div className="p-4">
        ...
        <div className="flex items-center justify-between">
          {example.category && (
            <CategoryBadge category={example.category as ShowcaseCategory} locale={locale} />
          )}
          ...
          {showEditButton && (
            ...
            >
              {locale === 'sr-Cyrl' ? 'Уреди' : locale === 'sr-Latn' ? 'Uredi' : 'Edit'} →
            </Link>
          )}
        </div>
