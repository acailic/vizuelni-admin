...
import { generateQualityReport } from '@/lib/charts/quality-check'
...
import { QualityIndicator } from '@/components/configurator/QualityIndicator'
...
function PreviewStepComponent({ data, locale: _locale, labels }: PreviewStepProps) {
  ...
  const qualityReport = useMemo(
    () =>
      ready && config.type && config.title
        ? generateQualityReport(config as ChartConfig, data)
        : null,
    [config, data, ready]
  )
  ...
  return (
    <div className="space-y-6">
      ...
      {qualityReport && <QualityIndicator report={qualityReport} />}
      ...
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-3">
        ...
        >
          <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 4H5a1 1 0 00-1 1v3m15-4h-3m3 0v3M4 16v3a1 1 0 001 1h3m8-4l4-4-4-4m-8 8l-4-4 4-4" />
          </svg>
          {l.actions.download_svg}
        </button>
        <button
          type="button"
