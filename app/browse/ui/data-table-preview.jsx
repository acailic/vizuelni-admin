...
export const DataTablePreview = ({ title, dimensions, measures, observations, linkToMetadataPanel, }) => {
    ...
    const sortedObservations = useMemo(() => {
        ...
        const convert = isNumericalMeasure(sortBy) || sortBy.isNumerical
            ? (v) => +v
            : (v) => { var _a, _b; return (_b = (_a = valuesByLabel.get(v)) === null || _a === void 0 ? void 0 : _a.position) !== null && _b !== void 0 ? _b : v; };
        return [...observations].sort((a, b) => compare(convert(a[sortBy.id]), convert(b[sortBy.id])));
    ...
    const handleSort = useCallback((component) => {
        if ((sortBy === null || sortBy === void 0 ? void 0 : sortBy.id) === component.id) {
    ...
    return (<div ref={tooltipContainerRef} style={{ width: "100%" }}>
      <Table>
        <caption style={{ display: "none" }}>{title}</caption>
        <TableHead sx={{ position: "sticky", top: 0 }}>
          <TableRow>
            {sortedComponents.map((component) => {
            return (<TableCell key={component.id} component={"th"} role="columnheader" onClick={() => handleSort(component)} sx={{
                    textAlign: isNumericalMeasure(component) ? "right" : "left",
                    whiteSpace: "nowrap",
                }}>
                  <TableSortLabel active={!sortBy || sortBy.id === component.id} direction={!sortBy ? "desc" : sortDirection} IconComponent={SvgIcChevronDown} sx={{
                    "&:hover > svg": {
                        ...(!sortBy ? { transform: "translateY(-15%)" } : {}),
                    },
                }}>
                    <ComponentLabel component={component} tooltipProps={tooltipProps} linkToMetadataPanel={linkToMetadataPanel}/>
                  </TableSortLabel>
                </TableCell>);
        })}
          </TableRow>
        ...
        <TableBody>
          {sortedObservations ? (sortedObservations.map((obs, i) => {
            return (<TableRow key={i}>
                  {sortedComponents.map((c) => {
                    const numerical = isNumericalMeasure(c);
                    const format = formatters[c.id];
                    const v = obs[c.id];
                    return (<TableCell key={c.id} component={"td"} sx={{ textAlign: numerical ? "right" : "left" }}>
