...
export const NavigationItem = ({ children, filters, next, count, active, level = 1, disableLink, countBg, }) => {
    ...
    const [newFiltersAdd, href] = useMemo(() => {
        const extraURLParams = stringify(pickBy({
            includeDrafts,
            search,
            topic: level === 2 && !disableLink ? next.iri : undefined,
        }, Boolean));
        const newFilters = [...filters].filter((f) => (disableLink ? true : f.__typename !== "DataCubeAbout") &&
    ...
    const [newFiltersRemove, removeFilterPath] = useMemo(() => {
        const extraURLParams = stringify(pickBy({ includeDrafts, search }, Boolean));
        const newFilters = filters.filter((f) => f.__typename !== "DataCubeAbout" && f.iri !== next.iri);
        return [
            newFilters,
            `/browse/${newFilters.map(encodeFilter).join("/")}?${extraURLParams}`,
        ];
    }, [includeDrafts, search, filters, next.iri]);
    return (<MotionBox {...accordionPresenceProps} data-testid="navItem">
      <MaybeLink href={href} passHref legacyBehavior disabled={!!disableLink} scroll={false} shallow>
        <Link className={classes.root} variant="body2" onClick={disableLink && !active
            ? (e) => {
                e.preventDefault();
          ...
          {children}
          {highlighted ? (<MaybeLink href={removeFilterPath} passHref legacyBehavior disabled={!!disableLink} scroll={false} shallow>
              <ButtonBase className={classes.removeFilterButton} onClick={disableLink
                ? (e) => {
                    e.preventDefault();
            ...
            </MaybeLink>) : count !== undefined ? (<NavigationChip backgroundColor={countBg}>{count}</NavigationChip>) : null}
        </Link>
      </MaybeLink>
    </MotionBox>);
};
