...
export const useRedirectToLatestCube = ({ datasetIri, dataSource, }) => {
    ...
    useEffect(() => {
        if (dataSource.type !== "sparql") {
            console.error(`Cannot redirect to unversioned IRI if dataSource.type !== "sparql", here it's ${dataSource.type}`);
            return;
        }
        if (router.isReady) {
            handleLoad();
