 ...
 */
export const createUseBrowseState = ({ syncWithUrl, }) => {
    const useParamsHook = syncWithUrl ? useUrlSyncState : useState;
    return () => {
        const inputRef = useRef(null);
        const [browseParams, setParams] = useParamsHook({});
        const { search, type, order, iri, includeDrafts, dataset: paramDataset, } = browseParams;
        ...
        const setSearch = useEvent((v) => setParams((prev) => ({ ...prev, search: v })));
        const setIncludeDrafts = useEvent((v) => setParams((prev) => ({ ...prev, includeDrafts: v })));
        const setOrder = useEvent((v) => setParams((prev) => ({ ...prev, order: v })));
        const setDataset = useEvent((v) => setParams((prev) => ({ ...prev, dataset: v })));
        return useMemo(() => {
