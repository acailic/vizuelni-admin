...
export const BlockMoreButton = ({ blockKey }) => {
    ...
    return (<>
      <IconButton color="secondary" onClick={(e) => {
        ...
        }} sx={{ height: "fit-content" }} {...DISABLE_SCREENSHOT_ATTR} data-testid="block-more-button">
        <SvgIcMore />
      </IconButton>
      <ArrowMenuTopBottom open={!!anchor} anchorEl={anchor} onClose={(e) => {
            e.stopPropagation();
            handleClose();
        }} anchorOrigin={{ horizontal: "right", vertical: "bottom" }} transformOrigin={{ horizontal: "right", vertical: "top" }}>
        <div>
          <MenuActionItem type="button" as="menuitem" onClick={(e) => {
            e.stopPropagation();
            handleDuplicate();
            handleClose();
        }} leadingIconName="duplicate" label={t({ id: "block-controls.duplicate", message: "Duplicate" })}/>
          <MenuActionItem type="button" as="menuitem" color="red" requireConfirmation confirmationTitle={t({
            ...
            handleRemove();
            handleClose();
        }} leadingIconName="trash" label={t({ id: "block-controls.delete", message: "Delete" })}/>
        </div>
      </ArrowMenuTopBottom>
    </>);
};
