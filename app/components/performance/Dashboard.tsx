...
const PerformanceDashboard: React.FC = () => {
  ...
  const getRatingIcon = (rating: string) => {
    switch (rating) {
      case "good":
        return <GoodIcon sx={{ color: "#4caf50" }} />;
      case "needs-improvement":
        ...
        <CardContent>
          <Box display="flex" alignItems="center" mb={2}>
            ...
            <Typography variant="h4" component="div">
              Performance Score:{" "}
              <span style={{ color: getOverallScoreColor(overallScore) }}>
                {overallScore}/100
              </span>
            </Typography>
          </Box>
            ...
            <Card>
              <CardContent>
                ...
                <Typography variant="h4" color={getRatingColor(metric.rating)}>
                  {metric.value.toFixed(metric.name.includes("CLS") ? 3 : 0)}
                  {metric.unit}
                </Typography>
            ...
            {getPerformanceRecommendations(metrics).map(
              (recommendation, index) => (
                <React.Fragment key={index}>
                  ...
                  {index <
                    getPerformanceRecommendations(metrics).length - 1 && (
                    <Divider />
                  )}
                </React.Fragment>
              )
      ...
      {process.env.NODE_ENV === "development" && (
        <Alert severity="info" sx={{ mt: 3 }}>
          <Typography variant="body2">
            Performance metrics are being tracked in development mode. In
            production, these metrics will be automatically sent to analytics
            for monitoring.
          </Typography>
        </Alert>
      )}
...
const getProgressPercentage = (
  ...
  return Math.max(
    0,
    25 -
      ((value - threshold.needsImprovement) / threshold.needsImprovement) * 25
  );
};
      ...
      switch (metric.name) {
        case "Largest Contentful Paint (LCP)":
          recommendations.push({
            ...
            description:
              "Compress images, use modern formats (WebP, AVIF), implement lazy loading, and optimize server response times.",
          });
          break;
        case "First Input Delay (FID)":
          recommendations.push({
            ...
            description:
              "Split code into smaller chunks, remove unused JavaScript, and defer non-critical scripts.",
          });
          break;
        case "Cumulative Layout Shift (CLS)":
          recommendations.push({
            ...
            description:
              "Include size attributes for images and embeds, avoid inserting content above existing content.",
          });
          break;
        case "First Contentful Paint (FCP)":
          recommendations.push({
            ...
            description:
              "Reduce server response time, eliminate render-blocking resources, and optimize critical CSS.",
          });
          break;
        case "Time to First Byte (TTFB)":
          recommendations.push({
            ...
            description:
              "Use CDN, implement server-side caching, optimize database queries, and use HTTP/2 or HTTP/3.",
          });
          break;
      }
    }
