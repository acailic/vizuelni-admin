...
const useTextBlockStyles = makeStyles(() => ({
    root: {
...
export const TextBlock = forwardRef(({ children, block, className, dragHandleProps, ...rest }, ref) => {
    const locale = useLocale();
    const getLocalizedValue = (values) => {
        const key = (locale in values ? locale : "en");
        return values[key];
    };
    const [state, dispatch] = useConfiguratorState(hasChartConfigs);
    const layouting = isLayouting(state);
    const classes = useTextBlockStyles({ layouting });
    const handleTextBlockClick = useEvent((block) => {
        if (!layouting) {
            return;
        }
        dispatch({
    ...
    {...rest} ref={ref} id={block.key} className={clsx(classes.root, className, TEXT_BLOCK_WRAPPER_CLASS)} onClick={(e) => {
            if (e.isPropagationStopped()) {
                return;
            }
            handleTextBlockClick(block);
        }}>
      <div className={TEXT_BLOCK_CONTENT_CLASS} style={{ flexGrow: 1, height: "fit-content" }}>
        <Markdown>
          {text || t({ id: "annotation.add.text", message: "[ Add text ]" })}
        </Markdown>
      </div>
      {layouting ? (<TextBlockActionElements block={block} dragHandleProps={dragHandleProps}/>) : null}
      {children}
    </div>);
});
export const renderBaseTextBlock = (block) => {
    return (<TextBlock 
    // Important, otherwise react-grid-layout breaks.
    key={block.key} block={block}/>);
};
export const DndTextBlock = ({ block }) => {
    const { setActivatorNodeRef, setNodeRef: setDraggableNodeRef, attributes, listeners, isDragging, } = useDraggable({ id: block.key });
    const { setNodeRef: setDroppableNodeRef, isOver: isDragOver, active: isDragActive, } = useDroppable({ id: block.key });
    ...
    return (<TextBlock {...attributes} ref={setRef} block={block} className={isDragging || isDragActive || isDragOver
...
const TextBlockActionElements = ({ block, dragHandleProps, }) => {
    return (<ActionElementsContainer>
...
export const useSyncTextBlockHeight = () => {
    ...
    const isFreeCanvas = layout.type === "dashboard" && layout.layout === "canvas";
    useLayoutEffect(() => {
        if (!isFreeCanvas) {
            return;
        }
        selectAll(`.${TEXT_BLOCK_WRAPPER_CLASS}`).each(function () {
            const wrapperEl = this;
            const contentEl = wrapperEl.querySelector(`.${TEXT_BLOCK_CONTENT_CLASS}`);
            if (!contentEl) {
                return;
            }
            const key = wrapperEl.id;
            const h = Math.ceil(contentEl.clientHeight / ROW_HEIGHT) || 1;
            const newLayouts = Object.fromEntries(Object.entries(layout.layouts).map(([bp, layouts]) => [
                bp,
                layouts.map((b) => {
