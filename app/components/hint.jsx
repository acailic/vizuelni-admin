...
export const Spinner = ({ size = 40, ...props }) => {
    return (<Flex {...props} sx={{
      ...
      <svg width={43} height={44} viewBox="0 0 43 44" fill="none">
        ...
        <path d="M41.5 22C41.5 25.2891 40.6889 28.5273 39.1384 31.428C37.588 34.3286 35.3461 36.8022 32.6114 38.6294C29.8767 40.4567 26.7335 41.5814 23.4603 41.9037C20.1872 42.2261 16.885 41.7363 13.8463 40.4776C10.8077 39.219 8.12633 37.2304 6.03979 34.6879C3.95326 32.1455 2.52595 29.1277 1.8843 25.9019C1.24264 22.676 1.40644 19.3417 2.36119 16.1944C3.31595 13.047 5.03218 10.1836 7.35786 7.85791" stroke="#1F2937" strokeWidth={3}/>
      </svg>
    </Flex>);
};
...
export const Loading = ({ delayMs = 1000 }) => {
    ...
    return (<Flex className={classes.root} sx={{ animation: `0s linear ${delayMs}ms forwards ${delayedShow}` }}>
      <Spinner />
      <div>
        <Typography variant="h3" component="p">
          <Trans id="hint.loading.data">Loading data...</Trans>
        </Typography>
        {variant === "long" ? (<Typography variant="h3" component="p">
...
const useInlineLoadingStyles = makeStyles((theme) => {
    return {
        root: {
            display: "flex",
            flexDirection: "column",
...
export const InlineLoading = ({ delayMs = 1000 }) => {
    const classes = useInlineLoadingStyles();
    return (<Box className={classes.root} sx={{ animation: `0s linear ${delayMs}ms forwards ${delayedShow}` }}>
      <Spinner />
      <Typography>
        <Trans id="hint.loading.data">Loading data...</Trans>
      </Typography>
...
export const LoadingOverlay = () => {
    ...
    return (<MotionBox className={classes.overlay} initial={{
...
export const LoadingDataError = ({ message }) => {
    return (<HintError>
      <AlertTitle>
      ...
      <Link target="_blank" href="https://visualization-tool.status.interactivethings.io/">
        {" "}
...
export const ChartUnexpectedError = ({ error }) => {
    return (<HintError>
      <AlertTitle>
      ...
      {error ? (<Typography variant="body2" component="div" mt={2}>
          {error.message}
        </Typography>) : null}
    </HintError>);
};
...
const mkHint = (color, displayName) => {
    const Component = ({ smaller, sx, ...rest }) => {
        return (<Alert color={color} icon={smaller ? false : <Icon name={colorToIcon[color]}/>} sx={{
