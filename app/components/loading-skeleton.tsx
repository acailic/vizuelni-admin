 ...
 */
export const LoadingSkeleton = ({ count = 1, height = 40, width = "100%" }: SkeletonProps) => {
  return (
    <Stack spacing={1}>
      {Array.from({ length: count }).map((_, index) => (
        <Skeleton key={index} variant="rectangular" height={height} width={width} />
      ))}
    </Stack>
  );
};
  ...
  return (
    <Box sx={{ width: "100%", p: 2 }}>
      ...
      {Array.from({ length: rows }).map((_, rowIndex) => (
        <Stack key={`row-${rowIndex}`} direction="row" spacing={2} sx={{ mb: 1 }}>
 ...
 */
export const TextSkeleton = ({ lines = 5 }: { lines?: number }) => {
  return (
    <Stack spacing={1}>
  ...
  return (
    <Box sx={{ p: 3 }}>
      ...
      <Stack direction="row" spacing={2} sx={{ mb: 4 }}>
        {[1, 2, 3, 4].map((i) => (
          <Box key={i} sx={{ flex: 1, p: 2, border: "1px solid", borderColor: "divider", borderRadius: 1 }}>
