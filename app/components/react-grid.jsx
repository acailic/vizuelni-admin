...
const ResponsiveGrid = ResponsiveReactGridLayout;
export const availableHandlesByBlockType = {
...
export const ChartGridLayout = ({ children, className, layouts, resize, ...rest }) => {
    const classes = useStyles();
    const [state, dispatch] = useConfiguratorState(hasChartConfigs);
    const layout = state.layout;
    assert(layout.type === "dashboard" && layout.layout === "canvas", "ChartGridLayout can only be used in a canvas layout!");
    ...
    const [enhancedLayouts, setEnhancedLayouts] = useState(() => {
        ...
        return mapValues(layouts, (chartLayouts) => {
            return chartLayouts.map((chartLayout) => {
                var _a;
                const block = layout.blocks.find((block) => block.key === chartLayout.i);
                return {
                    ...chartLayout,
                    maxW: MAX_W,
                    w: Math.min(MAX_W, chartLayout.w),
                    resizeHandles: resize && block ? availableHandlesByBlockType[block.type] : [],
                    minH: (_a = chartLayout.minH) !== null && _a !== void 0 ? _a : MIN_H,
    ...
    useEffect(() => {
        ...
        const newLayouts = Object.fromEntries(Object.entries(enhancedLayouts).map(([breakpoint, chartLayouts]) => {
            return [
                breakpoint,
                chartLayouts.map((chartLayout) => {
                    const block = layout.blocks.find((block) => block.key === chartLayout.i);
                    if (block === null || block === void 0 ? void 0 : block.initialized) {
                        return chartLayout;
                    }
                    let minH = MIN_H;
                    const chartKey = chartLayout.i;
                    const wrapper = document.querySelector(`#${getChartWrapperId(chartKey)}`);
                    if (wrapper) {
                        const chartContainer = wrapper.querySelector(`.${chartContainerClasses.chartContainer}`);
                        if (chartContainer) {
                            const minWrapperHeight = wrapper.scrollHeight -
    ...
    return (<ResponsiveGrid {...rest} layouts={layouts} className={clsx(classes.root, className)} cols={COLS} rowHeight={ROW_HEIGHT} useCSSTransforms={false} compactType="vertical" preventCollision={false} isResizable={resize}>
      {children}
    </ResponsiveGrid>);
};
export const getInitialTileWidth = () => MAX_W / 4;
export const getInitialTileHeight = () => INITIAL_H;
export const generateLayout = function ({ count, maxWidth = MAX_W, maxHeight = MAX_H, layout, resizeHandles, }) {
