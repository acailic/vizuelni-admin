...
export const PopulationTrends = memo(({ data, width = 900, height = 500, margin = { top: 50, right: 80, bottom: 70, left: 80 }, historicalColor = "#2196f3", projectionColor = "#ff9800", title = "Population Trends", }) => {
    ...
    useEffect(() => {
        ...
        const xScale = scaleLinear()
            ...
            .range([0, innerWidth])
            .nice();
        const yExtent = extent(data, (d) => d.total);
        ...
        // Add Y axis
        g.append("g").call(axisLeft(yScale).ticks(8)).style("font-size", "11px");
        // Add title
        svg
            .append("text")
            .attr("x", width / 2)
        ...
        g.append("rect")
            ...
            if (dataPoint) {
                ...
                focus
                    .select("circle")
                    .attr("fill", dataPoint.type === "historical"
                    ? historicalColor
                    : projectionColor);
                focus
                    .select(".tooltip-bg")
                    .attr("stroke", dataPoint.type === "historical"
                    ? historicalColor
                    : projectionColor);
                focus.select(".tooltip-year").text(`${dataPoint.year}`);
                focus
