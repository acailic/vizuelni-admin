...
function ColumnChartComponent({ data, xKey, yKey, width = defaultColumnConfig.width, height = defaultColumnConfig.height, margin = defaultColumnConfig.margin, colors = ['#ff9800', '#ff5722', '#f44336', '#e91e63', '#9c27b0', '#673ab7'], xLabel = '', yLabel = '', multiSeries = false, stacked = false, showZeroLine = false }) {
    ...
    useEffect(() => {
        ...
        // Extract values
        const xValues = data.map((d) => { var _a; return String((_a = d[xKey]) !== null && _a !== void 0 ? _a : ''); });
        // Calculate max Y value based on stacked or grouped
        let maxY;
        if (stacked && shouldUseAllSeries) {
            maxY =
                max(data.map((d) => resolvedSeriesKeys.reduce((sum, key) => sum + (Number(d[key]) || 0), 0))) || 0;
        }
        else {
            const allYValues = data.flatMap((d) => resolvedSeriesKeys.map((key) => Number(d[key]) || 0));
