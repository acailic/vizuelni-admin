 ...
 */
export function SimpleChart({ data, chartType, xKey, yKey, width = 800, height = 400 }) {
    ...
    if (!data || data.length === 0) {
        return (<Alert severity="info" sx={{ borderRadius: 2 }}>
        Nema podataka za prikaz / No data to display
      </Alert>);
    }
    if (!keys.x || !keys.y) {
        return (<Alert severity="warning" sx={{ borderRadius: 2 }}>
    ...
    return (<Box sx={{ position: 'relative' }}>
      ...
      <Paper elevation={0} sx={{
        ...
        <svg width={width} height={height} style={{
          ...
          {(chartType === 'map' || chartType === 'scatterplot') && (<text x={width / 2} y={height / 2} textAnchor="middle" fill="#999" fontSize="16">
              {chartType} vizualizacija u razvoju
            </text>)}
        </svg>

        {/* Interactive Tooltip */}
        {tooltip.visible && (<Box sx={{
                position: 'absolute',
                left: tooltip.x,
                top: tooltip.y,
      ...
      {/* Legend */}
      {data.length > 50 && (<Typography variant="caption" color="text.secondary" sx={{ mt: 2, display: 'block', textAlign: 'center' }}>
          Prikazano prvih 50 od {data.length} redova
        </Typography>)}
    </Box>);
}
// Enhanced Bar Chart (Horizontal) with animations and interactions
function BarChart({ data, stats, width, height, setTooltip }) {
    ...
    return (<g transform={`translate(${margin.left}, ${margin.top})`}>
      {/* Grid lines */}
      {[0, 0.25, 0.5, 0.75, 1].map((tick, i) => (<g key={i}>
          <line x1={tick * chartWidth} y1={0} x2={tick * chartWidth} y2={data.length * (barHeight + 5)} stroke="#f0f0f0" strokeWidth={1}/>
          <text x={tick * chartWidth} y={data.length * (barHeight + 5) + 20} textAnchor="middle" fontSize="11" fill="#999">
      ...
      {data.map((d, i) => {
            ...
            return (<g key={i}>
            {/* Bar background */}
            <rect x={0} y={y} width={chartWidth} height={barHeight} fill="#f8f9fa" rx={4}/>
            {/* Animated bar */}
            <rect x={0} y={y} width={barWidth} height={barHeight} fill="url(#barGradient)" rx={4} filter="url(#shadow)" style={{
                    cursor: 'pointer',
            ...
            <text x={-10} y={y + barHeight / 2} textAnchor="end" alignmentBaseline="middle" fontSize="13" fill="#333" fontWeight="500">
              {d.label.length > 20 ? d.label.substring(0, 20) + '...' : d.label}
            </text>
            <text x={barWidth + 10} y={y + barHeight / 2} alignmentBaseline="middle" fontSize="12" fill="#667eea" fontWeight="700">
...
function ColumnChart({ data, stats, width, height, setTooltip }) {
    ...
    return (<g transform={`translate(${margin.left}, ${margin.top})`}>
      {/* Grid lines */}
      {[0, 0.25, 0.5, 0.75, 1].map((tick, i) => (<g key={i}>
          <line x1={0} y1={chartHeight - tick * chartHeight} x2={chartWidth} y2={chartHeight - tick * chartHeight} stroke="#f0f0f0" strokeWidth={1} strokeDasharray={tick === 0 ? "0" : "4,4"}/>
          <text x={-15} y={chartHeight - tick * chartHeight} textAnchor="end" alignmentBaseline="middle" fontSize="11" fill="#999">
            {formatNumber(tick * maxValue)}
          </text>
        </g>))}
      ...
      {/* Axis Labels */}
      <text x={-50} y={chartHeight / 2} fontSize="13" fill="#666" fontWeight="600" textAnchor="middle" transform={`rotate(-90, -50, ${chartHeight / 2})`}>
        Vrednost
      </text>
      ...
      {data.map((d, i) => {
            ...
            return (<g key={i}>
            <rect x={x} y={chartHeight - columnHeight} width={barWidth} height={columnHeight} fill="url(#barGradient)" rx={6} filter="url(#shadow)" style={{
                    cursor: 'pointer',
                    transition: 'all 0.3s cubic-bezier(0.4, 0, 0.2, 1)',
            ...
            <text x={x + barWidth / 2} y={chartHeight + 20} textAnchor="end" fontSize="11" fill="#666" fontWeight="500" transform={`rotate(-45, ${x + barWidth / 2}, ${chartHeight + 20})`}>
...
function LineChart({ data, stats, width, height, setTooltip }) {
    const margin = { top: 40, right: 40, bottom: 80, left: 70 };
    const chartWidth = width - margin.left - margin.right;
    const chartHeight = height - margin.top - margin.bottom;
    const maxValue = stats.max;
    ...
    return (<g transform={`translate(${margin.left}, ${margin.top})`}>
      {/* Grid */}
      {[0, 0.25, 0.5, 0.75, 1].map((tick, i) => (<g key={i}>
          <line x1={0} y1={chartHeight - tick * chartHeight} x2={chartWidth} y2={chartHeight - tick * chartHeight} stroke="#f0f0f0" strokeWidth={1} strokeDasharray="4,4"/>
          <text x={-15} y={chartHeight - tick * chartHeight} textAnchor="end" alignmentBaseline="middle" fontSize="11" fill="#999">
      ...
      <path d={`${pathD} L ${chartWidth} ${chartHeight} L 0 ${chartHeight} Z`} fill="url(#areaGradient)" style={{
            animation: 'fadeIn 1s ease-out'
        }}/>

      {/* Line with gradient stroke */}
      <path d={pathD} fill="none" stroke="url(#barGradient)" strokeWidth={3} strokeLinecap="round" strokeLinejoin="round" style={{
      ...
      {points.map((p, i) => (<g key={i}>
          <circle cx={p.x} cy={p.y} r={6} fill="#ffffff" stroke="url(#barGradient)" strokeWidth={3} filter="url(#shadow)" style={{
                cursor: 'pointer',
          ...
          {i % Math.max(1, Math.ceil(data.length / 8)) === 0 && (<text x={p.x} y={chartHeight + 20} textAnchor="middle" fontSize="11" fill="#666" fontWeight="500" transform={`rotate(-45, ${p.x}, ${chartHeight + 20})`}>
...
// Enhanced Area Chart
function AreaChart({ data, stats, width, height, setTooltip }) {
    const margin = { top: 40, right: 40, bottom: 80, left: 70 };
    const chartWidth = width - margin.left - margin.right;
    const chartHeight = height - margin.top - margin.bottom;
    const maxValue = stats.max;
    ...
    return (<g transform={`translate(${margin.left}, ${margin.top})`}>
      ...
      {/* Area with gradient */}
      <path d={areaD} fill="url(#areaGradient)" style={{
            animation: 'fillArea 1.5s ease-out'
        }}/>

      {/* Line */}
      <path d={pathD} fill="none" stroke="#667eea" strokeWidth={3} strokeLinecap="round"/>

      {/* Interactive points */}
      {points.map((p, i) => (<circle key={i} cx={p.x} cy={p.y} r={5} fill="#ffffff" stroke="#667eea" strokeWidth={2} style={{
                cursor: 'pointer',
                transition: 'all 0.3s ease',
                animation: `pop 0.5s ease-out ${i * 0.05}s both`
            }} onMouseEnter={(e) => {
...
function PieChart({ data, width, height, setTooltip }) {
    ...
    return (<g>
      ...
      {slices.map((_slice, i) => (<defs key={`grad-${i}`}>
      ...
      {slices.map((slice, i) => {
            ...
            return (<g key={i}>
            <path d={pathD} fill={`url(#pieGradient${i})`} stroke="white" strokeWidth={3} filter="url(#shadow)" style={{
                    cursor: 'pointer',
                    transition: 'all 0.3s cubic-bezier(0.4, 0, 0.2, 1)',
            ...
            {slice.percentage > 0.05 && (<text x={labelX} y={labelY} textAnchor="middle" alignmentBaseline="middle" fontSize="14" fill="white" fontWeight="700" style={{
      ...
      <g transform={`translate(${width - 200}, 40)`}>
        <rect width={180} height={slices.length * 32 + 20} fill="white" rx={8} filter="url(#shadow)"/>
        {slices.map((slice, i) => (<g key={`legend-${i}`} transform={`translate(15, ${15 + i * 32})`}>
            <rect width={20} height={20} fill={`url(#pieGradient${i})`} rx={4}/>
