 ...
 */
function detectVisualizationColumns(data) {
    ...
    columns.forEach(col => {
        ...
        const numericCount = sampleValues.filter(val => val !== null && val !== undefined && !isNaN(Number(val))).length;
 ...
 */
function prepareDataForVisualization(data, maxRows = 20) {
...
export const ChartVisualizer = ({ data, chartType, title, description }) => {
    ...
    return (<Box>
      {title && (<Typography variant="h6" sx={{ mb: 2, textAlign: 'center' }}>
          {title}
        </Typography>)}
      {description && (<Typography variant="body2" color="text.secondary" sx={{ mb: 2, textAlign: 'center' }}>
          {description}
        </Typography>)}
      ...
      <Box sx={{ minHeight: 400 }}>
        {chartType === 'line' && (<LineChart data={preparedData} {...commonProps} color="#4caf50"/>)}

        {chartType === 'bar' && (<BarChart data={preparedData} {...commonProps} color="#2196f3"/>)}

        {chartType === 'column' && (<ColumnChart data={preparedData} {...commonProps}/>)}

        {chartType === 'pie' && (<PieChart data={preparedData.slice(0, 10)} // Limit pie chart to 10 slices
         labelKey={columns.categoryColumn} valueKey={columns.valueColumn} width={600} height={600}/>)}

        {(chartType === 'area' || chartType === 'map' || chartType === 'scatterplot') && (<Box sx={{ p: 4, textAlign: 'center', backgroundColor: 'grey.50', borderRadius: 1 }}>
      ...
      {data.length > preparedData.length && (<Alert severity="info" sx={{ mt: 2 }}>
