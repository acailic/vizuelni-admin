 ...
 */
export const DebugView = ({ data, title = 'Debug Data View', maxRows = 5 }) => {
    ...
    return (<Box sx={{
      ...
      {displayData.length === 0 ? (<Box sx={{ color: '#666', fontStyle: 'italic' }}>
          No data available
        </Box>) : (<Box>
          {displayData.map((row, index) => (<Box key={`row-${index}-${JSON.stringify(row).substring(0, 20)}`} sx={{
                    marginBottom: '15px',
                    padding: '10px',
                    backgroundColor: '#fff',
