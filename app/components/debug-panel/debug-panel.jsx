...
const DebugInteractiveFilters = () => {
    ...
    return (<>
      ...
      <Box sx={{ p: 5 }}>
        <Inspector expandLevel={3} data={getInteractiveFiltersState()} table={false}/>
      </Box>
    </>);
};
const CubeMetadata = ({ datasetIri, dataSource, chartConfig, }) => {
    ...
    const handleChange = useEvent((_ev, expanded) => setExpanded(expanded));
    return (<Stack direction="row" spacing={2}>
      <Accordion onChange={handleChange} expanded={expanded}>
        ...
        <AccordionDetails>
          ...
          {expanded && (data === null || data === void 0 ? void 0 : data.dataCubesComponents) ? (<Inspector data={Object.fromEntries([
                [
...
const DebugConfigurator = () => {
    ...
    const sparqlEditorUrl = dataSourceToSparqlEditorUrl(configuratorState.dataSource);
    return (<>
      <Typography component="h3" variant="h4" sx={{ px: 5, color: "grey.700" }}>
        Cube Tools
      </Typography>
      {chartConfig.cubes.map((cube) => (<Stack key={cube.iri} spacing={2} sx={{ pl: 5, py: 3 }}>
          <Button component="a" color="primary" variant="text" size="sm" href={`https://cube-viewer.zazuko.com/?endpointUrl=${encodeURIComponent(configuratorState.dataSource.url)}&user=&password=&sourceGraph=&cube=${encodeURIComponent(cube.iri)}`} target="_blank" rel="noopener noreferrer" startIcon={<Icon name="legacyLinkExternal" size={16}/>}>
            <Typography variant="body2">Open in Cube Viewer</Typography>
          </Button>
          <Button component="a" color="primary" variant="text" size="sm" href={`${sparqlEditorUrl}#query=${encodeURIComponent(`#pragma describe.strategy cbd

               DESCRIBE <${cube.iri}>`)}&requestMethod=POST`} target="_blank" rel="noopener noreferrer" startIcon={<Icon name="legacyLinkExternal" size={16}/>}>
            <Typography variant="body2">Cube Metadata Query</Typography>
          </Button>
          <CubeMetadata datasetIri={cube.iri} dataSource={configuratorState.dataSource} chartConfig={chartConfig}/>
        </Stack>))}
      <Typography component="h3" variant="h4" sx={{ px: 5, display: "flex", alignItems: "center", color: "grey.700" }}>
        Configurator State{" "}
        <Button component="span" variant="text" size="sm" onClick={() => {
...
export const DebugPanel = (props) => {
    ...
    return (<Accordion disableGutters className={classes.debugPanel} square {...DISABLE_SCREENSHOT_ATTR}>
      <AccordionSummary expandIcon={<Box p={2}>
