...
export function NavBar() {
    ...
    const renderNavigationItems = () => {
        return navigationItems.map((item) => (<Link key={item.href} href={item.href} passHref legacyBehavior>
    ...
    const renderMobileNavigationItems = () => {
        return navigationItems.map((item) => (<ListItem key={item.href} disablePadding>
    ...
    return (<>
      <AppBar position="static" sx={{ backgroundColor: 'primary.main' }}>
        <Toolbar>
          ...
          {/* Mobile Menu Button */}
          {isMobile && (<IconButton size="large" edge="start" color="inherit" aria-label="menu" onClick={handleMobileMenuToggle}>
              <MenuIcon />
            </IconButton>)}
        </Toolbar>
      </AppBar>
