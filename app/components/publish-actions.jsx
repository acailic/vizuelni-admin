...
export const PublishActions = (props) => {
...
export const TriggeredPopover = (props) => {
    ...
    return (<>
      ...
      <Popover open={!!anchorEl} anchorEl={anchorEl} {...popoverProps} anchorPosition={popoverProps.anchorPosition
            ? {
                top: ((_a = popoverProps.anchorPosition) === null || _a === void 0 ? void 0 : _a.top) - height / 2,
                left: ((_b = popoverProps.anchorPosition) === null || _b === void 0 ? void 0 : _b.left) - width,
            }
            : undefined} elevation={4} onClose={() => setAnchorEl(undefined)}>
        <Box ref={ref}>{children}</Box>
      </Popover>
    </>);
};
const Embed = ({ chartWrapperRef, configKey, locale }) => {
    const [iframeHeight, setIframeHeight] = useState(0);
    const handlePopoverOpen = useEvent(() => {
        if (chartWrapperRef === null || chartWrapperRef === void 0 ? void 0 : chartWrapperRef.current) {
            const height = Math.ceil(chartWrapperRef.current.getBoundingClientRect().height);
            setIframeHeight(height);
        }
    });
    return (<TriggeredPopover popoverProps={{
            anchorOrigin: {
                vertical: "bottom",
                horizontal: "right",
            },
        ...
        }} renderTrigger={(setAnchorEl) => (<Button size="sm" startIcon={<Icon name="embed" size={20}/>} onClick={(e) => setAnchorEl(e.currentTarget)}>
          <Trans id="button.embed">Embed</Trans>
        </Button>)}>
      <EmbedContent iframeHeight={iframeHeight} configKey={configKey} locale={locale}/>
    </TriggeredPopover>);
};
const EmbedRadio = ({ infoMessage, ...rest }) => {
    return (<Flex sx={{ alignItems: "center", gap: 1 }}>
      <Radio {...rest}/>
      {infoMessage && (<Tooltip arrow title={infoMessage} PopperProps={{ sx: { width: 190, p: 0 } }}>
...
const EmbedToggleSwitch = ({ infoMessage, label, ...rest }) => {
    return (<Flex sx={{ alignItems: "center", gap: 1 }}>
      <FormControlLabel control={<Switch {...rest}/>} label={<Typography variant="body2">{label}</Typography>} sx={{ mr: 0 }}/>
      {infoMessage && (<Tooltip arrow title={infoMessage} PopperProps={{ sx: { width: 190, p: 0 } }}>
          <div style={{ fontSize: 0 }}>
            <Icon name="infoCircle" size={16}/>
          </div>
...
const Share = ({ configKey, locale }) => {
    return (<TriggeredPopover popoverProps={{
            ...
            return (<Button size="sm" onClick={(e) => {
...
export const EmbedContent = ({ locale, configKey, iframeHeight, }) => {
    ...
    return (<Flex sx={{ flexDirection: "column", gap: 4, p: 4 }}>
      <Flex sx={{ flexDirection: "column", gap: 2 }}>
        ...
        <Flex sx={{ alignItems: "center", gap: 4, mt: 3, mb: 2 }}>
          <EmbedRadio value="responsive" checked={responsive} onChange={() => setResponsive(true)} label={t({
          ...
          <EmbedRadio value="static" checked={!responsive} onChange={() => setResponsive(false)} label={t({
            id: "publication.embed.iframe.static",
            message: "Static iframe",
        })} infoMessage={t({
            id: "publication.embed.iframe.static.warn",
            message: "For embedding visualizations in systems without JavaScript support (e.g. WordPress).",
        })}/>
        </Flex>
        <Accordion defaultExpanded={Object.values(embedParams).some((d) => d)} sx={{
            boxShadow: 0,
            "&::before": {
                display: "none",
            },
          ...
          <AccordionDetails sx={{ pt: 3 }}>
            <EmbedToggleSwitch checked={embedParams.removeBorder} onChange={(_, checked) => {
            ...
            message: "For embedding visualizations in systems without a border.",
        })}/>
            <EmbedToggleSwitch checked={embedParams.optimizeSpace} onChange={(_, checked) => {
            ...
            <EmbedToggleSwitch checked={embedParams.removeMoreOptionsButton} onChange={(_, checked) => {
            setEmbedQueryParam("removeMoreOptionsButton", checked);
        }} label={t({
            id: "publication.embed.iframe.remove-more-options-button",
            message: "Remove options for table view, copy & edit, sharing, and downloading",
        })}/>
            <EmbedToggleSwitch checked={embedParams.removeLabelsInteractivity} onChange={(_, checked) => {
            ...
            <EmbedToggleSwitch checked={embedParams.removeFootnotes} onChange={(_, checked) => {
            setEmbedQueryParam("removeFootnotes", checked);
        }} label={t({
            id: "publication.embed.iframe.remove-legend",
            message: "Hide footnotes",
        })}/>
            <EmbedToggleSwitch checked={embedParams.removeFilters} onChange={(_, checked) => {
...
export const ShareContent = ({ configKey, locale, }) => {
    const [shareUrl, setShareUrl] = useState("");
    const i18n = useI18n();
    useEffect(() => {
        setShareUrl(`${window.location.origin}/${locale}/v/${configKey}`);
    }, [configKey, locale]);
    return (<Flex sx={{ flexDirection: "column", gap: 2, p: 4 }}>
      <Flex sx={{ justifyContent: "space-between", alignItems: "center" }}>
        <Typography variant="h6" component="div" sx={{ fontWeight: 700 }}>
          <Trans id="publication.popup.share">Share</Trans>
        </Typography>
        <Flex>
          <IconButton title={i18n._(t({
            id: "publication.share.linktitle.linkedin",
            message: `Share on LinkedIn`,
        }))} href={`https://www.linkedin.com/sharing/share-offsite/?url=${encodeURIComponent(shareUrl)}`} target="_blank" rel="noopener noreferrer">
            <Icon name="linkedIn"/>
          </IconButton>
          <IconButton title={i18n._(t({
            id: "publication.share.linktitle.x",
            message: `Share on X (Twitter)`,
        }))} href={`https://twitter.com/intent/tweet?url=${encodeURIComponent(shareUrl)}&via=bafuCH`} target="_blank" rel="noopener noreferrer">
            <Icon name="twitter"/>
          </IconButton>
          <IconButton title={i18n._(t({
            id: "publication.share.linktitle.facebook",
            message: `Share on Facebook`,
        }))} href={`https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(shareUrl)}`} target="_blank" rel="noopener noreferrer">
            <Icon name="facebook"/>
          </IconButton>
          <IconButton title={i18n._(t({
            id: "publication.share.linktitle.mail",
            message: `Share via email`,
        }))} href={`mailto:?subject=${i18n._(t({
