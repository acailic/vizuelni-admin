...
export const ChartPanelLayoutTall = ({ blocks, renderBlock, }) => {
    ...
    return (<>
      {rows.map((row, i) => (<ChartPanelLayoutTallRow key={i} row={row}/>))}
    </>);
};
const ChartPanelLayoutTallRow = ({ row }) => {
    ...
    switch (row.type) {
        ...
        case "narrow":
            ...
            return (<Box className={classes.root}>{row.blocks.map(row.renderBlock)}</Box>);
    }
};
const getChartPanelLayoutTallRows = ({ blocks, renderBlock, }) => {
    const rows = [];
