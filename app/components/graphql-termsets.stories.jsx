...
export const Termsets = () => {
    var _a;
    const [cube, setCube] = useState("https://energy.ld.admin.ch/sfoe/bfe_ogd84_einmalverguetung_fuer_photovoltaikanlagen/9");
    ...
    return (<div>
      ...
      <Stack gap={1} direction="column" alignItems="start">
        <FormControlLabel label="Compatible cube" labelPlacement="start" control={<Stack gap={1} direction="row" alignItems="center">
              <Select size="sm" native onChange={(ev) => setCube(ev.target.value)} value={cube}>
      ...
      <Stack mt={2} gap={2} direction="column">
        {(_a = data === null || data === void 0 ? void 0 : data.dataCubeComponentTermsets) === null || _a === void 0 ? void 0 : _a.map((componentTermsets) => {
            return (<Stack gap={1} direction="column" key={componentTermsets.iri}>
              ...
              <Stack gap={1} direction="row">
                {componentTermsets.termsets.map((termset) => {
                    return (<Tag key={termset.iri} type="termset">
                      {termset.label}
                    </Tag>);
                })}
              </Stack>
