...
export const DragHandle = forwardRef((props, ref) => {
    ...
    return (<Box {...DISABLE_SCREENSHOT_ATTR} ref={ref} {...rest} className={clsx(classes.dragIconWrapper, props.className, chartPanelLayoutGridClasses.dragHandle)}>
      <Icon name="dragndrop"/>
    </Box>);
});
export const useDragOverClasses = makeStyles((theme) => ({
    root: {
        opacity: ({ isDragging, isDragOver }) => {
            return isDragging ? 0.2 : isDragOver ? 0.8 : 1;
        },
        outline: ({ isDragging, isDragOver }) => {
            return `2px solid ${isDragOver && !isDragging ? theme.palette.primary.main : "transparent"}`;
        },
