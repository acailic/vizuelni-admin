/**
 * Standalone Area Chart Component
 *
 * A D3-based area chart component that works in any React application.
 * Decoupled from Next.js and framework-agnostic.
 *
 * @example
 * ```tsx
 * import { AreaChart } from '@acailic/vizualni-admin/charts';
 *
 * function MyChart() {
 *   const data = [
 *     { year: '2020', value: 100 },
...
export interface AreaChartProps extends Omit<ChartProps, "config"> {
  config: BaseChartConfig & {
    /** Show line stroke on top of area (default: minimal) */
    showLine?: boolean;
    ...
    /** Area stacking mode: 'overlap' | 'stack' */
    stackMode?: "overlap" | "stack";
    /** Opacity of area fill (0-1, default: 0.7) */
    areaOpacity?: number;
    /** Gradient start opacity (0-1, default: 0.5) */
    gradientStartOpacity?: number;
    /** Gradient end opacity (0-1, default: 0.05) */
...
export const AreaChart = memo(
  ({
    ...
    useEffect(() => {
      ...
      // Calculate stacked values if in stack mode
      const stackedData =
        stackMode === "stack"
          ? data.map((d) => {
      ...
      seriesKeys.forEach((key, index) => {
        ...
        const dataToUse = stackMode === "stack" ? stackedData : data;
        ...
        const areaGenerator = area<any>()
          ...
          .y0((d) => {
            if (stackMode === "stack" && index > 0) {
              const prevKey = seriesKeys[index - 1];
              return yScale(Number(d[prevKey]) || 0);
            }
            return innerHeight;
          })
        ...
        // Add line stroke on top if requested
        if (showLine) {
          ...
          linePath
            ...
            .duration(animated ? animationDuration * 0.5 : 0)
            .ease(easeCubicOut)
            .attr("opacity", 1);
        }

        // Add dots with staggered animation (optional, for visual clarity)
        g.selectAll(`.dot-${index}`)
          ...
          .transition()
          .delay(animated ? animationDuration * 0.6 + index * 100 : 0)
      ...
      if (seriesKeys.length > 1) {
        ...
        seriesKeys.forEach((key, i) => {
          ...
          // Add area icon for legend
          const seriesColor = professionalColors[i % professionalColors.length];
