...
const getSource = (dataSourceType) => {
...
export const resolveDimensionType = (dataKind, scaleType, timeUnit, related) => {
    const relatedTypes = Array.from(new Set(related.map((d) => d.type)));
    if (relatedTypes.length > 1) {
        console.warn(`WARNING: dimension has more than 1 related type`, relatedTypes);
    }
        ...
        if (scaleType === "Ordinal") {
            if (timeUnit) {
                if (timeUnit === TimeUnit.Month || timeUnit === TimeUnit.Year) {
                    ...
                    throw Error(`Unsupported time unit for TemporalEntityDimension: ${timeUnit}`);
                }
            }
...
export const resolveMeasureType = (scaleType) => {
    return scaleType === "Ordinal" ? "OrdinalMeasure" : "NumericalMeasure";
};
export const datasourceUrlValue = (url) => {
...
export const datasourceValidationError = () => {
    return new GraphQLError("BAD_USER_INPUT: Provided value is not an allowed data source");
};
