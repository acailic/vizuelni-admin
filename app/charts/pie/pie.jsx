...
export const Pie = () => {
    const { bounds: { width, height }, arcs, arcGenerator, outerRadius, getSegment, colors, getRenderingKey, getY, } = useChartState();
    ...
    const handleHover = useEvent((el, observation, { segment }) => {
        if (!isEditingAnnotation) {
            select(el).attr("stroke", "black").attr("stroke-width", 1);
        }
        onHover(observation, { segment });
    });
    const handleHoverOut = useEvent((el) => {
        if (!isEditingAnnotation) {
    ...
    useEffect(() => {
        ...
        if (piesContainer && labelsContainer && connectorsContainer) {
            ...
            renderContainer(piesContainer, {
                ...common,
                id: "pies",
                render: (g, opts) => renderPies(g, renderData, {
                    ...opts,
                    arcGenerator,
                    onClick,
                    onHover: handleHover,
            ...
            renderContainer(connectorsContainer, {
                ...common,
                id: "connectors",
                render: (g, opts) => renderPieValueLabelConnectors(g, valueLabelsData, {
                    ...opts,
                }),
            });
