...
export const TableContentProvider = ({ headerGroups, tableColumnsMeta, customSortCount, totalColumnsWidth, children, }) => {
    ...
    return (<TableContentContext.Provider value={value}>
      {children}
...
export const TableContent = ({ children }) => {
    ...
    const { headerGroups, tableColumnsMeta, customSortCount, totalColumnsWidth } = ctx;
    return (<>
        ...
        {headerGroups.map((headerGroup) => {
            ...
            <Box {...headerGroup.getHeaderGroupProps()}>
              {headerGroup.headers.map((column) => {
                    const { dim, columnComponentType } = tableColumnsMeta[column.id];
                    ...
                    return (
                    // eslint-disable-next-line react/jsx-key
                    <Flex className={clsx(classes.headerGroup, columnComponentType === "NumericalMeasure"
                            ? classes.headerGroupMeasure
                            : undefined)} {...column.getHeaderProps(column.getSortByToggleProps())}>
                    <TableSortLabel active={isCustomSorted} direction={column.isSortedDesc ? "desc" : "asc"} sx={{
                            "& svg": {
