...
export const CellDesktop = ({ cell, columnMeta, }) => {
    const { columnComponentType, textStyle, textColor, columnColor, barColorPositive, barColorNegative, barColorBackground, barShowBackground, } = columnMeta;
    const classes = useStyles();
    const { links } = useChartState();
    switch (columnMeta.type) {
        case "text":
            return (<Flex sx={{
                    alignItems: "center",
                    justifyContent: columnComponentType === "NumericalMeasure"
                        ? "flex-end"
                        : "flex-start",
                    color: textColor,
        ...
        case "category":
            ...
            return (<Flex sx={{ alignItems: "center", fontWeight: textStyle, pl: 1, pr: 3 }} {...cell.getCellProps()}>
          <LinkedCellWrapper cell={cell} columnMeta={columnMeta} links={links}>
        ...
        case "heatmap":
            ...
            return (<Flex className={classes.heatmapCell} sx={{
                    color: isNull
                        ? textColor
        ...
        case "bar":
          ...
          <LinkedCellWrapper cell={cell} columnMeta={columnMeta} links={links}>
            ...
            {cell.value !== null && widthScale && (<Box sx={{
                ...
                <Box className={classes.barForeground} sx={{
                ...
                <Box className={classes.barBackground} sx={{
                        left: `${cell.value < 0
                            ? widthScale(0)
                            : getBarLeftOffset(cell.value, widthScale)}px`,
                    }}/>
              </Box>)}
          </LinkedCellWrapper>
        </Flex>);
        default:
            return (<Flex className={classes.defaultCell} sx={{
                    justifyContent: columnComponentType === "NumericalMeasure"
                        ? "flex-end"
                        : "flex-start",
                    textAlign: columnComponentType === "NumericalMeasure" ? "right" : "left",
                    color: textColor,
                    backgroundColor: columnColor,
                    fontWeight: textStyle,
                }} {...cell.getCellProps()}>
...
export const getBarLeftOffset = (v, scale) => {
    if (v >= 0) {
...
export const getBarWidth = (v, scale) => {
    if (v >= 0) {
