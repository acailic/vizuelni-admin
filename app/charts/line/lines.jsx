...
export const ErrorWhiskers = () => {
    const { getX, getY, getYErrorPresent, getYErrorRange, chartData, yScale, xScale, showYUncertainty, colors, getSegment, bounds, } = useChartState();
...
export const Lines = ({ dotSize }) => {
    const { getX, xScale, getY, yScale, grouped, colors, bounds } = useChartState();
    const { margins } = bounds;
    const { labelFontSize, fontFamily } = useChartTheme();
    const valueLabelsContainerRef = useRef(null);
    ...
    return (<g transform={`translate(${margins.left} ${margins.top})`}>
      {Array.from(grouped).map((observation, i) => {
            return (<Fragment key={observation[0]}>
            <Line key={i} path={lineGenerator(observation[1])} color={colors(observation[0])}/>
          </Fragment>);
        })}
...
const Line = memo(function Line({ path, color, }) {
    return <path data-testid="chart-line" d={path} stroke={color} fill="none"/>;
});
const getPointRadius = (dotSize) => {
    switch (dotSize) {
        case "Small":
            return 2;
        case "Medium":
        ...
        case undefined:
            return 2;
        default:
            const _exhaustiveCheck = dotSize;
            return _exhaustiveCheck;
    }
};
const Points = ({ dotSize }) => {
    const { getX, xScale, getY, yScale, bounds, chartData, getSegment, colors } = useChartState();
    ...
    return (<g>
      {dots.map(({ x, y, fill }, i) => (<circle key={i} cx={x} cy={y} r={r} fill={fill}/>))}
    </g>);
};
