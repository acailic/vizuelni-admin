...
export const ErrorWhiskers = () => {
    const { bounds, xScale, yScaleIn, getXErrorRange, getXErrorPresent, yScale, getSegment, grouped, showXUncertainty, } = useChartState();
    ...
    const renderData = useMemo(() => {
        ...
        return grouped
            ...
            .flatMap(([segment, observations]) => observations.map((d) => {
            const y0 = yScaleIn(getSegment(d));
            const barHeight = Math.min(bandwidth, 15);
            const [x1, x2] = getXErrorRange(d);
            return {
                key: `${segment}-${getSegment(d)}`,
                y: yScale(segment) + y0 + bandwidth / 2 - barHeight / 2,
                x1: xScale(x1),
                x2: xScale(x2),
                height: barHeight,
            };
...
export const BarsGrouped = () => {
    const { bounds, xScale, yScaleIn, getX, yScale, getSegment, colors, grouped, getRenderingKey, } = useChartState();
    const ref = useRef(null);
    const enableTransition = useTransitionStore((state) => state.enable);
    const transitionDuration = useTransitionStore((state) => state.duration);
    ...
    const renderData = useMemo(() => {
        return grouped.flatMap(([segment, observations]) => {
            return observations.map((d) => {
                ...
                const x = (_a = getX(d)) !== null && _a !== void 0 ? _a : NaN;
                return {
                    key,
                    y: yScale(segment) + yScaleIn(y),
