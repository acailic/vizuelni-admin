...
import { useBandYVariables, useBaseVariables, useChartData, useInteractiveFiltersVariables, useNumericalXErrorVariables, useNumericalXVariables, useSegmentVariables, } from "@/charts/shared/chart-state";
import { useRenderingKeyVariable } from "@/charts/shared/rendering-utils";
import { useChartConfigFilters } from "@/config-utils";
import { isTemporalEntityDimension } from "@/domain/data";
import { sortByIndex } from "@/utils/array";
export const useBarsGroupedStateVariables = (props) => {
    ...
    const sortData = useCallback((data) => {
        ...
        const yGetter = isTemporalEntityDimension(yDimension)
            ...
            : getY;
        const order = [
            ...rollup(data, (v) => sum(v, (d) => getX(d)), (d) => yGetter(d)),
        ]
...
export const useBarsGroupedStateData = (chartProps, variables) => {
    const { chartConfig, observations } = chartProps;
    const { sortData, yDimension, getYAsDate, getX, getSegmentAbbreviationOrLabel, getTimeRangeDate, } = variables;
