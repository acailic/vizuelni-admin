 ...
 */
const ChartDataWrapperInner = ({ chartConfig, LoadingOverlayComponent = LoadingOverlay, Component, ComponentProps, componentIds, dataSource, embedParams, observationQueryFilters, fetching: fetchingProp = false, error: propError, }) => {
    var _a;
    ...
    const { data: metadataData, fetching: fetchingMetadata, error: metadataError, } = metadataQuery;
    const { data: componentsData, fetching: fetchingComponents, error: componentsError, } = componentsQuery;
    const { data: observationsData, fetching: fetchingObservations, error: observationsError, } = observationsQuery;
    ...
    const error = propError || metadataError || componentsError || observationsError;
    useEffect(() => {
        chartLoadingState.set("data", fetching);
    }, [chartLoadingState, fetching]);
    const { dimensionsById, measuresById } = useMemo(() => {
        return {
            dimensionsById: keyBy(dimensions !== null && dimensions !== void 0 ? dimensions : [], (d) => d.id),
            measuresById: keyBy(measures !== null && measures !== void 0 ? measures : [], (d) => d.id),
        };
    }, [dimensions, measures]);
    if (error) {
        ...
        return (<div className={classes.dataWrapper} data-chart-loaded={!chartLoadingState.loading}>
        {observations.length > 0 && (<A11yTable title={title} observations={observations} dimensions={dimensions} measures={measures}/>)}
        ...
        <AnimatePresence>
          {chartLoadingState.loading ? (<LoadingOverlayComponent />) : observations.length === 0 ? (<NoDataHint />) : null}
        </AnimatePresence>
      </div>);
    }
    else {
 ...
 */
export const ChartDataWrapper = (props) => {
    const key = useMemo(() => props.chartConfig.cubes
        .map((c) => `${c.iri}${c.joinBy ? `:${c.joinBy}` : ""}`)
        .join(" / "), [props.chartConfig.cubes]);
    return <ChartDataWrapperInner key={key} {...props}/>;
};
const Error = ({ message }) => {
    return (<Flex flexGrow={1} justifyContent="center" minHeight={300}>
