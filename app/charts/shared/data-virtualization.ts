...
export class QuadTreeIndex implements SpatialIndex {
  ...
  private buildIndex() {
    ...
    const grid: DataPoint[][] = Array(rows * cols)
      .fill(null)
      .map(() => []);
  ...
  getPointsInBounds(viewport: Viewport): DataPoint[] {
    ...
    const startCol = Math.max(
      0,
      Math.floor((viewport.x - this.bounds.minX) / gridSize)
    );
    const endCol = Math.min(
      cols - 1,
      Math.floor((viewport.x + viewport.width - this.bounds.minX) / gridSize)
    );
    const startRow = Math.max(
      0,
      Math.floor((viewport.y - this.bounds.minY) / gridSize)
    );
    const endRow = Math.min(
      rows - 1,
      Math.floor((viewport.y + viewport.height - this.bounds.minY) / gridSize)
    );
...
export function useDataVirtualization(
  ...
  const getOptimizedRenderData = useCallback(() => {
    ...
    const optimalPointSize = lodManager.getOptimalPointSize(
      viewport,
      data.length
    );
    ...
    return visiblePoints
      .filter(
        (_, index) =>
          !lodManager.shouldSkipPoint(index, visiblePoints.length, detailLevel)
      )
      .map((point) => ({
        ...point,
  ...
  const getNearestPoints = useCallback(
    (x: number, y: number, radius: number, maxPoints?: number) => {
      if (spatialIndex) {
