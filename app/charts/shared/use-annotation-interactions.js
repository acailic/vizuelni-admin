...
export const useAnnotationInteractions = ({ focusingSegment, }) => {
    ...
    const onClick = useEvent((observation, { segment }) => {
        if (!isEditingAnnotation || !activeAnnotation || !activeField) {
            return;
        }
        const activeNewTargets = getAnnotationTargetsFromObservation(observation, {
            chartConfig,
            definitiveFilters,
            segment,
        });
        for (const annotation of annotations) {
            ...
            const newTargets = annotation.key !== activeField
    ...
    const onHover = useEvent((observation, { segment }) => {
        dispatchInteraction({
