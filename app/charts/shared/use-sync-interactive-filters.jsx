 ...
 *
 */
export const useSyncInteractiveFilters = (chartConfig, dashboardFilters) => {
    ...
    const setCalculationType = useChartInteractiveFilters((d) => d.setCalculationType);
    const lastOverridesRef = useRef({});
    const isFirstRunRef = useRef(true);
    // Time range filter
    const presetFrom = interactiveFiltersConfig.timeRange.presets.from &&
        parseDate(interactiveFiltersConfig.timeRange.presets.from);
    const presetTo = interactiveFiltersConfig.timeRange.presets.to &&
        parseDate(interactiveFiltersConfig.timeRange.presets.to);
    const presetFromStr = presetFrom === null || presetFrom === void 0 ? void 0 : presetFrom.toString();
    const presetToStr = presetTo === null || presetTo === void 0 ? void 0 : presetTo.toString();
    useEffect(() => {
    ...
    const dashboardComponentIds = dashboardFilters === null || dashboardFilters === void 0 ? void 0 : dashboardFilters.dataFilters.componentIds;
    const newPotentialInteractiveDataFilters = useMemo(() => {
        if (componentIds) {
            ...
            return componentIds.concat(dashboardComponentIds !== null && dashboardComponentIds !== void 0 ? dashboardComponentIds : []);
        }
    ...
    useEffect(() => {
        ...
        const next = newPotentialInteractiveDataFilters.reduce((acc, iri) => {
            var _a;
            const dashboardFilter = dashboardFilters === null || dashboardFilters === void 0 ? void 0 : dashboardFilters.dataFilters.filters[iri];
            if ((dashboardFilter === null || dashboardFilter === void 0 ? void 0 : dashboardFilter.type) === "single") {
                acc[iri] = dashboardFilter;
                return acc;
            }
            const configFilter = filters[iri];
            if (!configFilter || configFilter.type === "multi") {
                const allowed = (_a = configFilter === null || configFilter === void 0 ? void 0 : configFilter.values) !== null && _a !== void 0 ? _a : {};
                const isAllowed = (val) => val !== undefined ? !configFilter || !!allowed[`${val}`] : false;
                const current = currentDataFilters[iri];
                const override = interactiveFiltersConfig.dataFilters.defaultValueOverrides[iri];
                const lastOverride = lastOverridesRef.current[iri];
                const overrideChanged = isFirstRunRef.current || !isEqual(override, lastOverride);
                if (overrideChanged && override && Array.isArray(override)) {
                    const validOverrides = override.filter((val) => isAllowed(val));
                    const configuredFilterType = interactiveFiltersConfig.dataFilters.filterTypes[iri];
                    if (configuredFilterType === "multi") {
                        acc[iri] = {
                            type: "multi",
                            values: Object.fromEntries(validOverrides.map((v) => [v, true])),
                        };
                    }
                ...
                if ((current === null || current === void 0 ? void 0 : current.type) === "single" && isAllowed(current.value)) {
                    acc[iri] = current;
                    return acc;
                }
                if ((current === null || current === void 0 ? void 0 : current.type) === "multi") {
            ...
            const current = currentDataFilters[iri];
            if ((current === null || current === void 0 ? void 0 : current.type) === "single" && current.value !== FIELD_VALUE_NONE) {
                acc[iri] = current;
            }
            else if ((configFilter === null || configFilter === void 0 ? void 0 : configFilter.type) === "single") {
        ...
        dashboardFilters === null || dashboardFilters === void 0 ? void 0 : dashboardFilters.dataFilters.filters,
        filters,
    ...
    useEffect(() => {
        ...
        if ((prev === null || prev === void 0 ? void 0 : prev.type) === "single" || (next === null || next === void 0 ? void 0 : next.type) === "single") {
            updateDataFilter(dimensionId, (next === null || next === void 0 ? void 0 : next.type) === "single" ? next.value : FIELD_VALUE_NONE);
        }
    }, [changes, updateDataFilter]);
    // Maybe it should be more generic?
    const interactiveCategoriesResetTrigger = useMemo(() => (isSegmentInConfig(chartConfig) ? chartConfig.fields.segment : null), [chartConfig]);
    // Interactive legend
    ...
    useEffect(() => resetCategories(), [resetCategories, interactiveCategoriesResetTrigger]);
    // Calculation
