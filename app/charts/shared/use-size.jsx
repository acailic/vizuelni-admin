...
export const Observer = ({ children }) => {
    ...
    const isResizing = prevWidth !== width ||
        prevHeight !== height ||
    ...
    return (<ChartObserverContext.Provider value={size}>
      {children}
    </ChartObserverContext.Provider>);
};
const ChartObserverContext = createContext(undefined);
export const useSize = () => {
    const ctx = useContext(ChartObserverContext);
    if (ctx === undefined) {
        throw Error("You need to wrap your component in <ChartObserverContextProvider /> to useSize()");
    }
...
export const useObserverRef = () => {
    const ctx = useContext(ChartObserverContext);
    if (ctx === undefined) {
        throw Error("You need to wrap your component in <ChartObserverContextProvider /> to useObserverRef()");
    }
    return ctx.ref;
};
