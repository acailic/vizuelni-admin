...
export const ChartContainer = ({ children }) => {
    ...
    key={chartConfig.chartType} ref={ref} aria-hidden="true" className={classes.chartContainer} style={{
...
export const ChartSvg = ({ children }) => {
    ...
    useEffect(() => {
        if (ref.current) {
            ...
            if (!ref.current.getAttribute("height")) {
                ref.current.setAttribute("height", `${chartHeight + margins.bottom + margins.top}`);
            }
            const sel = select(ref.current);
            (enableTransition
                ? sel.transition().duration(transitionDuration)
                : sel).attr("height", `${chartHeight + margins.bottom + margins.top}`);
        }
    }, [
        chartHeight,
        margins.bottom,
    ...
    return (<svg ref={ref} id={CHART_SVG_ID} width={width} style={{ position: "absolute", left: 0, top: 0 }}>
      {interactiveFiltersConfig.calculation.active && (<foreignObject {...DISABLE_SCREENSHOT_ATTR} width={width - margins.right} y={0} height={26} style={{ display: "flex" }}>
          <CalculationToggle />
        </foreignObject>)}
      {children}
    </svg>);
};
export const ChartControlsContainer = (props) => {
    const { sx, ...rest } = props;
