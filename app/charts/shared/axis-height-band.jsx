...
export const AxisHeightBand = () => {
    ...
    const { xScale, getYLabel, yTimeUnit, yScale, bounds, yDimension, leftAxisLabelSize, leftAxisLabelOffsetTop, formatYAxisTick, } = useChartState();
    const enableTransition = useTransitionStore((state) => state.enable);
    const transitionDuration = useTransitionStore((state) => state.duration);
    const formatDate = useTimeFormatUnit();
    const { chartHeight, margins } = bounds;
    const { labelColor, axisLabelFontSize, gridColor, labelFontSize, fontFamily, domainColor, } = useChartTheme();
    ...
    useEffect(() => {
        if (ref.current) {
            ...
            const axis = axisLeft(yScale)
                ...
                .tickFormat(formatYAxisTick !== null && formatYAxisTick !== void 0 ? formatYAxisTick : ((tick) => tick));
    ...
    return (<>
      <g ref={ref}/>
      <foreignObject y={leftAxisLabelOffsetTop} width={leftAxisLabelSize.width} height={leftAxisLabelSize.height} style={{ display: "flex" }}>
...
export const AxisHeightBandDomain = () => {
    ...
    const enableTransition = useTransitionStore((state) => state.enable);
    const transitionDuration = useTransitionStore((state) => state.duration);
    const { xScale, yScale, bounds, minY } = useChartState();
    const { chartHeight, margins } = bounds;
