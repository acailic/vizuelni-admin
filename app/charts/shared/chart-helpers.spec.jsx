...
const makeCubeNsGetters = (cubeIri) => ({
    col: (col) => `${cubeIri}/dimension/${col}`,
    val: (col, n) => `${cubeIri}/dimension/${col}/${n}`,
});
const { col, val } = makeCubeNsGetters("http://environment.ld.admin.ch/foen/px/0703010000_105");
...
const commonInteractiveFiltersState = {
    categories: {
        type: true,
    },
    timeRange: {
...
describe("useQueryFilters", () => {
    it("should merge interactive filters with meaningful values even when disabled at publish time", () => {
        ...
        const queryFilters = prepareCubeQueryFilters({
            chartConfig: line1Fixture.data.chartConfig,
            cubeFilters: line1Fixture.data.chartConfig.filters,
    ...
    it("should scope interactive filters to cube filters", () => {
        ...
        const { result: queryFilters } = renderHook((props) => useQueryFilters(props), {
    ...
    it("should handle correctly dashboard filters", () => {
        useChartInteractiveFilters.mockImplementation(() => ({}));
        const chartConfig = {
            chartType: "line",
            interactiveFiltersConfig: {
                dataFilters: {
        ...
        const { result: queryFilters } = renderHook(useQueryFilters, {
            initialProps: {
                chartConfig,
                dashboardFilters: {
                    timeRange: {
    ...
    it("should handle multi-filter with FIELD_VALUE_NONE", () => {
        ...
        const interactiveDataFiltersWithNone = {
            [col("3")]: { type: "single", value: FIELD_VALUE_NONE },
        };
        const queryFilters = prepareCubeQueryFilters({
            chartConfig: line1Fixture.data.chartConfig,
    ...
    it("should prioritize dashboard filters over interactive filters", () => {
        const dashboardFilters = {
            ...
            dataFilters: {
                componentIds: [col("3")],
                filters: {
                    [col("3")]: { type: "single", value: val("3", "dashboard") },
                },
            },
        };
        const queryFilters = prepareCubeQueryFilters({
    ...
    it("should apply multi-filters in published charts", () => {
        ...
        const queryFilters = prepareCubeQueryFilters({
            ...
            dashboardFilters: undefined,
            interactiveDataFilters: interactiveDataFiltersWithMulti,
        });
        expect(queryFilters[col("3")]).toEqual({
            type: "multi",
            values: {
...
describe("getChartConfigComponentsIds", () => {
    ...
    it("should return correct componentsIds for line chart", async () => {
        lineConfig = await migrateChartConfig(line1Fixture.data.chartConfig, migrationOptions);
        mapConfig = await migrateChartConfig(map1Fixture.data.chartConfig, migrationOptions);
    ...
    it("should return correct componentsIds for dual line chart (join by)", async () => {
        const dualLineConfig = await migrateChartConfig(dualLine1Fixture, migrationOptions);
        const componentIds = extractChartConfigComponentIds({
            chartConfig: dualLineConfig,
        });
        expect(componentIds.includes("https://energy.ld.admin.ch/sfoe/bfe_ogd84_einmalverguetung_fuer_photovoltaikanlagen/9(VISUALIZE.ADMIN_COMPONENT_ID_SEPARATOR)https://energy.ld.admin.ch/sfoe/bfe_ogd84_einmalverguetung_fuer_photovoltaikanlagen/Jahr")).toBe(true);
