...
export const AxisWidthBand = () => {
    ...
    const { xScale, getXLabel, xDimension, xTimeUnit, formatXDate, yScale, bounds: { chartHeight, chartWidth, margins }, xAxisLabel, bottomAxisLabelSize, formatXAxisTick, } = useChartState();
    const { labelColor, gridColor, labelFontSize, fontFamily, axisLabelFontSize, domainColor, } = useChartTheme();
    const enableTransition = useTransitionStore((state) => state.enable);
    const transitionDuration = useTransitionStore((state) => state.duration);
    const resolvedFontSize = xScale.bandwidth() > labelFontSize ? labelFontSize : xScale.bandwidth();
    const xAxisTitleOffset = useXAxisTitleOffset(xScale, getXLabel, xTimeUnit, resolvedFontSize);
    useEffect(() => {
        if (ref.current) {
            const rotation = true;
            ...
            const axis = axisBottom(xScale)
                ...
                .tickFormat(formatXAxisTick !== null && formatXAxisTick !== void 0 ? formatXAxisTick : ((tick) => tick));
            ...
            g.selectAll(".tick line").attr("stroke", hasNegativeValues ? gridColor : domainColor);
            g.selectAll(".tick text")
                .attr("transform", rotation ? "rotate(90)" : "rotate(0)")
                .attr("x", rotation
                ? resolvedFontSize + (labelFontSize - resolvedFontSize) * 0.6
                : 0)
                .attr("dy", hasNegativeValues
    ...
    return (<>
      <foreignObject x={margins.left + chartWidth / 2 - bottomAxisLabelSize.width / 2} y={margins.top + chartHeight + xAxisTitleOffset + 8} width={chartWidth} height={bottomAxisLabelSize.height} style={{ display: "flex" }}>
