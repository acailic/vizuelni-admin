...
const setup = async ({ modifiedChartConfig, }) => {
    ...
    const Component = () => {
        ...
        useSyncInteractiveFilters(useModified ? modifiedChartConfig : chartConfig, undefined);
        return (<div>
        <button data-testid="use-modified-button" onClick={() => setUseModified(true)}>
          use modified
        </button>
    ...
    const root = render(<Provider value={mockClient}>
    ...
    const clickUseModified = () => fireEvent.click(root.getByTestId("use-modified-button"));
...
describe("useSyncInteractiveFilters", () => {
    it("should keep interactive filters in sync with values from chart config", async () => {
        ...
        expect(initIfState.dataFilters["foo(VISUALIZE.ADMIN_COMPONENT_ID_SEPARATOR)http://environment.ld.admin.ch/foen/px/0703010000_103/dimension/1"]).toEqual({
            type: "single",
            value: "http://environment.ld.admin.ch/foen/px/0703010000_103/dimension/1/0",
        });
        clickUseModified();
        const IFState2 = getIFState();
        expect(IFState2.dataFilters["foo(VISUALIZE.ADMIN_COMPONENT_ID_SEPARATOR)http://environment.ld.admin.ch/foen/px/0703010000_103/dimension/1"]).toEqual({
            type: "single",
            value: "http://environment.ld.admin.ch/foen/px/0703010000_103/dimension/1/1",
        });
    ...
    it("applies default override changes for multi even after user selection", async () => {
        ...
        const dimId = "foo(VISUALIZE.ADMIN_COMPONENT_ID_SEPARATOR)http://environment.ld.admin.ch/foen/px/0703010000_103/dimension/1";
        const val0 = "http://environment.ld.admin.ch/foen/px/0703010000_103/dimension/1/0";
        const val1 = "http://environment.ld.admin.ch/foen/px/0703010000_103/dimension/1/1";
        ...
        const Component = () => {
            ...
            const [useModified, setUseModified] = useState(false);
            useSyncInteractiveFilters(useModified
                ? {
                    ...modifiedChartConfig,
                    interactiveFiltersConfig: {
                ...
                : modifiedChartConfig, undefined);
            return (<div>
          <button data-testid="user-select" onClick={() => update(dimId, val0)}>
            select
          </button>
          <button data-testid="apply-override" onClick={() => setUseModified(true)}>
            override
          </button>
        ...
        <ConfiguratorStateProvider chartId="published" initialState={configState}>
          <InteractiveFiltersProvider chartConfigs={configState.chartConfigs}>
            <InteractiveFiltersChartProvider chartConfigKey={modifiedChartConfig.key}>
              <Component />
            </InteractiveFiltersChartProvider>
          </InteractiveFiltersProvider>
        </ConfiguratorStateProvider>
    ...
    it("applies override when multi has no config filter (all values allowed)", async () => {
            ...
            cubes: [
                {
                    ...
                    filters: {
                        ...
                        ...Object.fromEntries(Object.entries(chartConfig.cubes[0].filters).filter(([key]) => key !== dimId)),
                    },
                },
            ],
            interactiveFiltersConfig: {
        ...
        const Component = () => {
            ...
            return (<div>
          <button data-testid="apply-override2" onClick={() => setOverride(val1)}>
            override2
          </button>
