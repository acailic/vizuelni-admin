...
import { ChartPerformanceMonitor } from "./performance-manager";
...
const DEFAULT_CHART_TYPES: Array<"scatterplot" | "line" | "area"> = [
  "scatterplot",
  "line",
  "area",
];
...
export const PerformanceTest = ({
  ...
  const [generatedData, setGeneratedData] = useState<
    Map<string, TestDataPoint[]>
  >(new Map());

  const canvasRef = useRef<HTMLCanvasElement>(null);
  const monitorRef = useRef<ChartPerformanceMonitor>(
    new ChartPerformanceMonitor()
  );

  // Generate test data
  const generateTestData = useCallback(
    (count: number, type: "scatterplot" | "line" | "area"): TestDataPoint[] => {
      ...
      if (type === "scatterplot") {
        ...
        for (let series = 0; series < seriesCount; series++) {
          for (let i = 0; i < pointsPerSeries; i++) {
            data.push({
              ...
              y:
                300 + Math.sin(i / 50) * 100 + Math.random() * 50 + series * 50,
        ...
        for (let series = 0; series < seriesCount; series++) {
          for (let i = 0; i < pointsPerSeries; i++) {
            const y1 =
              300 + Math.sin(i / 50) * 100 + Math.random() * 30 + series * 80;
            data.push({
              x: (i / pointsPerSeries) * 800,
              y: y1,
              color: colors[series % colors.length],
  ...
  const runSingleTest = useCallback(
    async (
      ...
      setCurrentTest(
        `${chartType} - ${renderingMethod} - ${dataSize.toLocaleString()} points`
      );
      ...
      for (let iteration = 0; iteration < testRuns; iteration++) {
        ...
        if (renderingMethod === "canvas") {
          ...
          if (canvasRef.current) {
            const renderer = useCanvasRenderer(canvasRef as any, {
              width: 800,
              height: 600,
  ...
  const stats = useMemo(() => {
    ...
    return {
      canvas: {
        avgFPS:
          canvasResults.length > 0
            ? canvasResults.reduce((s, r) => s + r.fps, 0) /
              canvasResults.length
            : 0,
        avgRenderTime:
          canvasResults.length > 0
            ? canvasResults.reduce((s, r) => s + r.renderTime, 0) /
              canvasResults.length
            : 0,
      ...
      svg: {
        avgFPS:
          svgResults.length > 0
            ? svgResults.reduce((s, r) => s + r.fps, 0) / svgResults.length
            : 0,
        avgRenderTime:
          svgResults.length > 0
            ? svgResults.reduce((s, r) => s + r.renderTime, 0) /
              svgResults.length
            : 0,
        ...
        <div style={{ marginBottom: "20px" }}>
          ...
          <div style={{ display: "flex", gap: "20px" }}>
            ...
            >
              ...
              <div>
                Average Render Time: {stats.canvas.avgRenderTime.toFixed(2)}ms
              </div>
              <div>
                Max Data Points: {stats.canvas.maxDataPoints.toLocaleString()}
              </div>
            </div>
            <div
              style={{
            ...
            >
              ...
              <div>
                Average Render Time: {stats.svg.avgRenderTime.toFixed(2)}ms
              </div>
              <div>
                Max Data Points: {stats.svg.maxDataPoints.toLocaleString()}
              </div>
            </div>
          </div>
        </div>
                ...
                <tr key={index}>
                  ...
                  <td
                    style={{
                      ...
                      border: "1px solid #ddd",
                      backgroundColor:
                        result.renderTime <= 16.67
                          ? "#d4edda"
                          : result.renderTime <= 33.33
                            ? "#fff3cd"
                            : "#f8d7da",
                    }}
                  >
                  ...
                  <td style={{ padding: "10px", border: "1px solid #ddd" }}>
                    {result.fps >= 30 && result.renderTime <= 16.67
                      ? "✅ Excellent"
                      : result.fps >= 15 && result.renderTime <= 33.33
                        ? "⚠️ Good"
                        : "❌ Poor"}
                  </td>
                </tr>
