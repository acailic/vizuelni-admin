...
const formatTree = (node, getLabel, depth = 0) => {
    return `${Array.from({ length: depth })
        .map(() => "     ")
        .join("")}${getLabel(node)}${node.children
        ? `
${node.children.map((c) => formatTree(c, getLabel, depth + 1)).join("\n")}`
        : ""}`;
};
const formatLayerTree = (layer) => formatTree(layer, (layer) => layer.title);
describe("parseWMSContent", () => {
    it("should parse WMS capabilities XML content", async () => {
        const xmlContent = (await readFile(join(__dirname, "./mocks/bio-discomap.wms.xml"))).toString();
...
`);
    ...
    it("should parse WMS capabilities XML content  (geo-admin)", async () => {
        const xmlContent = (await readFile(join(__dirname, "./mocks/geo-admin.wms.xml"))).toString();
        const endpoint = "http://example.com/wms";
        ...
        expect(ccsLayer === null || ccsLayer === void 0 ? void 0 : ccsLayer.children).toBeUndefined();
    });
    it("should parse WMS capabilities XML content  (geo-ag-ch)", async () => {
        const xmlContent = (await readFile(join(__dirname, "./mocks/wms-geo-ag-ch.wms.xml"))).toString();
...
`);
    ...
    it("should parse WMS capabilities XML content  (geodienste-ch-db-av-ita.wms.xml)", async () => {
        const xmlContent = (await readFile(join(__dirname, "./mocks/geodienste-ch-db-av-ita.wms.xml"))).toString();
        const endpoint = "https://wms.geo.ag.ch";
        const parsedLayers = parseWMSContent(xmlContent, endpoint);
...
describe("parseWMTSContent", () => {
    it("should parse WMTS capabilities XML content (zh)", async () => {
        const xmlContent = (await readFile(join(__dirname, "./mocks/zh.wmts.xml"))).toString();
        const endpoint = "http://example.com/wms";
...
`);
    ...
    it("should parse WMTS capabilities XML content  (geo-admin)", async () => {
        const xmlContent = (await readFile(join(__dirname, "./mocks/geo-admin.wmts.xml"))).toString();
        const endpoint = "http://example.com/wms";
        const parsedLayers = parseWMTSContent(xmlContent, endpoint);
...
`);
    ...
    it("should parse WMTS capabilities XML content  (Topographic_Map_Switzerland)", async () => {
        const xmlContent = (await readFile(join(__dirname, "./mocks/Topographic_Map_Switzerland.wmts.xml"))).toString();
        const endpoint = "http://example.com/wms";
        const parsedLayers = parseWMTSContent(xmlContent, endpoint);
...
`);
    ...
    it("should parse WMTS capabilities XML content  (geoserver-swissdatacube.wms.xml)", async () => {
        const xmlContent = (await readFile(join(__dirname, "./mocks/geoserver-swissdatacube.wmts.xml"))).toString();
