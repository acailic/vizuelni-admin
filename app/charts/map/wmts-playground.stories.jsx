...
const WMTSPlayground = () => {
    const [layers, setLayers] = useLocalState("storybook-wmts-playground", []);
    const onLayerCheck = useEvent((layer, checked) => {
        setLayers((layers) => checked ? [...layers, layer] : layers.filter((l) => l != layer));
    });
    ...
    return (<LocaleProvider value="en">
      <Box sx={{
            display: "grid",
        ...
        <Box sx={{
          ...
          <Accordion defaultExpanded sx={{ flexShrink: 1 }}>
            <AccordionSummary sx={{ typography: "body2" }} expandIcon={<Icon name="chevronDown"/>}>
              Added Layers
            </AccordionSummary>
            <AccordionDetails>
              <List>
                {layers.map((x) => {
            return (<ListItem key={x.id} secondaryAction={<IconButton edge="end" aria-label="delete" onClick={() => setLayers((layers) => layers.filter((y) => x !== y))}>
                          <Icon name="trash"/>
                        </IconButton>}>
    ...
    preserveDrawingBuffer style={{
            ...
            width: "100%",
            height: "100%",
        }} dragPan scrollZoom doubleClickZoom dragRotate={false} touchZoomRotate attributionControl={false}>
