...
const ProvidersTable = () => {
    ...
    return (<Box m={4} overflow="hidden">
      <FormControlLabel sx={{ mb: 2 }} control={<Checkbox checked={onlyWithExtraInfo} onChange={(e) => setOnlyWithExtraInfo(e.target.checked)} color="primary"/>} label="Only with extra info"/>
      <Table className={classes.table} size="small">
        ...
        <TableBody>
          {providersWithInfo.map((provider) => {
            var _a, _b;
            return (!onlyWithExtraInfo || provider.extraInfo) && (<TableRow key={provider.url} hover>
                  ...
                  {(() => {
                    ...
                    const canListLayersInfo = renderBooleanCell((_a = provider.extraInfo) === null || _a === void 0 ? void 0 : _a.canListLayers);
                    const canDisplayLayersInfo = renderBooleanCell((_b = provider.extraInfo) === null || _b === void 0 ? void 0 : _b.canDisplayLayers);
                    const canDisplayOnMapGeoAdminInfo = renderBooleanCell((_c = provider.extraInfo) === null || _c === void 0 ? void 0 : _c.canDisplayOnMapGeoAdmin);
                    return (<>
                        <TableCell className={classes.booleanCell} data-value={canListLayersInfo.dataValue}>
                          {canListLayersInfo.text}
                        </TableCell>
                        <TableCell className={classes.booleanCell} data-value={canDisplayLayersInfo.dataValue}>
                          {canDisplayLayersInfo.text}
                        </TableCell>
                        <TableCell className={classes.booleanCell} data-value={canDisplayOnMapGeoAdminInfo.dataValue}>
                          {canDisplayOnMapGeoAdminInfo.text}
                        </TableCell>
                      </>);
                })()}
                  <TableCell className={classes.workingLayerCell}>
                    {((_a = provider.extraInfo) === null || _a === void 0 ? void 0 : _a.workingLayers) &&
                    provider.extraInfo.workingLayers.length > 0
                    ? provider.extraInfo.workingLayers.join(", ")
                    : "-"}
                  </TableCell>
                  <TableCell className={classes.noteCell}>
                    {((_b = provider.extraInfo) === null || _b === void 0 ? void 0 : _b.note) ? provider.extraInfo.note : "-"}
                  </TableCell>
                </TableRow>);
        })}
        </TableBody>
