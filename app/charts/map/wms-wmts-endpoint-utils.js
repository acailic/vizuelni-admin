 ...
 * @returns {boolean}
 */
function isWmtsUrl(url) {
    return /wmts/i.test(url);
}
export const guessUrlType = (url) => {
    if (isWmtsUrl(url)) {
        return "wmts";
    }
    if (isWmsUrl(url)) {
 ...
 */
function guessExternalLayerUrl(provider, language) {
 ...
 */
function setWmtsGetCapParams(url, language) {
...
function setWmsGetCapParams(url, language) {
    // Mandatory params
...
const fetchWMSorWMSLayersFromEndpoint = async (endpoint, locale) => {
    ...
    try {
        ...
        console.error(`Error fetching WMTS layers from endpoint ${endpoint}`, endpoint);
        return [];
    }
};
export const getLayerKey = ({ type, id, }) => {
    return `${type}-${id}`;
};
const indexByKey = ({ wmsLayers, wmtsLayers, }) => {
    const layersByKey = {};
    visitHierarchy(wmsLayers !== null && wmsLayers !== void 0 ? wmsLayers : [], (layer) => {
        layersByKey[getLayerKey(layer)] = layer;
    });
    visitHierarchy(wmtsLayers !== null && wmtsLayers !== void 0 ? wmtsLayers : [], (layer) => {
        layersByKey[getLayerKey(layer)] = layer;
    });
    return layersByKey;
};
 ...
 */
export const isCRSSupported = (crs) => {
    return crs && crs.includes("EPSG:3857");
};
export const isRemoteLayerCRSSupported = (layer) => {
    return (layer.crs &&
        layer.crs.length > 0 &&
...
export const useWMTSorWMSLayers = (endpoints, { pause } = { pause: false }) => {
    const locale = useLocale();
    const queryFn = useCallback(async () => {
        const allLayers = (await Promise.all(endpoints.map((endpoint) => fetchWMSorWMSLayersFromEndpoint(endpoint, locale)))).flat();
