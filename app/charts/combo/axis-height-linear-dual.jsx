...
export const AxisHeightLinearDual = (props) => {
    ...
    const { y, yOrientationScales, colors, bounds } = useChartState();
    ...
    const { width: axisTitleWidth, height: axisTitleHeight } = useAxisTitleSize(axisTitle, { width: width * 0.5 - TICK_PADDING });
    const axisTitleX = leftAligned ? 0 : width - axisTitleWidth;
    return (<>
      <foreignObject data-testid={`axis-title-${orientation}`} x={axisTitleX} width={axisTitleWidth} height={axisTitleHeight} color={theme.palette.getContrastText(color)}>
        <OpenMetadataPanelWrapper component={y[orientation].dimension}>
          <span {...DISABLE_SCREENSHOT_COLOR_WIPE_ATTR} style={{
            width: "100%",
