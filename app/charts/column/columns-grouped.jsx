...
export const ErrorWhiskers = () => {
    const { bounds, xScale, xScaleIn, getY, getYErrorRange, getYErrorPresent, yScale, getSegment, grouped, showYUncertainty, } = useChartState();
    ...
    const renderData = useMemo(() => {
        ...
        return grouped
            ...
            const x0 = xScaleIn(getSegment(d));
            const barWidth = Math.min(bandwidth, 15);
            const y = getY(d);
            const [y1, y2] = getYErrorRange(d);
            return {
                key: `${segment}-${getSegment(d)}`,
                x: xScale(segment) + x0 + bandwidth / 2 - barWidth / 2,
                y: yScale(y),
                y1: yScale(y1),
                y2: yScale(y2),
                width: barWidth,
...
export const ColumnsGrouped = () => {
    const { bounds, xScale, xScaleIn, getY, yScale, getSegment, colors, grouped, getRenderingKey, } = useChartState();
    const ref = useRef(null);
    const enableTransition = useTransitionStore((state) => state.enable);
    const transitionDuration = useTransitionStore((state) => state.duration);
