...
const ColorPickerMenu = dynamic(() => import("../../../configurator/components/chart-controls/color-picker").then((mod) => mod.ColorPickerMenu), { ssr: false });
...
export const ColorPaletteCreator = (props) => {
    const { type } = props;
    switch (type) {
        case "diverging":
...
const SequentialColorPaletteCreator = (props) => {
    const { onUpdate, onAdd, colorValues } = props;
    assert(colorValues.length > 0, "Sequential color palette must have at least one color");
    ...
    useEffect(() => {
        if (!colorResult.startingColorHex ||
    ...
    return (<Flex flexDirection={"column"} gap={2}>
      <Box>
        <ColorRamp height={24} width={304} colorInterpolator={colorResult.interpolator}/>
      </Box>
      <Box sx={{ fontSize: "1rem", pb: 2, maxWidth: 304 }}>
        ...
        {baseColor && (<ColorSelectionRow type={"sequential"} key={baseColor.id} colorValues={[baseColor]} {...baseColor} onUpdate={onUpdate}/>)}
      </Box>
    </Flex>);
};
const DivergentColorPaletteCreator = (props) => {
    const { onAdd, onRemove, onUpdate, colorValues, title } = props;
    const classes = useStyles();
    const disabled = DIVERGING_MAX_ALLOWED_COLORS <= colorValues.length || title.length === 0;
    ...
    const colorResult = createDivergingInterpolator({
        ...
        options: {
            midColorHex: midColorHex === null || midColorHex === void 0 ? void 0 : midColorHex.color,
        },
    });
    return (<>
      <Flex flexDirection={"column"} gap={2}>
        ...
        <Box sx={{ fontSize: "1rem", pb: 2, maxWidth: 304 }}>
          ...
          {startColorHex && (<ColorSelectionRow type={"diverging"} key={startColorHex.id} colorValues={[startColorHex]} {...startColorHex} onUpdate={onUpdate}/>)}
        </Box>
        {midColorHex && (<Box sx={{ fontSize: "1rem", pb: 2, maxWidth: 304 }}>
            <Label htmlFor="custom-color-palette-mid">
              <Trans id="controls.custom-color-palettes.mid">
                Middle color
              </Trans>
            </Label>
            <ColorSelectionRow type={"diverging"} key={midColorHex.id} colorValues={[midColorHex]} {...midColorHex} onUpdate={onUpdate} onRemove={onRemove}/>
          </Box>)}
        <Box sx={{ fontSize: "1rem", pb: 2, maxWidth: 304 }}>
          <Label htmlFor="custom-color-palette-end">
            <Trans id="controls.custom-color-palettes.end">Ending color</Trans>
          </Label>
          {endColorHex && (<ColorSelectionRow type={"diverging"} key={endColorHex.id} colorValues={[endColorHex]} {...endColorHex} onUpdate={onUpdate}/>)}
        </Box>
      </Flex>
      <Button variant="text" fullWidth className={classes.addColorButton} disabled={disabled} startIcon={<Icon name="plus"/>} onClick={() => onAdd()}>
        <Trans id="controls.custom-color-palettes.add-color">Add color</Trans>
      </Button>
    </>);
};
const CategoricalColorPaletteCreator = (props) => {
    const { onAdd, onRemove, onUpdate, colorValues, title } = props;
    const classes = useStyles();
    const disabled = CATEGORICAL_MAX_ALLOWED_COLORS <= colorValues.length || title.length === 0;
    return (<>
      {colorValues.length > 0 && (<Box className={classes.categoricalColorListContainer}>
          {colorValues.map((item) => (<ColorSelectionRow type={"categorical"} colorValues={colorValues} key={item.id} {...item} onRemove={onRemove} onUpdate={onUpdate}/>))}
        </Box>)}
      <Button variant="text" fullWidth className={classes.addColorButton} disabled={disabled} startIcon={<Icon name="plus"/>} onClick={() => onAdd()}>
        <Trans id="controls.custom-color-palettes.add-color">Add color</Trans>
      </Button>
...
const ColorSelectionRow = (props) => {
    ...
    return (<Box className={classes.colorItem}>
      <Flex className={classes.colorItemContent}>
        <LegendItem label={color} color={color} symbol="square"/>
        <Flex sx={{ alignItems: "center", gap: 2 }}>
          {warningContrast && (<DisabledMessageIcon message={getContrastWarning()}/>)}
          <ColorPickerMenu colorId={id} colors={colorValues} selectedHexColor={color} onChange={(color) => onUpdate(color, id)} onRemove={onRemove}/>
        </Flex>
      </Flex>
    </Box>);
};
