...
export const ProfileContentTabs = (props) => {
    const { userId } = props;
    const { data: userConfigs } = useUserConfigs();
    const [value, setValue] = useState("home");
    const handleChange = useEvent((_, v) => {
        setValue(v);
    });
    const rootClasses = useRootStyles();
    const classes = useStyles();
    const { DRAFT: draftConfigs = [], PUBLISHED: publishedConfigs = [] } = useMemo(() => {
        return groupBy(userConfigs, (config) => config.published_state);
    }, [userConfigs]);
    if (!userConfigs) {
        return null;
    }
    return (<TabContext value={value}>
      <Box className={clsx(rootClasses.section, classes.section)}>
        <Box className={rootClasses.sectionContent}>
          <TabList className={classes.tabList} onChange={handleChange}>
            <Tab className={classes.tab} label={t({
            ...
            <Tab data-testid="color-palettes-tab" className={classes.tab} label={t({
      ...
      <TabPanel className={classes.tabPanel} value="home">
        <Box className={classes.tabPanelContent} sx={{ display: "flex", flexDirection: "column", gap: "3rem" }}>
          <ProfileVisualizationsTable title={t({
            id: "login.profile.my-drafts",
            message: "My Drafts",
        })} userId={userId} userConfigs={draftConfigs} preview onShowAll={() => setValue("drafts")}/>
          <ProfileVisualizationsTable title={t({
            id: "login.profile.my-published-visualizations",
            message: "My Visualizations",
        })} userId={userId} userConfigs={publishedConfigs} preview onShowAll={() => setValue("published")}/>
        </Box>
      </TabPanel>
      <TabPanel className={classes.tabPanel} value="published">
        <Box className={classes.tabPanelContent}>
          <ProfileVisualizationsTable title={t({
            id: "login.profile.my-published-visualizations",
            message: "My Visualizations",
        })} userId={userId} userConfigs={publishedConfigs}/>
        </Box>
      </TabPanel>
      <TabPanel className={classes.tabPanel} value="drafts">
        <Box className={classes.tabPanelContent}>
          <ProfileVisualizationsTable title={t({
            id: "login.profile.my-drafts",
            message: "My Drafts",
        })} userId={userId} userConfigs={draftConfigs}/>
        </Box>
      </TabPanel>
      <TabPanel className={classes.tabPanel} value="palettes">
        <Box className={classes.tabPanelContent}>
          <ProfileColorPaletteContent title={t({
            id: "login.profile.my-color-palettes",
            message: "My Color Palettes",
        })}/>
        </Box>
