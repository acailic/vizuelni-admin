...
export const LoginMenu = () => {
    ...
    return (<div>
      {user ? (<>
          <Button variant="text" onClick={(e) => setAnchorEl(e.currentTarget)} sx={{
                gap: 0.25,
                minWidth: 0,
                minHeight: 0,
                padding: 0,
          ...
          <Menu anchorEl={anchorEl} anchorOrigin={{ vertical: "bottom", horizontal: "center" }} transformOrigin={{ vertical: "top", horizontal: "center" }} open={!!anchorEl} onClose={() => setAnchorEl(null)} PaperProps={{ ref: paperEl, sx: { mt: 3 } }} sx={{ "& .MuiLink-root": { pr: "64px !important" } }}>
            <MenuActionItem type="link" as="menuitem" label={t({
                id: "login.profile.my-visualizations",
                message: "My visualizations",
            })} href="/profile"/>
            {ADFS_PROFILE_URL && (<MenuActionItem type="link" as="menuitem" label="eIAM MyAccount" href={ADFS_PROFILE_URL}/>)}
            <MenuActionItem type="button" as="menuitem" label={t({
                id: "login.profile.feedback",
                message: "Feedback",
            })} trailingIconName="arrowRight" onClick={() => setFeedbackMenuOpen(true)}/>
            <MenuActionItem type="button" as="menuitem" label={t({
                id: "login.sign-out",
                message: "Sign out",
...
const Feedback = ({ paperEl, open, handleClose, }) => {
    ...
    return (<Menu anchorPosition={position} open={open} onClose={handleClose} anchorReference="anchorPosition" PaperProps={{
            sx: {
