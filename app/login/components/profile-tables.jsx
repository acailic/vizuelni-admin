...
export const SectionContent = ({ children, title, }) => {
    const rootClasses = useRootStyles();
    return (<Box className={rootClasses.sectionContent}>
      {title && (<Typography variant="h3" sx={{ mb: 6, fontWeight: 700 }}>
          {title}
        </Typography>)}
      {children}
    </Box>);
...
export const ProfileVisualizationsTable = ({ title, userId, userConfigs, preview, onShowAll, }) => {
    return (<SectionContent title={title}>
          ...
          <StyledTable>
            ...
            <TableBody>
              {userConfigs
                ...
                .map((config) => (<ProfileVisualizationsRow key={config.key} userId={userId} config={config}/>))}
            </TableBody>
          </StyledTable>
          {preview && (<Button variant="text" color="primary" size="sm" endIcon={<Icon name="arrowDown" size={20}/>} onClick={onShowAll} sx={{ mt: 6 }}>
...
const ProfileVisualizationsRow = ({ userId, config, }) => {
    var _a, _b, _c, _d;
    const { dataSource } = config.data;
    const dataSets = Array.from(new Set(config.data.chartConfigs.flatMap((d) => d.cubes.map((d) => d.iri))));
    ...
    const [shareEl, setShareEl] = useState();
    const [embedEl, setEmbedEl] = useState();
    const { isOpen: isRenameOpen, open: openRename, close: closeRename, } = useDisclosure();
    ...
    const actions = useMemo(() => {
        const actions = [
            ...
            {
                type: "button",
                onClick: (e) => setShareEl(e === null || e === void 0 ? void 0 : e.currentTarget),
            ...
            {
                type: "button",
                onClick: (e) => setEmbedEl(e === null || e === void 0 ? void 0 : e.currentTarget),
            ...
            },
            {
                ...
                color: "red",
                leadingIconName: removeConfigMut.status === "fetching" ? "refresh" : "trash",
                ...
                confirmationText: isPublished
                    ? t({
                        ...
                        message: "This action cannot be undone. Removing this chart will affect all the places where it's embedded!",
                    })
                    : t({
    ...
    return (<TableRow>
      ...
      <TableCell>
        <NextLink href={isPublished ? publishLink : editLink} passHref legacyBehavior>
          <Link>
            <OverflowTooltip arrow title={<Markdown>{chartTitle}</Markdown>} color="primary.main">
      ...
      <TableCell>
        {fetching ? (<Skeleton width="50%" height={32}/>) : dataSet ? (<NextLink href={`/browse?dataset=${dataSet}&dataSource=${sourceToLabel(dataSource)}`} passHref legacyBehavior>
            <Link>
              <OverflowTooltip arrow title={(_b = (_a = data === null || data === void 0 ? void 0 : data.dataCubesMetadata[0]) === null || _a === void 0 ? void 0 : _a.title) !== null && _b !== void 0 ? _b : ""} color="primary.main">
                <Typography variant="body2" component="p" noWrap>
                  {(_d = (_c = data === null || data === void 0 ? void 0 : data.dataCubesMetadata[0]) === null || _c === void 0 ? void 0 : _c.title) !== null && _d !== void 0 ? _d : ""}
                </Typography>
              </OverflowTooltip>
            </Link>
          </NextLink>) : (<Typography variant="body2" component="p" noWrap>
            {t({
      ...
      <TableCell align="right">
        ...
        <RenameDialog config={config} open={isRenameOpen} onClose={closeRename} locale={locale} userId={userId}/>
      </TableCell>
    </TableRow>);
};
const getAdjustedPopoverPosition = (element) => {
    if (element) {
