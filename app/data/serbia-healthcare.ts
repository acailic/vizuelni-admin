...
export interface WaitingListData {
  ...
  category: 'surgery' | 'diagnostics' | 'specialist';
}
...
export interface HealthcareWorkerExodus {
  year: number;
  doctorsLeft: number;
  nursesLeft: number;
  totalLeft: number;
...
export const waitingLists: WaitingListData[] = [
  {
...
export const hospitalCapacity: HospitalCapacity[] = [
...
export const healthcareWorkerExodus: HealthcareWorkerExodus[] = [
  ...
  { year: 2020, doctorsLeft: 195, nursesLeft: 290, totalLeft: 485 }, // Slight decline due to pandemic
...
export const healthcareStats = {
  ...
  averageWaitTimeAllProcedures: Math.round(
    waitingLists.reduce((sum, item) => sum + item.averageWaitDays, 0) / waitingLists.length
  ),
  criticalWaitingLists: waitingLists.filter(
    item => item.averageWaitDays > item.recommendedMaxDays * 2
  ).length,
