...
export interface EnergyProduction {
  ...
  hydropower: number; // percentage
  naturalGas: number; // percentage
  solarWind: number; // percentage
  other: number; // percentage
}

export interface EnergyImports {
  year: number;
  importedGWh: number;
  importCostMillionEUR: number;
  electricityImportPercentage: number; // of total consumption
  gasImportPercentage: number;
}

export interface RenewableEnergyComparison {
  country: string;
...
export interface EmissionsData {
  ...
  co2PerCapitaTons: number;
  pmEmissionsThousandTons: number; // particulate matter from coal
}
...
export interface EnergyInfrastructure {
  coalPlants: {
    ...
    capacityMW: number;
    pollutionLevel: 'very high' | 'high' | 'medium';
  }[];
  renewableProjects: {
    type: string;
    typeEn: string;
    plannedCapacityMW: number;
    completedPercentage: number;
    status: 'delayed' | 'in progress' | 'completed';
  }[];
}

// Energy production mix - HEAVILY DEPENDENT ON COAL
export const energyProduction: EnergyProduction[] = [
...
export const energyImports: EnergyImports[] = [
...
export const renewableComparison: RenewableEnergyComparison[] = [
  { country: 'Srbija', renewablePercentage: 27.2, year: 2024 }, // hydropower 25.2% + solar/wind 2.0%
...
export const energyInfrastructure: EnergyInfrastructure = {
  coalPlants: [
...
export const energyStats = {
  ...
  averagePlantAge: Math.round(
    energyInfrastructure.coalPlants.reduce((sum, plant) => sum + plant.ageYears, 0) /
    energyInfrastructure.coalPlants.length
  ),
  averagePlantEfficiency: Math.round(
    energyInfrastructure.coalPlants.reduce((sum, plant) => sum + plant.efficiencyPercentage, 0) /
    energyInfrastructure.coalPlants.length
  ),
  ...
  completedRenewableCapacity: Math.round(
    energyInfrastructure.renewableProjects.reduce((sum, p) => sum + (p.plannedCapacityMW * p.completedPercentage / 100), 0)
  )
};
