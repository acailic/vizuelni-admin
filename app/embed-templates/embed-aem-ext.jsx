...
const EmbedHTML = ({ locale, chartId, embedQueryParams }) => (<html lang={locale}>
    ...
    <body style={{ padding: 0, margin: 0, background: "#fff" }}>
      <iframe title="chart" src={`${PUBLIC_URL}/${locale}/embed/${chartId}?${getEmbedParamsQueryString(embedQueryParams)}`} data-visualize-iframe style={{ width: 1, minWidth: "100%" }} frameBorder="0" allowTransparency scrolling="no"/>
      <div id=""/>
      <script defer src={`${PUBLIC_URL}/dist/embed.js`}></script>
    </body>
  </html>);
export const renderEmbedMarkup = (props) => {
