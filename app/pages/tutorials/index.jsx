...
export default function TutorialsIndex() {
    ...
    return (<DemoLayout title={pageTitle} description={pageDescription} hideBackButton>
      ...
      <TutorialSearch tutorials={allTutorials} onFilteredTutorialsChange={setFilteredTutorials} locale={locale}/>

      {/* Tutorials by Category */}
      {Object.entries(tutorialsByCategory).map(([category, tutorials]) => (<Box key={category} sx={{ mb: 6 }}>
          <Typography variant="h5" sx={{ mb: 3, fontWeight: 600, color: "text.primary" }}>
            {getCategoryLabel(category, locale)}
          </Typography>
          <Grid container spacing={3}>
            {tutorials.map((tutorial) => (<Grid item xs={12} sm={6} md={4} key={tutorial.id}>
        ...
        <Grid container spacing={3}>
          ...
          <Grid item xs={12} sm={4}>
            ...
            "&:hover": {
              ...
              <Typography variant="h3" sx={{ fontWeight: 800, mb: 1 }}>
                {Math.round(allTutorials.reduce((sum, t) => sum + t.estimatedTime, 0) /
            allTutorials.length)}
              </Typography>
              <Typography variant="body1" sx={{ opacity: 0.9, fontWeight: 500 }}>
