...
export default function DemoPage() {
    ...
    const config = category ? getDemoConfig(category) : null;
    ...
    const { dataset, resource, data, loading, error, refetch } = useDataGovRs({
        searchQuery: config === null || config === void 0 ? void 0 : config.searchQuery,
    ...
    if (!config) {
        return (<DemoLayout title="Demo nije pronađen" description="Tražena demo kategorija ne postoji.">
    ...
    return (<DemoLayout title={title} description={description} datasetInfo={dataset
      ...
      {/* Data Visualization */}
      {!loading && !error && dataset && data && (<Box>
          {/* Dataset Info Card */}
          <Paper sx={{ p: 3, mb: 4, backgroundColor: 'white' }}>
            ...
            {dataset.description && (<Typography variant="body1" color="text.secondary" sx={{ mb: 2 }}>
                {dataset.description}
              </Typography>)}

            <Box sx={{ display: 'flex', gap: 2, flexWrap: 'wrap' }}>
              {dataset.tags && dataset.tags.length > 0 && (<Typography variant="body2" color="text.secondary">
                  <strong>Tagovi:</strong> {dataset.tags.join(', ')}
                </Typography>)}
              {resource && (<Typography variant="body2" color="text.secondary">
                  <strong>Format:</strong> {resource.format}
                </Typography>)} 
              {Array.isArray(data) && (<Typography variant="body2" color="text.secondary">
          ...
          {/* Export Controls */}
          {Array.isArray(data) && data.length > 0 && (<Box sx={{ mb: 3 }}>
              <ExportControls targetElementId="main-chart-container" fileNamePrefix={`${category}-chart`}/>
            </Box>)}
          ...
          <Paper id="main-chart-container" sx={{ p: 4, mb: 4, backgroundColor: 'white' }}>
            ...
            {Array.isArray(data) && data.length > 0 ? (<SimpleChart data={data} chartType={config.chartType} width={Math.min(1000, typeof window !== 'undefined' ? window.innerWidth - 100 : 1000)} height={450}/>) : (<Typography variant="body2" color="text.secondary">
          ...
          {activeTab === 0 && Array.isArray(data) && data.length > 0 && (<>
              <Box sx={{ mb: 3 }}>
                <ExportControls targetElementId="detailed-chart-container" fileNamePrefix={`${category}-detailed-chart`}/>
              </Box>
              <Paper id="detailed-chart-container" sx={{ p: 4, mb: 4, backgroundColor: 'white' }}>
                <ChartVisualizer data={data} chartType={config.chartType} title={`${config.chartType.charAt(0).toUpperCase() + config.chartType.slice(1)} vizualizacija`}/>
              </Paper>
            </>)}
          ...
          {activeTab === 1 && (<Box>
              ...
              {Array.isArray(data) && data.length > 0 ? (<TableContainer component={Paper} sx={{ maxHeight: 600 }}>
                  <Table stickyHeader size="small">
                    <TableHead>
                      <TableRow>
                        {Object.keys(data[0]).map((key) => (<TableCell key={key} sx={{ fontWeight: 600, backgroundColor: 'grey.100' }}>
                            {key}
                          </TableCell>))}
                      </TableRow>
                    </TableHead>
                    <TableBody>
                      {data.slice(0, 50).map((row, i) => (<TableRow key={i} hover>
                          {Object.values(row).map((value, j) => (<TableCell key={j}>
                ...
                </TableContainer>) : (<Paper sx={{ p: 3, textAlign: 'center' }}>
                  <Typography color="text.secondary">
                    Podaci nisu dostupni u tabelarnom formatu
                  </Typography>
                </Paper>)}

              {Array.isArray(data) && data.length > 50 && (<Typography variant="body2" color="text.secondary" sx={{ mt: 2, textAlign: 'center' }}>
      ...
      {/* Empty State */}
      {!loading && !error && !dataset && (<DemoEmpty message="Podaci nisu pronađeni. Pokušajte kasnije."/>)}
    </DemoLayout>);
}
/**
