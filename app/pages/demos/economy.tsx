...
export default function EconomyDemo() {
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      markDataLoadEnd(economicIndicators.length, false);
      markRenderStart();
      // Render timing will be captured when the component finishes rendering
      setTimeout(() => markRenderEnd(), 0);
    }, 0);
  }, [markDataLoadStart, markDataLoadEnd, markRenderStart, markRenderEnd]);
  ...
  const dashboardContent = (
    <Box>
      <LiveDatasetPanel
        demoId="economy"
        title={
          locale === "sr" ? "Živi podaci (ekonomija)" : "Live data (economy)"
        }
      />
      ...
      >
        {locale === "sr" ? (
          <>
            <strong>📊 EKONOMSKI PREGLED:</strong> Srbija je prošla kroz
            značajnu ekonomsku transformaciju od 2000. godine. Prosečan rast
            BDP-a poslednjih 5 godina iznosi{" "}
            <strong>{avgGrowth.toFixed(1)}%</strong>. Nezaposlenost je smanjena
            sa 26% (2000) na{" "}
            <strong>{latestYear.unemployment.toFixed(1)}%</strong> (
            {latestYear.year}).
          </>
        ) : (
          <>
            <strong>📊 ECONOMIC OVERVIEW:</strong> Serbia has undergone
            significant economic transformation since 2000. Average GDP growth
            in the last 5 years is <strong>{avgGrowth.toFixed(1)}%</strong>.
            Unemployment decreased from 26% (2000) to{" "}
            <strong>{latestYear.unemployment.toFixed(1)}%</strong> (
            {latestYear.year}).
          </>
        )}
        ...
        <Grid item xs={12} md={6} lg={3}>
          <Card
            sx={{
              ...
              borderLeft: 4,
              borderColor:
                latestYear.gdpGrowth > 2 ? "success.main" : "warning.main",
            }}
          >
            <CardContent>
              <Typography variant="caption" color="text.secondary">
              ...
              >
                {latestYear.gdpGrowth > 0 ? "+" : ""}
                {latestYear.gdpGrowth.toFixed(1)}%
              </Typography>
              <Typography variant="body2" color="text.secondary">
        ...
        <Grid item xs={12} md={6} lg={3}>
          <Card
            sx={{
              ...
              borderLeft: 4,
              borderColor:
                latestYear.unemployment < 10 ? "success.main" : "warning.main",
            }}
          >
            <CardContent>
              <Typography variant="caption" color="text.secondary">
              ...
              <Typography variant="body2" color="text.secondary">
                {locale === "sr"
                  ? `${(
                      latestYear.unemployment - previousYear.unemployment
                    ).toFixed(1)}p.p. vs 2022`
                  : `${(
                      latestYear.unemployment - previousYear.unemployment
                    ).toFixed(1)}pp vs 2022`}
              </Typography>
            </CardContent>
          </Card>
        </Grid>
        ...
        <Grid item xs={12} md={6} lg={3}>
          <Card
            sx={{
              ...
              borderLeft: 4,
              borderColor:
                latestYear.inflation > 5 ? "error.main" : "success.main",
            }}
          >
            <CardContent>
              <Typography variant="caption" color="text.secondary">
      ...
      {/* Code Block for GDP Growth Chart */}
      <DemoCodeBlock
        title={
          locale === "sr"
            ? "📝 Kod za grafikon rasta BDP-a"
            : "📝 Code for GDP Growth Chart"
        }
        code={`import { LineChart } from '@/components/demos/charts';

// Sample economic data
const data = [
  { year: '2020', 'GDP Growth': 2.1 },
  { year: '2021', 'GDP Growth': 3.5 },
  { year: '2022', 'GDP Growth': 4.1 },
  { year: '2023', 'GDP Growth': 3.8 },
];
...
/>`}
      ...
      {/* Code Block for Foreign Trade Chart */}
      <DemoCodeBlock
        title={
          locale === "sr"
            ? "📝 Kod za grafikon spoljne trgovine"
            : "📝 Code for Foreign Trade Chart"
        }
        code={`import { ColumnChart } from '@/components/demos/charts';

const tradeData = [
  { year: '2020', Exports: 19.2, Imports: 25.8 },
  { year: '2021', Exports: 21.5, Imports: 28.3 },
  { year: '2022', Exports: 23.8, Imports: 31.2 },
  { year: '2023', Exports: 24.1, Imports: 32.9 },
];
...
// Or with the new standalone API
import { LineChart } from '@acailic/vizualni-admin/charts';
...
<LineChart
  ...
  config={{
    xAxis: 'year',
    yAxis: ['Exports', 'Imports'],
    seriesKeys: ['Exports', 'Imports'],
  }}
  height={500}
/>`}
        language="tsx"
      ...
      <Paper elevation={2} sx={{ p: 3 }}>
        ...
        <Grid container spacing={3}>
          <Grid item xs={12} md={6}>
            ...
            <Box sx={{ mb: 2 }}>
              <Chip
                label={locale === "sr" ? "Trgovinski deficit" : "Trade deficit"}
                color="error"
                sx={{ mr: 1, mb: 1 }}
              />
              <Typography variant="body2" sx={{ mt: 1 }}>
                {locale === "sr"
                  ? `Deficit od €${Math.abs(
                      tradeBalance[tradeBalance.length - 1].balance
                    ).toFixed(1)}M u 2023`
                  : `Deficit of €${Math.abs(
                      tradeBalance[tradeBalance.length - 1].balance
                    ).toFixed(1)}B in 2023`}
            ...
            <Box sx={{ mb: 2 }}>
              <Chip
                label={locale === "sr" ? "Javni dug" : "Public debt"}
                color="warning"
                sx={{ mr: 1, mb: 1 }}
              />
              <Typography variant="body2" sx={{ mt: 1 }}>
            ...
            <Box sx={{ mb: 2 }}>
              <Chip
                label={locale === "sr" ? "Siromaštvo" : "Poverty"}
                color="error"
                sx={{ mr: 1, mb: 1 }}
              />
              <Typography variant="body2" sx={{ mt: 1 }}>
          ...
          <Grid item xs={12} md={6}>
            ...
            <Box sx={{ mb: 2 }}>
              <Chip
                label={locale === "sr" ? "Rekordne SDI" : "Record FDI"}
                color="success"
                sx={{ mr: 1, mb: 1 }}
              />
              <Typography variant="body2" sx={{ mt: 1 }}>
  ...
  return (
    <DemoPageTemplate
      ...
      columns={[
        ...
        {
          key: "gdpGrowth",
          header: locale === "sr" ? "Rast BDP (%)" : "GDP Growth (%)",
          width: 150,
        },
        {
          key: "inflation",
          header: locale === "sr" ? "Inflacija (%)" : "Inflation (%)",
          width: 150,
        },
        {
          key: "unemployment",
          header: locale === "sr" ? "Nezaposlenost (%)" : "Unemployment (%)",
          width: 150,
        },
        {
          key: "event",
          header: locale === "sr" ? "Događaj" : "Event",
          width: 250,
        },
      ]}
    />
