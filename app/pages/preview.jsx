...
const chartStateStore = create((set) => ({
    state: null,
    setState: (state) => set({ state }),
}));
const updateState = async (rawState) => {
    try {
...
export default function Preview() {
    ...
    useEffect(() => {
        var _a;
        (_a = window === null || window === void 0 ? void 0 : window.parent) === null || _a === void 0 ? void 0 : _a.postMessage({ type: "ready" }, "*");
    }, [window]);
    useEffect(() => {
        if (!window) {
            return;
      ...
      <I18nProvider i18n={i18n}>
        <GraphqlProvider>
          <ThemeProvider theme={federalTheme.theme}>
            {state ? (<ConfiguratorStateProvider 
        // Reset the state on e.g. manual hash parameters URL edit.
        key={JSON.stringify(state)} chartId="published" initialState={state}>
                <ChartPublished configKey="preview" isPreview {...state}/>
              </ConfiguratorStateProvider>) : null}
          </ThemeProvider>
        </GraphqlProvider>
      </I18nProvider>
    </LocaleProvider>);
