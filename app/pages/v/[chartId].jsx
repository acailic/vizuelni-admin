...
const VisualizationPage = (props) => {
    ...
    const canEdit = status === "found" &&
        config.user_id &&
        config.user_id === ((_a = session.data) === null || _a === void 0 ? void 0 : _a.user.id);
    ...
    }, [config === null || config === void 0 ? void 0 : config.data, config === null || config === void 0 ? void 0 : config.key, status]);
    const chartConfig = state ? getChartConfig(state) : undefined;
    useEffect(function removePublishSuccessFromURL() {
    ...
    useEffect(function setCorrectDataSource() {
        if (status === "found" && config.data.dataSource.url !== dataSource.url) {
            setDataSource(config.data.dataSource);
        }
    }, [config === null || config === void 0 ? void 0 : config.data.dataSource, dataSource.url, setDataSource, status]);
    if (status === "notfound" ||
        state === undefined ||
    ...
    return (<>
      <Head>
        ...
        <meta property="og:description" content={state.layout.meta.description[locale]}/>
        <meta name="twitter:card" content="summary_large_image"/>
      </Head>
      <ContentLayout>
        {config.published_state === PUBLISHED_STATE.PUBLISHED && (<Box className={classes.actionBar}>
            <PublishActions chartWrapperRef={chartWrapperRef} configKey={key} locale={locale}/>
          </Box>)}
        <Box px={[2, 4]} mb="auto" mx="auto" width="100%" overflow="hidden">
          <Box sx={{
            ...
            {publishSuccess && (<Box mt={2} mb={5}>
                <PublishSuccess />
              </Box>)}
            {config.published_state === PUBLISHED_STATE.DRAFT && (<Box mt={2} mb={5}>
                <HintWarning sx={{
                flexDirection: "row",
                alignItems: "center",
                m: "auto",
                [`& .${alertClasses.message}`]: {
                  ...
                  {canEdit ? (<Button component={NextLink} href={`/create/new?edit=${config.key}`} variant="outlined" color="inherit" size="sm">
            ...
            <Typography variant="body2" component="div" my={6} color="monochrome.700">
              {publishSuccess ? (<Trans id="hint.how.to.share">
    ...
    spacing={{ xs: 2, sm: 2 }} sx={{ mb: 5 }}>
              ...
              <NextLink href={{ pathname: "/create/new", query: { copy: key } }} passHref legacyBehavior>
                <Button>
