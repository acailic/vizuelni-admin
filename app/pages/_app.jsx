...
const GQLDebugPanel = dynamic(() => import("@/gql-flamegraph/devtool").then((mod) => mod.DebugPanel), { ssr: false });
export default function App({ Component, pageProps: { session, ...pageProps }, }) {
    var _a;
    const { events: routerEvents, asPath, locale: routerLocale } = useRouter();
    const locale = parseLocaleString(routerLocale !== null && routerLocale !== void 0 ? routerLocale : "");
    useNProgress();
    ...
    const shouldShowGQLDebug = process.env.NODE_ENV === "development" || flag("debug");
    return (<>
      <Head>
        ...
        {(_a = federalTheme.preloadFonts) === null || _a === void 0 ? void 0 : _a.map((src) => (<link key={src} rel="preload" href={src} as="font" crossOrigin="anonymous"/>))}
      </Head>
...
const descriptionByLocale = {
    "sr-Latn": "Kreirajte i ugradite vizualizacije iz skupova podataka sa portala data.gov.rs, zvaničnog portala otvorenih podataka Republike Srbije.",
    "sr-Cyrl": "Креирајте и уградите визуализације из скупова података са портала data.gov.rs, званичног портала отворених података Републике Србије.",
