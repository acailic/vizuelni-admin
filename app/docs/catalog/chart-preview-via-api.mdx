...
In order to get parameters, you can use the _Copy preview link_ button found in
the editor or use the
...
message with chart state to the iframe window when ready.
...
This method works by sending a POST request to the `/preview_post` page with
chart state when clicking on a form button. The `/preview_post` page retrieves
the content of a request in `getServerSideProps` and renders a preview of a
chart.
