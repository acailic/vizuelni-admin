 ...
 */
export const useRenderTime = (componentName, enabled = process.env.NODE_ENV === "development") => {
    ...
    useEffect(() => {
        ...
        console.log(`[Performance] ${componentName} rendered in ${duration.toFixed(2)}ms (render #${renderCount.current})`);
 ...
 */
export const useComponentLifecycle = (componentName, enabled = process.env.NODE_ENV === "development") => {
    useEffect(() => {
        if (!enabled)
            return;
...
export const getMemoryUsage = () => {
    ...
    if (performance.memory) {
        return {
            ...
            usedPercentage: ((performance.memory.usedJSHeapSize / performance.memory.jsHeapSizeLimit) *
                100).toFixed(2),
        };
    }
    return null;
