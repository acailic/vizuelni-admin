...
import { getUnversionedCubeIri, getUnversionedCubeIriServerSide, } from "@/utils/chart-config/upgrade-cube";
...
export const chartConfigMigrations = [
    {
        description: `MAP
    baseLayer {
      + locked
            ...
            if (newConfig.chartType === "map") {
                ...
                if (areaLayer) {
                    ...
                    newConfig.fields.areaLayer = {
                        show: false,
                        componentIri: "",
                        measureIri: (symbolLayer === null || symbolLayer === void 0 ? void 0 : symbolLayer.measureIri) || "",
                ...
                if (symbolLayer) {
                    ...
                    newConfig.fields.symbolLayer = {
                        ...
                        componentIri: (areaLayer === null || areaLayer === void 0 ? void 0 : areaLayer.componentIri) || "",
                        measureIri: (areaLayer === null || areaLayer === void 0 ? void 0 : areaLayer.measureIri) || "",
                        colors: {
                            type: "fixed",
                            value: "#1f77b4",
                            opacity: 80,
      ...
      + color {
        ...
        up: (config) => {
            ...
            if (newConfig.chartType === "map") {
                ...
                const { areaLayer, symbolLayer } = fields;
                if (areaLayer) {
                    const { componentIri, measureIri, palette, colorScaleType, colorScaleInterpolationType, nbClass, } = areaLayer;
                    newConfig.fields.areaLayer = {
                        componentIri,
                        color: {
                            type: "numerical",
                            ...
                            ...(colorScaleType === "discrete"
                                ? {
                                    ...
                                    interpolationType: colorScaleInterpolationType === "linear"
                                        ? "quantize"
                                        : colorScaleInterpolationType,
                                    nbClass,
                                }
                ...
                if (symbolLayer) {
                    ...
                    newConfig.fields.symbolLayer = {
                        componentIri,
                        measureIri,
                        color: colors.type === "fixed" || colors.type === "categorical"
                            ? colors
                            : {
                                type: "numerical",
            ...
            if (newConfig.chartType === "map") {
                ...
                if (areaLayer) {
                    ...
                    newConfig.fields.areaLayer = {
                        ...
                        nbClass: (_a = color.nbClass) !== null && _a !== void 0 ? _a : 3,
                    };
                }
                if (symbolLayer) {
    ...
    fields {
      + animation {
        ...
        up: (config) => {
            ...
            const { interactiveFiltersConfig } = newConfig;
            if (interactiveFiltersConfig) {
                const { legend, timeRange, timeSlider, dataFilters } = interactiveFiltersConfig;
                newConfig.interactiveFiltersConfig = { legend, timeRange, dataFilters };
                if (["column", "pie", "scatterplot"].includes(newConfig.chartType) &&
                    (timeSlider === null || timeSlider === void 0 ? void 0 : timeSlider.componentIri)) {
    ...
    },
    {
        ...
        down: (config) => {
            const newConfig = { ...config, version: "2.0.0" };
            if (["comboLineSingle", "comboLineDual", "comboLineColumn"].includes(newConfig.chartType)) {
                newConfig.chartType = "line";
                newConfig.fields = {
                    x: newConfig.fields.x,
                    y: {
                        componentIri: newConfig.chartType === "comboLineSingle"
        ...
        up: (config) => {
            ...
            if (newConfig.chartType === "comboLineSingle") {
                ...
                newConfig.fields.y.colorMapping = mapValueIrisToColor({
                    ...
                    dimensionValues: newConfig.fields.y.componentIris.map((iri) => ({ value: iri, label: iri })),
                });
            }
    ...
    },
    {
      ...
      + cubes {
        ...
        up: (config) => {
            ...
            newConfig.cubes = newConfig.cubes.map((cube) => ({
                ...cube,
                publishIri: cube.iri,
            }));
            return newConfig;
    ...
    },
    {
      ...
      cubes {
        ...
        up: async (config, configuratorState) => {
            var _a;
            const unversionedCubeIris = [];
            const getClosestUnversionedCubeIri = (id) => {
                let score = 0;
                let unversionedIri = "";
            ...
            newConfig.cubes = await Promise.all(newConfig.cubes.map(async (cube) => {
                var _a;
                const { publishIri, ...rest } = cube;
                ...
                return {
                    ...rest,
                    joinBy: (_a = rest.joinBy) === null || _a === void 0 ? void 0 : _a.map((joinBy) => stringifyComponentId({
                        unversionedCubeIri: unversionedIri,
                        unversionedComponentIri: joinBy,
                    })),
                    filters: Object.fromEntries(Object.entries(cube.filters).map(([k, v]) => [
            ...
            for (const [k, v] of Object.entries(newConfig.fields)) {
                if (typeof v !== "object") {
                    continue;
                }
                if ("componentIris" in v) {
                    v.componentIds = v.componentIris.map((iri) => isJoinById(iri)
                        ? iri
                        : stringifyComponentId({
                    ...
                    v.componentId = isJoinById(v.componentIri)
                        ...
                        : v.componentIri
                            ? stringifyComponentId({
                                unversionedCubeIri: getClosestUnversionedCubeIri(v.componentIri),
                ...
                if ("measureIri" in v && v.measureIri !== FIELD_VALUE_NONE) {
                    v.measureId = isJoinById(v.measureIri)
                        ...
                        : v.measureIri
                            ? stringifyComponentId({
                                unversionedCubeIri: getClosestUnversionedCubeIri(v.measureIri),
                                unversionedComponentIri: v.measureIri,
                            })
                            : "";
                    delete v.measureIri;
                }
                if ((newConfig.chartType === "comboLineColumn" ||
                    newConfig.chartType === "comboLineDual" ||
                    newConfig.chartType === "comboLineSingle") &&
                    k === "y") {
                    v.colorMapping = Object.fromEntries(Object.entries(v.colorMapping).map(([k, v]) => [
                        stringifyComponentId({
                ...
                if (v.color && "componentIri" in v.color) {
                    v.color.componentId = isJoinById(v.color.componentIri)
                        ...
                        : stringifyComponentId({
                            unversionedCubeIri: getClosestUnversionedCubeIri(v.color.componentIri),
                ...
                if ("leftAxisComponentIri" in v) {
                    v.leftAxisComponentId = isJoinById(v.leftAxisComponentIri)
                        ...
                        : stringifyComponentId({
                            unversionedCubeIri: getClosestUnversionedCubeIri(v.leftAxisComponentIri),
                ...
                if ("rightAxisComponentIri" in v) {
                    v.rightAxisComponentId = isJoinById(v.rightAxisComponentIri)
                        ...
                        : stringifyComponentId({
                            unversionedCubeIri: getClosestUnversionedCubeIri(v.rightAxisComponentIri),
                ...
                if ("columnComponentIri" in v) {
                    v.columnComponentId = isJoinById(v.columnComponentIri)
                        ...
                        : stringifyComponentId({
                            unversionedCubeIri: getClosestUnversionedCubeIri(v.columnComponentIri),
                ...
                if ("lineComponentIri" in v) {
                    v.lineComponentId = isJoinById(v.lineComponentIri)
                        ? v.lineComponentIri
                        : stringifyComponentId({
                            unversionedCubeIri: getClosestUnversionedCubeIri(v.lineComponentIri),
            ...
            if (newConfig.chartType === "table") {
                newConfig.sorting = newConfig.sorting.map((sorting) => {
                    ...
                    return {
                        ...
                        componentId: isJoinById(componentIri)
                            ? componentIri
                            : stringifyComponentId({
                                unversionedCubeIri: getClosestUnversionedCubeIri(componentIri),
                                unversionedComponentIri: componentIri,
                            }),
                    };
                });
            }
            for (const v of Object.values((_a = newConfig.interactiveFiltersConfig) !== null && _a !== void 0 ? _a : {})) {
                if ("componentIris" in v) {
                    v.componentIds = v.componentIris.map((iri) => isJoinById(iri)
                        ? iri
                        : stringifyComponentId({
        ...
        down: async (config) => {
            var _a;
            const newConfig = { ...config, version: "3.4.0" };
            newConfig.cubes = newConfig.cubes.map(async (cube) => {
                var _a;
                return {
                    ...cube,
                    joinBy: (_a = cube.joinBy) === null || _a === void 0 ? void 0 : _a.map((joinBy) => parseComponentId(joinBy).unversionedComponentIri),
                    publishIri: cube.iri,
                    filters: Object.fromEntries(Object.entries(cube.filters).map(([k, v]) => [
            ...
            for (const [_k, v] of Object.entries(newConfig.fields)) {
                if (typeof v !== "object") {
                    continue;
                }
                const k = _k;
                if ("componentIds" in v) {
                    v.componentIris = v.componentIds.map((id) => parseComponentId(id).unversionedComponentIri);
                ...
                if (v.color && "componentId" in v.color) {
                    v.color.componentIri = parseComponentId(v.color.componentId).unversionedComponentIri;
                    delete v.color.componentId;
                }
                if ("leftAxisComponentId" in v) {
                    v.leftAxisComponentIri = parseComponentId(v.leftAxisComponentId).unversionedComponentIri;
                    delete v.leftAxisComponentId;
                }
                if ("rightAxisComponentId" in v) {
                    v.rightAxisComponentIri = parseComponentId(v.rightAxisComponentId).unversionedComponentIri;
                    delete v.rightAxisComponentId;
                }
                if ("columnComponentId" in v) {
                    v.columnComponentIri = parseComponentId(v.columnComponentId).unversionedComponentIri;
                    delete v.columnComponentId;
                }
                if ("lineComponentId" in v) {
                    v.lineComponentIri = parseComponentId(v.lineComponentId).unversionedComponentIri;
    ...
    },
    {
      ...
      fields {
        ...
        up: (config) => {
            ...
            if (newConfig.chartType === "map") {
                if (((_a = newConfig.fields.areaLayer) === null || _a === void 0 ? void 0 : _a.color) &&
                ...
                if (((_b = newConfig.fields.symbolLayer) === null || _b === void 0 ? void 0 : _b.color) &&
            ...
            if (!((_c = newConfig.fields) === null || _c === void 0 ? void 0 : _c.color)) {
                const hasColorMapping = !!(((_e = (_d = newConfig.fields) === null || _d === void 0 ? void 0 : _d.y) === null || _e === void 0 ? void 0 : _e.colorMapping) ||
                    ((_g = (_f = newConfig.fields) === null || _f === void 0 ? void 0 : _f.segment) === null || _g === void 0 ? void 0 : _g.colorMapping));
                if (!hasColorMapping) {
                    newConfig.fields = {
                        ...newConfig.fields,
                        color: {
            ...
            if ((_j = (_h = newConfig.fields) === null || _h === void 0 ? void 0 : _h.segment) === null || _j === void 0 ? void 0 : _j.colorMapping) {
                newConfig.fields = {
                    ...newConfig.fields,
                    color: {
                        type: "segment",
            ...
            if ((_m = (_l = newConfig.fields) === null || _l === void 0 ? void 0 : _l.y) === null || _m === void 0 ? void 0 : _m.colorMapping) {
                newConfig.fields = {
                    ...newConfig.fields,
                    color: {
                        type: "measures",
        ...
        up: (config) => {
            ...
            if (!newConfig.interactiveFiltersConfig) {
                ...
                newConfig.interactiveFiltersConfig.dataFilters.filterTypes =
                    Object.fromEntries(newConfig.interactiveFiltersConfig.dataFilters.componentIds.map((id) => [id, "single"]));
            }
            return newConfig;
        },
        down: (config) => {
...
const makeBumpChartConfigVersionMigration = ({ fromVersion, toVersion, fromChartConfigVersion, toChartConfigVersion, }) => {
    return {
...
export const configuratorStateMigrations = [
    {
        description: "ALL",
        from: "1.0.0",
        to: "2.0.0",
        up: async (config) => {
            const newConfig = { ...config, version: "2.0.0" };
            const migratedChartConfig = await migrateChartConfig(newConfig.chartConfig, {
                migrationProps: newConfig,
                toVersion: "2.0.0",
            });
        ...
        down: async (config) => {
            ...
            for (const chartConfig of newConfig.chartConfigs) {
                ...
                if (chartConfig.dataSet === dataSet) {
                    ...
                    console.warn("Cannot migrate chartConfig dataSet to configuratorState dataSet because they are not the same.");
                }
            }
            newConfig.dataSet = dataSet;
            return newConfig;
    ...
    }),
    {
        ...
        down: (config) => {
            ...
            newConfig.meta = (_a = newConfig.layout.meta) !== null && _a !== void 0 ? _a : {
                title: {
                    "sr-Latn": "",
                    "sr-Cyrl": "",
                    en: "",
                ...
                dashboardFilters: {
                    timeRange: oldTimeRangeFilter
                        ? {
                            ...
                            timeUnit: "",
                            presets: {
                                from: (_a = oldTimeRangeFilter.from) !== null && _a !== void 0 ? _a : "",
                                to: (_b = oldTimeRangeFilter.to) !== null && _b !== void 0 ? _b : "",
                            },
                        }
                        : {
                            active: false,
    ...
    },
    {
        ...
        up: (config) => {
            ...
            if (newConfig.layout.type === "dashboard" &&
                newConfig.layout.layout === "canvas") {
                newConfig.layout.layoutsMetadata = newConfig.chartConfigs.reduce((acc, chartConfig) => {
                    acc[chartConfig.key] = {
                        initialized: true,
                    };
                    return acc;
    ...
    },
    {
        ...
        up: async (config) => {
            ...
            for (const cIri of dataFilters.componentIris) {
                cubeLoop: for (const cube of newConfig.chartConfigs.flatMap((c) => c.cubes)) {
                    for (const fId of Object.keys(cube.filters)) {
                        if (fId.includes(cIri)) {
                            dataFilters.componentIris = [
                                ...dataFilters.componentIris.filter((c) => c !== cIri),
                                fId,
                            ];
        ...
        down: async (config) => {
            ...
            for (const cId of dataFilters.componentIds) {
                dataFilters.componentIds = [
                    ...dataFilters.componentIds.filter((c) => c !== cId),
                    parseComponentId(cId).unversionedComponentIri,
                ];
            }
            for (const fId of Object.keys(dataFilters.filters)) {
                dataFilters.filters[parseComponentId(fId).unversionedComponentIri] = dataFilters.filters[fId];
    ...
    }),
    {
      ...
      layout {
        ...
        up: async (config) => {
            ...
            newConfig.layout.blocks = newConfig.chartConfigs.map((chartConfig) => {
                return {
                    type: "chart",
                    key: chartConfig.key,
                    initialized: false,
        ...
        down: async (config) => {
            ...
            if (newConfig.layout.type === "dashboard" &&
                newConfig.layout.layout === "canvas") {
                newConfig.layout.layoutsMetadata = Object.fromEntries(newConfig.layout.blocks.map((block) => {
...
export const migrateConfiguratorState = makeMigrate(configuratorStateMigrations, {
    defaultToVersion: CONFIGURATOR_STATE_VERSION,
});
function makeMigrate(migrations, options) {
    const { defaultToVersion } = options;
    return async (data, options = {}) => {
        const { fromVersion, toVersion = defaultToVersion, migrationProps, } = options;
        const migrate = async (data, { fromVersion, } = {}) => {
            var _a;
            const fromVersionFinal = (_a = fromVersion !== null && fromVersion !== void 0 ? fromVersion : data.version) !== null && _a !== void 0 ? _a : "1.0.0";
            const direction = upOrDown(fromVersionFinal, toVersion);
            if (direction === "same") {
                return data;
            }
            const migration = migrations.find((d) => d[direction === "up" ? "from" : "to"] === fromVersionFinal);
            if (migration) {
...
export const upOrDown = (fromVersion, toVersion) => {
