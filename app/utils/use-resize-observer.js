...
export const useResizeObserver = (cb) => {
    ...
    const handleRef = useEventCallback((node) => {
        ...
        elRef.current = node;
        if (!roRef.current) {
            const resizeHandler = throttle((entries) => {
                ...
                const { inlineSize: newWidth, blockSize: newHeight } = entry.contentBoxSize[0];
                ...
                const newSize = (Math.abs(newHeight - height) > 16 && newHeight > 0) ||
    ...
    return [handleRef, size.width, size.height];
};
