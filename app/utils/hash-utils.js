...
export const objectToHashString = (o) => {
    return `#${objectToKeyValuePairs(o).join(JOIN_SEPARATOR)}`;
};
/** Converts a URL-friendly hash string back into the original object. */
export const hashStringToObject = (hashString) => {
    return keyValuePairsToObject(decodeURIComponent(hashString.slice(1)).split(JOIN_SEPARATOR));
};
const objectToKeyValuePairs = (o) => {
    const flatObject = flattenObject(o);
    return Object.entries(flatObject).map(([k, v]) => {
        return `${k}${VALUE_SEPARATOR}${v}`;
    });
};
const keyValuePairsToObject = (keyValuePairs) => {
    const o = {};
...
const flattenObject = (o, parentKey = "", result = {}) => {
    if (typeof o === "object" &&
        o !== null &&
        !Array.isArray(o) &&
...
const unflattenObject = (o) => {
    ...
    const transformArraysAndScalars = (obj) => {
