...
describe("parse-search-results", () => {
    it("should correctly parse cube", async () => {
        const data = await parseCSV(path.join(__dirname, "./query-search-results-photovoltaikanlagen.mock.csv"));
    ...
    it("should build search cubes from CSV (shared dimension query, all)", async () => {
        const data = await parseCSV(path.join(__dirname, "./query-search-results-shared-dimensions.mock.csv"));
    ...
    it("should build search cubes from CSV (shared dimension query, temporal)", async () => {
        const data = await parseCSV(path.join(__dirname, "./query-search-results-temporal.mock.csv"));
        const searchCubes = buildSearchCubes(data);
        expect(searchCubes.slice(0, 3)).toMatchInlineSnapshot(`
      [
        {
...
it("should merge search cubes together", async () => {
    const cubesShared = buildSearchCubes(await parseCSV(path.join(__dirname, "./query-search-results-shared-dimensions.mock.csv")));
    const cubesTemporal = buildSearchCubes(await parseCSV(path.join(__dirname, "./query-search-results-temporal.mock.csv")));
    const searchCubes = Object.values([...cubesShared, ...cubesTemporal].reduce((acc, d) => {
        acc[d.iri] = mergeSearchCubes(acc[d.iri], d);
        return acc;
    }, {}));
    const cubeWithTemporalAndShared = searchCubes.some((cube) => {
        var _a, _b;
        const temporal = (_a = cube.dimensions) === null || _a === void 0 ? void 0 : _a.find((dimension) => dimension.timeUnit);
        const shared = (_b = cube.dimensions) === null || _b === void 0 ? void 0 : _b.find((dimension) => dimension.termsets.length);
