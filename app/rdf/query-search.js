...
import { buildLocalizedSubQuery, GROUP_SEPARATOR, iriToNode, makeVisualizeDatasetFilter, } from "@/rdf/query-utils";
const makeInFilter = (name, values) => {
    return `
    ${values.length > 0
 ...
 */
const fanOutExclusiveFilters = (filters) => {
    if (!filters) {
        return [filters];
    }
...
export const mergeSearchCubes = (a, b) => {
    var _a, _b, _c, _d;
    const mergedDimensions = uniqBy([...((_a = a === null || a === void 0 ? void 0 : a.dimensions) !== null && _a !== void 0 ? _a : []), ...((_b = b.dimensions) !== null && _b !== void 0 ? _b : [])], "id").map((d) => ({
        ...d,
        termsets: uniqBy(d.termsets, "iri"),
    }));
    const mergedTermsets = uniqBy([...((_c = a === null || a === void 0 ? void 0 : a.termsets) !== null && _c !== void 0 ? _c : []), ...((_d = b.termsets) !== null && _d !== void 0 ? _d : [])], "iri");
    return {
        ...a,
        ...b,
...
export const searchCubes = async ({ query, locale: _locale, filters, includeDrafts, fetchDimensionTermsets, sparqlClient, }) => {
    var _a, _b;
    const locale = _locale !== null && _locale !== void 0 ? _locale : defaultLocale;
    // Search cubeIris along with their score
    const themeValues = (_a = filters === null || filters === void 0 ? void 0 : filters.filter((x) => x.type === SearchCubeFilterType.DataCubeTheme).map((v) => v.value)) !== null && _a !== void 0 ? _a : [];
    const creatorValues = (_b = filters === null || filters === void 0 ? void 0 : filters.filter((x) => x.type === SearchCubeFilterType.DataCubeOrganization).map((v) => v.value)) !== null && _b !== void 0 ? _b : [];
    const exclusiveFilters = fanOutExclusiveFilters(filters);
    const scoresQueries = exclusiveFilters.map((filters) => mkScoresQuery(locale, filters, creatorValues, themeValues, includeDrafts, query, fetchDimensionTermsets));
    const scoreResults = await Promise.all(scoresQueries.map((scoresQuery) => sparqlClient.query.construct(scoresQuery, {
        operation: "postUrlencoded",
    })));
    const parsedCubes = Object.values(scoreResults
    ...
    return cubes
        .sort((a, b) => descending(infoByCube[a.iri].score, infoByCube[b.iri].score))
        .map((cube) => ({
        cube,
        highlightedTitle: query ? highlight(cube.title, query) : cube.title,
        highlightedDescription: query && cube.description
            ? highlight(cube.description, query)
            : cube.description,
    }));
};
const mkScoresQuery = (locale, filters, creatorValues, themeValues, includeDrafts, query, fetchDimensionTermsets) => {
    return `
  ${pragmas}
  ...
  CONSTRUCT {
    ...
    WHERE {
      ...
      ${filters === null || filters === void 0 ? void 0 : filters.map((filter) => {
        ...
        ${!(filters === null || filters === void 0 ? void 0 : filters.find((f) => f.type === SearchCubeFilterType.DataCubeTermset))
        ? `
          OPTIONAL {
            ?termsetIri meta:isUsedIn ?iri .
            ${buildLocalizedSubQuery("termsetIri", "schema:name", "termsetLabel", { locale })}
      ...
      ${creatorValues.includes("https://register.ld.admin.ch/opendataswiss/org/bundesamt-fur-umwelt-bafu")
        ? `
      OPTIONAL {
        ...
        ${buildLocalizedSubQuery("subthemeIri", "schema:name", "subthemeLabel", { locale })}
      } 
      `
        : ""}
