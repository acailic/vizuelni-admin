...
export const PUBLIC_URL = (
  clientEnv?.PUBLIC_URL ?? process.env.PUBLIC_URL ?? normalizedBasePath ?? ""
).replace(/\/$/, "");
...
export const WHITELISTED_DATA_SOURCES =
  clientEnv?.WHITELISTED_DATA_SOURCES ??
...
export const BUILD_GITHUB_REPO = (process.env.NEXT_PUBLIC_GITHUB_REPO || "").replace(
  /^git\+https/,
  "https",
); // Don't use git+https for the link, need to check with Abraxas
export const BASE_VECTOR_TILE_URL = process.env.NEXT_PUBLIC_BASE_VECTOR_TILE_URL ?? "";
export const MAPTILER_STYLE_KEY = process.env.NEXT_PUBLIC_MAPTILER_STYLE_KEY ?? "";
