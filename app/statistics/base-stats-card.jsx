...
export const BaseStatsCard = ({ title, subtitle, data, columnName = "Date", trend, showPercentage = false, }) => {
    ...
    const totalCount = (_a = sum(data, ([, { count }]) => count)) !== null && _a !== void 0 ? _a : 1;
    const maxCount = (_b = max(data, ([, { count }]) => count)) !== null && _b !== void 0 ? _b : 1;
    return (<Card className={classes.root}>
      ...
      <div className={classes.table} style={{
        ...
        <div className={classes.tableHeader}>
          <Typography variant="caption" sx={{ position: "sticky", top: 0, fontWeight: "bold" }}>
            {columnName}
          </Typography>
        </div>
        ...
        {showPercentage ? (<div className={classes.tableHeader} style={{ textAlign: "end" }}>
            <Typography variant="caption" sx={{ fontWeight: "bold" }}>
              Share
            </Typography>
          </div>) : null}
        <div className={classes.divider}/>
        {data.map(([key, datum]) => (<Bar key={key} {...datum} maxCount={maxCount} totalCount={totalCount} showPercentage={showPercentage}/>))}
      </div>
    </Card>);
};
const Bar = ({ label, count, maxCount, totalCount, showPercentage, }) => {
    const classes = useStyles();
    ...
    return (<>
      ...
      {showPercentage ? (<div className={classes.tableCell} style={{ textAlign: "end" }}>
          <Typography variant="caption" sx={{ lineHeight: "normal" }}>
            {formatInteger((count / totalCount) * 100)}%
          </Typography>
        </div>) : null}
      <div className={classes.tableCell}>
        {count > 0 ? (!easterEgg ? (<Box sx={{
              ...
              {Array(Math.ceil((count / maxCount) * 40))
                .fill(0)
                .map((_, i) => {
                return (<motion.div key={i} style={{ display: "inline-block" }} initial={{ scale: 0, opacity: 0 }} animate={{
                        opacity: 1,
                        scale: 1,
