...
export const useSearchDatasetPanelStore = createStore((set) => ({
    isOpen: false,
    toggle: () => {
...
const useAddChartConfigBasedOnNewDataset = () => {
    ...
    const handleAddNewDataset = useEventCallback(async (datasetIri) => {
        const chartState = await initChartStateFromCube(client, datasetIri, dataSource, locale);
        if (!chartState) {
            return;
        }
        assert((chartState === null || chartState === void 0 ? void 0 : chartState.state) === "CONFIGURING_CHART", "After init, chart state should be in CONFIGURING_CHART state");
        dispatch({
...
export const AddNewDatasetPanel = () => {
    ...
    return (<RightDrawer open={isOpen} onClose={handleClose} onExited={handleExited}>
      <DialogCloseButton onClick={() => handleClose()}/>
      <ConfiguratorStateProvider chartId="new" allowDefaultRedirect={false}>
        <SelectDatasetStep variant="drawer" dataset={dataSetIri} onClickBackLink={(ev) => {
            ev.preventDefault();
            setDataSetIri("");
