...
export const ColorPalette = ({ field, disabled, colorConfigPath, component, }) => {
    const [state, dispatch] = useConfiguratorState(isConfiguring);
    const chartConfig = getChartConfig(state);
    const user = useUser();
    const { data: customColorPalettes, invalidate } = useUserPalettes();
    const hasColors = hasDimensionColors(component);
    const defaultPalette = hasColors && component
        ? getDefaultCategoricalPalette(component.values.map((d) => d.color).filter(Boolean))
        : null;
    ...
    const currentPaletteName = isColorInConfig(chartConfig)
        ...
        : get(chartConfig, `fields["${chartConfig.activeField}"].${colorConfigPath ? `${colorConfigPath}.` : ""}paletteId`);
    const currentPalette = palettes.find((p) => p.value === currentPaletteName);
    const handleChangePalette = useEvent((paletteId) => {
    ...
    const handleChartConfigUpdate = (value, selectedPalette) => {
        ...
        const customPalette = (customColorPalettes === null || customColorPalettes === void 0 ? void 0 : customColorPalettes.find((p) => p.paletteId === value)) ||
            selectedPalette;
        if (customPalette) {
    ...
    const handleOpenCreateColorPalette = useEvent((e) => {
        setAnchorEl(e.currentTarget);
    });
    const handleCloseCreateColorPalette = useEvent((palette) => {
        invalidate();
        setAnchorEl(undefined);
        handleChartConfigUpdate(palette === null || palette === void 0 ? void 0 : palette.paletteId, palette);
        anchorEl === null || anchorEl === void 0 ? void 0 : anchorEl.focus();
    });
    const isValidValue = (value) => {
        const isPaletteValue = palettes.some((p) => p.value === value);
        const isCustomPaletteValue = customColorPalettes === null || customColorPalettes === void 0 ? void 0 : customColorPalettes.some((p) => p.paletteId === value);
    ...
    return (<Flex sx={{
      ...
      <div>
        ...
        <Select size="sm" MenuProps={selectMenuProps} renderValue={(selected) => {
            var _a;
            if (!selected || !isValidValue(selected)) {
                return (<Trans id="controls.color.palette.select">
                  Select a color palette
                </Trans>);
            }
            return (<Flex gap={0.5} flexWrap="wrap">
                {currentPalette
                    ? currentPalette.colors.map((color, i) => (<ColorSquare key={i} color={color} disabled={disabled}/>))
                    : (_a = customColorPalettes === null || customColorPalettes === void 0 ? void 0 : customColorPalettes.find((palette) => palette.paletteId === currentPaletteName)) === null || _a === void 0 ? void 0 : _a.colors.map((color, i) => (<ColorSquare key={i} color={color} disabled={disabled}/>))}
              </Flex>);
        }} value={value} displayEmpty>
          {user && (<MenuItem onClick={handleOpenCreateColorPalette}>
              <Button variant="text" size="sm" color="primary" sx={{
                "&:hover": {
                    color: "primary.main",
                },
            }}>
          ...
          {customColorPalettes && customColorPalettes.length > 0 && (<Flex sx={{ flexDirection: "column", gap: 2, pt: 3 }}>
              <Typography variant="caption" sx={{ px: 4, color: "text.secondary", fontWeight: 700 }}>
          ...
          {customColorPalettes &&
            (customColorPalettes === null || customColorPalettes === void 0 ? void 0 : customColorPalettes.length) > 0 &&
            customColorPalettes
                .filter((palette) => palette.type === "categorical")
                .map((palette) => (<ColorPaletteMenuItem key={palette.paletteId} value={palette.paletteId} label={palette.name} colors={palette.colors} selected={palette.paletteId === value} onClick={() => handleChangePalette(palette.paletteId)}/>))}
          ...
          {palettes.map((palette) => (<ColorPaletteMenuItem key={palette.value} value={palette.value} label={palette.label} colors={palette.colors} selected={palette.value === value} onClick={() => handleChangePalette(palette.value)}/>))}
        </Select>
      </div>
      {component && (<ColorPaletteControls field={field} component={component} state={state} colorConfigPath={colorConfigPath} customColorPalettes={customColorPalettes}/>)}
      <ConfiguratorDrawer open={!!anchorEl} hideBackdrop>
        <ColorPaletteDrawerContent onClose={(palette) => handleCloseCreateColorPalette(palette)} type="categorical" customColorPalettes={customColorPalettes}/>
      </ConfiguratorDrawer>
    </Flex>);
};
const ColorPaletteMenuItem = ({ value, label, colors, selected, onClick, }) => {
    return (<MenuItem value={value} onClick={onClick} sx={{
            display: "flex",
      ...
      <Flex sx={{ flexDirection: "column", gap: 1, width: "100%" }}>
        ...
        <Flex sx={{
          ...
          <Flex sx={{ gap: 0.5, flexWrap: "wrap" }}>
            {colors.map((c) => (<ColorSquare key={c} color={c}/>))}
          </Flex>
          <div style={{
            minWidth: 24,
            minHeight: 20,
...
export const ColorSquare = ({ disabled, color, }) => {
    const classes = useColorSquareStyles();
    return (<Box className={classes.root} data-testid="select-color-square" sx={{
            backgroundColor: disabled ? "grey.300" : color,
        }}/>);
};
const ColorPaletteControls = ({ field, colorConfigPath, component, state, customColorPalettes, }) => {
    const [, dispatch] = useConfiguratorState();
    const chartConfig = getChartConfig(state);
    const paletteId = isColorInConfig(chartConfig)
        ? get(chartConfig, `fields.color.paletteId`)
        : get(chartConfig, `fields["${field}"].${colorConfigPath ? `${colorConfigPath}.` : ""}paletteId`, DEFAULT_CATEGORICAL_PALETTE_ID);
    const customPalette = useMemo(() => {
        return customColorPalettes === null || customColorPalettes === void 0 ? void 0 : customColorPalettes.find((p) => p.paletteId === paletteId);
    }, [customColorPalettes, paletteId]);
    const colorMapping = isColorInConfig(chartConfig)
        ? get(chartConfig, `fields.color.colorMapping`)
        : get(chartConfig, `fields["${field}"].${colorConfigPath ? `${colorConfigPath}.` : ""}colorMapping`);
    ...
    if (colorMapping) {
        const currentPalette = getPalette({
            paletteId,
            fallbackPalette: customPalette === null || customPalette === void 0 ? void 0 : customPalette.colors,
        });
        ...
        const same = matchedColorsInPalette.every((d, i) => d === colorMappingColors[i]) ||
            paletteId === "dimension";
        return (<Flex sx={{
                alignItems: "center",
                columnGap: 2,
        ...
        <Button variant="text" color="primary" size="xs" disabled={same} onClick={resetColorPalette}>
          <Trans id="controls.color.palette.reset">Reset color palette</Trans>
        </Button>
        •
        <Button variant="text" color="primary" size="xs" onClick={shuffleColors}>
