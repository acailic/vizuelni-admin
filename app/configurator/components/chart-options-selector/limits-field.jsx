...
export const LimitsField = ({ chartConfig, dimensions, measure, }) => {
    var _a;
    const [_, dispatch] = useConfiguratorState(isConfiguring);
    const filters = useChartConfigFilters(chartConfig);
    const onToggle = useEvent((checked, limit) => {
        const actionProps = {
    ...
    const availableLimitOptions = useMemo(() => {
        return measure.limits
            .map((limit) => {
            const { limit: maybeLimit, wouldBeValid, relatedAxisDimensionValueLabel, } = getMaybeValidChartConfigLimit({
                chartConfig,
                measureId: measure.id,
                axisDimension,
                limit,
            ...
            const { color = "#ffffff", lineType = "solid", symbolType = "circle", } = maybeLimit !== null && maybeLimit !== void 0 ? maybeLimit : {};
            return {
    ...
    const colors = getPalette({
        paletteId,
        fallbackPalette: (_a = userPalettes === null || userPalettes === void 0 ? void 0 : userPalettes.find((d) => d.paletteId === paletteId)) === null || _a === void 0 ? void 0 : _a.colors,
    });
    const supportsLimitSymbols = getSupportsLimitSymbols(chartConfig);
    return availableLimitOptions.length > 0 ? (<ControlSection collapse>
      ...
      <ControlSectionContent component="fieldset">
        {availableLimitOptions.map(({ relatedAxisDimensionValueLabel, maybeLimit, limit, color, lineType, symbolType, }, i) => {
            /** It means that the limit is rendered as overarching line, not tied
             * to any axis dimension value. */
            const hasNoAxisDimension = relatedAxisDimensionValueLabel === undefined;
            return (<Box key={i} sx={{ mb: 2 }}>
                <Flex flexDirection="column" sx={{
                    gap: 4,
                    mb: limit.type === "value-range" || supportsLimitSymbols
                        ? 4
                        : 0,
                }}>
                  <Box sx={{ display: "flex", alignItems: "center", gap: 3 }}>
                    <Switch id={`limit-${i}`} label={t({
                    id: "controls.section.targets-and-limit-values.show-target",
                    message: "Show target",
                })} checked={!!maybeLimit} onChange={(e) => {
                    onToggle(e.target.checked, limit);
                }}/>
                  </Box>
                  <Flex justifyContent="space-between" alignItems="center" width="100%">
                    <LegendItem label={limit.name} color={color} symbol="square"/>
                    <ColorPickerMenu colors={colors} selectedHexColor={color} onChange={(color) => {
                    dispatch({
                        type: "LIMIT_SET",
                        value: {
                            measureId: measure.id,
                ...
                {limit.type === "single" && supportsLimitSymbols ? (<div>
                    ...
                    <RadioGroup>
                      <Radio name={`limit-${i}-symbol-type-dot`} label={t({ id: "controls.symbol.dot", message: "Dot" })} value="dot" checked={symbolType === "circle"} onChange={() => {
                        dispatch({
                            type: "LIMIT_SET",
                            value: {
                                measureId: measure.id,
                      ...
                      <Radio name={`limit-${i}-symbol-type-cross`} label={t({
                        id: "controls.symbol.cross",
                        message: "Cross",
                    })} value="cross" checked={symbolType === "cross"} onChange={() => {
                        dispatch({
                            type: "LIMIT_SET",
                            value: {
                                measureId: measure.id,
                      ...
                      <Radio name={`limit-${i}-symbol-type-triangle`} label={t({
                        id: "controls.symbol.triangle",
                        message: "Triangle",
                    })} value="triangle" checked={symbolType === "triangle"} onChange={() => {
                        dispatch({
                            type: "LIMIT_SET",
                            value: {
                                measureId: measure.id,
                ...
                {limit.type !== "single" ||
                    ...
                    <RadioGroup>
                      <Radio name={`limit-${i}-line-type-solid`} label={t({
                        id: "controls.line.solid",
                        message: "Solid",
                    })} value="solid" checked={lineType === "solid"} onChange={() => {
                        dispatch({
                            type: "LIMIT_SET",
                            value: {
                                measureId: measure.id,
                      ...
                      <Radio name={`limit-${i}-line-type-dashed`} label={t({
                        id: "controls.line.dashed",
                        message: "Dashed",
                    })} value="dashed" checked={lineType === "dashed"} onChange={() => {
                        dispatch({
                            type: "LIMIT_SET",
                            value: {
                                measureId: measure.id,
