...
export const ColorComponentField = ({ chartConfig, encoding, component, componentTypes, dimensions, measures, getFieldOptionGroups, optional, enableUseAbbreviations, }) => {
    ...
    const nbColorOptions = useMemo(() => {
        return Array.from({ length: Math.min(7, Math.max(0, nbOptions - 2)) }, (_, i) => i + 3).map((d) => ({ value: d, label: `${d}` }));
    }, [nbOptions]);
    ...
    const colorComponent = [...dimensions, ...measures].find((d) => d.id === colorComponentId);
    ...
    const nbClass = get(chartConfig, ["fields", field, "color", "nbClass"]);
    return (<ControlSection collapse>
      <SectionTitle iconName="swatch">
        <Trans id="controls.color">Color</Trans>
      </SectionTitle>
      <ControlSectionContent>
        <ChartOptionSelectField id="color-component" label={t({
            id: "controls.select.measure",
            message: "Select a measure",
        })} field={field} path="color.componentId" options={options} optionGroups={optionGroups} optional={optional}/>
        {enableUseAbbreviations && (<Box sx={{ mt: 1 }}>
            <Abbreviations field={field} path="color" component={colorComponent}/>
          </Box>)}
        {field === "symbolLayer" && (<Box sx={{ mt: 1 }}>
            <ChartOptionSliderField field={field} path="color.opacity" label={t({
                id: "controls.color.opacity",
                message: "Opacity",
            })} min={0} max={100} step={10} defaultValue={colorType === "fixed"
        ...
        {colorType === "fixed" ? (<>
            ...
            })} field={field} path="color.value"/>
          </>) : colorType === "categorical" ? (colorComponentId &&
            component.id !== colorComponentId &&
            colorComponent &&
            !isMeasure(colorComponent) ? (<DimensionValuesMultiFilter dimension={colorComponent} field={field} colorConfigPath="color" colorComponent={colorComponent}/>) : (<ColorPalette field="symbolLayer" colorConfigPath="color" component={colorComponent}/>)) : colorType === "numerical" ? (<div>
            ...
            <RadioGroup>
              <ChartOptionRadioField label={t({
                id: "controls.color.scale.type.continuous",
                message: "Continuous",
            })} field={field} path="color.scaleType" value="continuous"/>
              {nbOptions >= 3 && (<ChartOptionRadioField label={t({
                    id: "controls.color.scale.type.discrete",
                    message: "Discrete",
                })} field={field} path="color.scaleType" value="discrete"/>)}
            </RadioGroup>
          </div>) : null}
        {colorScaleType === "discrete" && nbOptions >= 3 ? (<Stack spacing={2}>
            <ChartOptionSelectField id="color-scale-interpolation" label={t({
                id: "controls.color.scale.interpolation",
                message: "Interpolation",
            })} field={field} path="color.interpolationType" options={[
                {
                    label: t({
            ...
            <ChartOptionSelectField id="number-of-colors" label={t({
                id: "controls.color.scale.number.of.classes",
                message: "Number of classes",
            })} field={field} path="color.nbClass" options={nbColorOptions} getValue={(d) => +d}/>
          </Stack>) : null}
      </ControlSectionContent>
    </ControlSection>);
};
