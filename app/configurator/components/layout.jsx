...
export const PanelHeaderLayout = (props) => {
    const { children, type, ...rest } = props;
    const classes = useStyles();
    const [ref] = useResizeObserver(({ height }) => {
        if (height) {
            document.documentElement.style.setProperty(__PANEL_HEADER_CSS_VAR, `${height}px`);
        }
    });
    return (<Box {...rest} ref={ref} component="section" data-testid="panel-header" className={clsx(classes.panelHeaderLayout, classes[`${type}PanelHeaderLayout`], rest.className)}>
      {children}
    </Box>);
};
export const PanelHeaderWrapper = (props) => {
    const { children, type, ...rest } = props;
    const classes = useStyles();
    return (<Box {...rest} component="section" data-testid={`panel-header-${type}`} className={clsx(classes[`${type}PanelHeaderWrapper`], rest.className)}>
      {children}
    </Box>);
};
export const PanelLayout = (props) => {
    const { children, type, ...rest } = props;
    const classes = useStyles();
    return (<Box {...rest} className={clsx(classes.panelLayout, classes[`${type}PanelLayout`], rest.className)}>
      {children}
    </Box>);
};
export const PanelBodyWrapper = (props) => {
    const { children, type, ...rest } = props;
    const classes = useStyles();
    return (<Box {...rest} component="section" data-testid={`panel-body-${type}`} className={clsx(classes[`${type}PanelBodyWrapper`], rest.className)}>
      {children}
    </Box>);
};
