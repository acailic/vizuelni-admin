...
export const ChartAnnotations = () => {
    ...
    return (<ControlSection role="tablist" aria-labelledby="controls-annotations" collapse defaultExpanded={false}>
      ...
      <ControlSectionContent gap="none" px="none">
        {chartConfig.annotations.map((annotation) => (<ChartAnnotationTab key={annotation.key} annotation={annotation}/>))}
        <AddAnnotationButton />
      </ControlSectionContent>
    </ControlSection>);
};
const ChartAnnotationTab = ({ annotation }) => {
    var _a;
    const locale = useLocale();
    const [state, dispatch] = useConfiguratorState(isConfiguring);
    const chartConfig = getChartConfig(state);
    const label = annotation.text[locale];
    const value = (_a = chartConfig.activeField) !== null && _a !== void 0 ? _a : "";
    ...
    return (<ControlTab upperLabel={label
            ? undefined
            : t({
        ...
        </Markdown>} icon="text" rightIcon={<Typography component="span" color="text.primary">
          <Icon name="pen"/>
        </Typography>} value={value} onClick={handleClick}/>);
};
