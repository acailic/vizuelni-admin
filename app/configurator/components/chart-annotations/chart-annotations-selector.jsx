...
export const ChartAnnotationsSelector = () => {
    ...
    const annotation = chartConfig.annotations.find((annotation) => annotation.key === activeField);
    ...
    const dimensions = useMemo(() => {
        var _a;
        return (_a = componentsData === null || componentsData === void 0 ? void 0 : componentsData.dataCubesComponents.dimensions) !== null && _a !== void 0 ? _a : [];
    }, [componentsData]);
    const annotationLabel = useMemo(() => {
        return getAnnotationLabel({ annotation, dimensions });
    }, [annotation, dimensions]);
    const handleStyleTypeChange = useEvent((highlightType) => {
    ...
    const handleColorChange = useEvent((color) => {
    ...
    const handleClearText = useEvent((key) => {
        dispatch({
    ...
    const handleClose = useEvent(() => {
        ...
        if (isEqual(omit(annotation, "key"), omit(defaultHighlightAnnotation, "key"))) {
    ...
    const handleDefaultOpenChange = useEvent((defaultOpen) => {
    ...
    const isSegmented = isSegmentInConfig(chartConfig) &&
    ...
    return (<>
      ...
      {isSegmented ? null : (<ControlSection collapse>
          <SectionTitle>
          ...
          <ControlSectionContent gap="xl">
            ...
            })} value="none" checked={annotation.highlightType === "none"} disabled={!hasTargets} onChange={() => handleStyleTypeChange("none")}/>
              <Radio label={t({
                id: "controls.fill-and-text",
                message: "Fill and text box",
            })} value="filled" checked={annotation.highlightType === "filled"} disabled={!hasTargets} onChange={() => handleStyleTypeChange("filled")}/>
            </RadioGroup>
            {annotation.highlightType === "filled" && annotation.color && (<ColorPicker label={annotation.color} color={annotation.color} symbol="square" onChange={handleColorChange}/>)}
          </ControlSectionContent>
        </ControlSection>)}
        ...
        <ControlSectionContent px="none">
          <ControlTab upperLabel={label
            ...
            : t({
              ...
              </Markdown>} icon="text" onClick={drawerState.open} value="text" disabled={!hasTargets}/>
          <Button variant="text" color="primary" size="xs" disabled={!hasTargets || !hasText} onClick={() => handleClearText(annotation.key)} sx={{ width: "fit-content", ml: 4 }}>
      ...
      <ControlSection collapse>
        ...
        })} checked={annotation.defaultOpen} disabled={!hasTargets || !hasText} onChange={() => handleDefaultOpenChange(!annotation.defaultOpen)}/>
        </ControlSectionContent>
      </ControlSection>
      <AnnotationDrawer opened={drawerState.isOpen} close={drawerState.close}/>
      <ControlSection hideTopBorder style={{ marginTop: "auto" }}>
        <ControlSectionContent>
          <Button variant="outlined" size="sm" color="error" startIcon={<Icon name="trash" size={20}/>} onClick={handleDelete}>
...
const AnnotationDrawer = ({ opened, close, }) => {
    const orderedLocales = useOrderedLocales();
    const [state, dispatch] = useConfiguratorState(isConfiguring);
    const chartConfig = getChartConfig(state);
    const activeField = chartConfig.activeField;
    const annotation = chartConfig.annotations.find((annotation) => annotation.key === activeField);
    const handleTextChange = useEvent((locale, value) => {
        if (!annotation) {
            return;
        }
        dispatch({
    ...
    return (<ConfiguratorDrawer open={opened} hideBackdrop>
      <div role="tabpanel" id={`annotations-panel-${activeField}`} aria-labelledby={`annotations-tab-${activeField}`} tabIndex={-1} style={{ overflowX: "hidden", overflowY: "auto" }}>
        <ControlSection hideTopBorder>
          ...
          <ControlSectionContent>
            {orderedLocales.map((locale) => (<div key={`${locale}-${activeField}`}>
                <MarkdownInput name={`annotation-${locale}`} label={getFieldLabel(locale)} value={annotation.text[locale]} characterLimit={500} disablePlugins={{
                headings: true,
            }} disableToolbar={{
                blockType: true,
                listToggles: true,
...
const getAnnotationLabel = ({ annotation, dimensions, }) => {
    if (!annotation) {
        return;
    }
...
const getTargetLabel = (target, { dimensions }) => {
    if (!target) {
        return;
    }
