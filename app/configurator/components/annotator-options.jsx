...
export const ChartAnnotatorSelector = () => {
    ...
    return (<AnnotatorOptions type="chart" activeField={chartConfig.activeField} meta={chartConfig.meta}/>);
};
export const LayoutAnnotatorSelector = () => {
    const [state] = useConfiguratorState(isLayouting);
    return (<AnnotatorOptions type="layout" activeField={state.layout.activeField} meta={state.layout.meta}/>);
};
const AnnotatorOptions = ({ type, activeField, meta, }) => {
    ...
    return (<Box ref={panelRef} role="tabpanel" id={`annotator-panel-${activeField}`} aria-labelledby={`annotator-tab-${activeField}`} tabIndex={-1} sx={{ overflowX: "hidden", overflowY: "auto" }}>
      <ControlSection hideTopBorder>
        ...
        <ControlSectionContent>
          {orderedLocales.map((locale) => (<div key={`${locale}-${activeField}`}>
              <MetaInputField type={type} inputType={activeField === "label" ? "text" : "markdown"} metaKey={activeField} locale={locale} label={getFieldLabel(locale)} value={meta[activeField][locale]} disableToolbar={{
                blockType: true,
