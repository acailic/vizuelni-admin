...
const DatasetRow = ({ canRemove, handleDatasetClick, cube, added, onRemoveAdded, }) => {
    const locale = useLocale();
    const client = useClient();
    const classes = useStyles();
    const [state, dispatch] = useConfiguratorState(isConfiguring);
    const [loading, setLoading] = useState(false);
    return (<MaybeTooltip title={<Box alignItems="center" display="flex" gap="0.75rem">
          <Box color="success.main">
            <Icon name="checkmarkCircle"/>
          </Box>
          {t({
                id: "dataset.search.preview.added-dataset",
                message: "New dataset added. You can now use its data in the chart(s).",
            })}
        </Box>} tooltipProps={{
            open: added,
            placement: "right",
            PopperProps: {
                onClick: () => onRemoveAdded === null || onRemoveAdded === void 0 ? void 0 : onRemoveAdded(),
                className: classes.tooltipPopper,
            },
        }}>
      <div key={cube.iri} className={clsx(classes.row, { [classes.added]: added })}>
        <Flex sx={{ flexDirection: "column", alignItems: "flex-start", gap: 1 }}>
          <Button variant="text" color="primary" size="sm" onClick={handleDatasetClick}>
            <Trans id="dataset.footnotes.dataset">Dataset</Trans>
          </Button>
          <Typography variant="body2">{cube.title}</Typography>
        </Flex>
        <div>
          {canRemove ? (<IconButton disabled={loading} onClick={async () => {
                try {
                    const chartConfig = getChartConfig(state);
                    const newCubes = chartConfig.cubes.filter((c) => c.iri !== cube.iri);
...
export const DatasetsControlSection = () => {
    ...
    const cubes = useMemo(() => {
        var _a;
        const activeChartConfig = state.chartConfigs.find((x) => x.key === state.activeChartKey);
        const cubes = uniqBy((_a = activeChartConfig === null || activeChartConfig === void 0 ? void 0 : activeChartConfig.cubes) !== null && _a !== void 0 ? _a : [], (x) => x.iri);
        return cubes;
    ...
    const { isOpen: isDatasetDialogOpen, open: openDatasetDialog, close: closeDatasetDialog, } = useDisclosure();
    const [addedIri, setAddedIri] = useState(null);
    ...
    return (<ControlSection role="tablist" aria-labelledby="controls-data" collapse defaultExpanded hideTopBorder>
      ...
      <ControlSectionContent data-testid="configurator-datasets">
        <Flex sx={{ flexDirection: "column", gap: 6 }}>
          {(_a = metadataQuery.data) === null || _a === void 0 ? void 0 : _a.dataCubesMetadata.map((cube) => {
            return (<DatasetRow key={cube.iri} handleDatasetClick={handleDatasetClick} canRemove={cubes.length > 1} added={addedIri === cube.iri} cube={cube}/>);
        })}
        </Flex>
        <Flex sx={{ justifyContent: "end" }}>
          <Button variant="text" size="sm" color="primary" startIcon={<Icon name="plus" size={20}/>} onClick={openDatasetDialog}>
            {t({ id: "chart.datasets.add", message: "Add dataset" })}
          </Button>
        </Flex>
        <AddDatasetDrawer state={state} open={isDatasetDialogOpen} onClose={closeDatasetDialog}/>
