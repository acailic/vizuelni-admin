...
export const PreviewContainer = ({ children, breakpoint, singleColumn, }) => {
    ...
    const width = useMemo(() => {
        if (breakpoint) {
            switch (breakpoint) {
                ...
                default:
                    const _exhaustiveCheck = breakpoint;
    ...
    return (<div className={classes.container}>
      <div className={classes.chartContainer} style={{ width, paddingTop: breakpoint ? "0.5rem" : 0 }}>
        {children}
      </div>
    </div>);
};
export const PreviewBreakpointToggleMenu = ({ value, onChange, }) => {
    const classes = useStyles();
    ...
    return (<ToggleButtonGroup className={classes.toggleButtonGroup} value={value} exclusive>
      {previewBreakpointOptions.map(({ breakpoint, iconName, title }) => {
            return [
                <Tooltip key={`${breakpoint}-tooltip`} title={title} arrow enterDelay={1000}>
            <ToggleButton className={clsx(classes.toggleButton, {
                        [classes.toggleButtonSelected]: value === breakpoint,
                    })} value={breakpoint} onClick={() => onChange(breakpoint)}>
              <Icon name={iconName} size={16}/>
            </ToggleButton>
          </Tooltip>,
                breakpoint !== "sm" ? (<Divider key={`${breakpoint}-divider`} className={classes.divider} orientation="vertical" flexItem/>) : null,
            ];
        })}
    </ToggleButtonGroup>);
};
