...
const EmptyGroups = () => {
    ...
    return (<Typography variant="body2" component="div" className={classes.emptyGroups}>
      <Trans id="controls.groups.empty-help">
        Drag and drop columns here to make groups.
      </Trans>
    </Typography>);
};
export const ChartConfiguratorTable = ({ state, }) => {
    const { dimensions, measures, currentDraggableId, chartConfig, handleDragEnd, handleDragStart, } = useTableChartController(state);
    ...
    const currentDraggedField = currentDraggableId !== null ? chartConfig.fields[currentDraggableId] : null;
    const isGroupsDropDisabled = (currentDraggedField === null || currentDraggedField === void 0 ? void 0 : currentDraggedField.componentType) === "NumericalMeasure";
    return (<>
      ...
      <ControlSection role="tablist" aria-labelledby="controls-chart-type" collapse>
        ...
        <ControlSectionContent gap="none">
          <ChartTypeSelector showHelp={false} state={state} chartKey={chartConfig.key}/>
      ...
      <ControlSection collapse>
        ...
        <ControlSectionContent gap="none" px="none" role="tablist" aria-labelledby="controls-settings">
          <ChartAnnotatorTabField key="sorting" value="table-sorting" icon="sort" mainLabel={<Trans id="controls.table.sorting">Sorting</Trans>}/>
        </ControlSectionContent>
      </ControlSection>
      <ControlSection collapse>
        ...
        <ControlSectionContent sx={{ my: 2 }}>
          ...
          <ChartOptionCheckboxField label={t({
            ...
            message: "Show Search",
        })} field={null} path="settings.showSearch"/>
        </ControlSectionContent>
      </ControlSection>
      <DragDropContext onDragEnd={(result) => handleDragEnd({
        ...
        <TabDropZone id="groups" title={<Trans id="controls.section.groups">Groups</Trans>} dimensions={dimensions} measures={measures} items={groupFields} isDropDisabled={isGroupsDropDisabled} emptyComponent={<EmptyGroups />}/>
        <TabDropZone id="columns" title={<Trans id="controls.section.columns">Columns</Trans>} dimensions={dimensions} measures={measures} items={columnFields}/>
      </DragDropContext>
