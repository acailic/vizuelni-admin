...
describe("initChartStateFromChart", () => {
    ...
    it("should fetch work if existing chart is valid", async () => {
        ...
        // @ts-ignore
        const { key, activeChartKey, chartConfigs, ...rest } = await initChartStateFromChartCopy(mockClient, "abcde");
        expect(key).toBe(undefined);
        const { key: chartConfigKey, ...chartConfig } = chartConfigs[0];
        const { key: migratedKey, activeChartKey: migratedActiveChartKey, chartConfigs: migratedChartsConfigs, ...migratedRest } = (await migrateConfiguratorState(fakeVizFixture));
        const { key: migratedChartConfigKey, ...migratedChartConfig } = migratedChartsConfigs[0];
        expect(rest).toEqual(expect.objectContaining(migrateConfiguratorState(migratedRest)));
...
describe("initChartFromLocalStorage", () => {
    it("should initialize from localStorage if valid", async () => {
        localStorage.setItem(getLocalStorageKey("viz1234"), JSON.stringify({
...
describe("initChartStateFromCube", () => {
    ...
    it("should work init fields with existing dataset and go directly to 2nd step", async () => {
        const res = await initChartStateFromCube(mockClient, "https://environment.ld.admin.ch/foen/ubd0104/3/", dataSource, "en");
        expect(res).toEqual(expect.objectContaining({
            state: "CONFIGURING_CHART",
        }));
    });
    it("should prefer possible filters if provided", async () => {
        const res = (await initChartStateFromCube(mockClient, "mapDataset", dataSource, "en"));
...
describe("getFiltersByMappingStatus", () => {
    it("should correctly retrieve categorical color iris", () => {
        ...
        expect([...mappedFiltersIds]).toEqual(expect.arrayContaining(["areaColorIri", "symbolColorIri"]));
    });
...
describe("publishing chart config", () => {
    it("should run correct number of times in singleURLs layout mode", async () => {
        ...
        const cb = vi.fn();
        await publishState({}, key, state, async (_, i) => cb(i));
        expect(cb.mock.calls.map((c) => c[0])).toEqual(Array.from({ length: publishableChartKeys.length }, (_, i) => i + 1));
    });
});
