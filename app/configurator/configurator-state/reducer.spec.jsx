...
import { configJoinedCubes, configStateMock, dimensionsJoinedCubes, groupedColumnChartDimensions, groupedColumnChartMeasures, } from "@/configurator/configurator-state/mocks";
import { applyNonTableDimensionToFilters, applyTableDimensionToFilters, deriveFiltersFromFields, ensureDashboardLayoutIsCorrect, handleChartFieldChanged, handleChartFieldDeleted, handleChartFieldUpdated, reducer, setRangeFilter, updateColorMapping, } from "@/configurator/configurator-state/reducer";
...
afterEach(() => {
    ...
    try {
        ...
        throw Error("One of the tests is dirtying the configStateMock. Please ensure that the configStateMock is not modified in the tests by using produce from immer.");
    }
});
...
const getCachedComponents = getCachedComponentsOriginal;
afterEach(() => {
    vi.restoreAllMocks();
});
describe("add dataset", () => {
    ...
    const addAction = {
        ...
        value: {
            ...
            joinBy: mkVersionedJoinBy({
                "https://first-dataset": ["year-period-1"],
                "http://second-dataset": ["year-period-2"],
            }),
        },
    };
    const removeAction = {
    ...
    const runReducer = (state, action) => produce(state, (state) => reducer(state, action));
    it("should add/remove a cube and replace correctly joinBy to the first cube", () => {
        var _a, _b;
        const newState = runReducer(state, addAction);
        const config = newState.chartConfigs[0];
        expect((_a = config.fields["areaLayer"]) === null || _a === void 0 ? void 0 : _a.componentId).toEqual("joinBy__0");
        expect((_b = config.fields["areaLayer"]) === null || _b === void 0 ? void 0 : _b.color.componentId).toEqual("joinBy__0");
    ...
    it("should correctly remove a cube", () => {
        ...
        getCachedComponents.mockImplementation((options) => {
            ...
            expect(options.cubeFilters.map((x) => x.joinBy).every((x) => x === undefined)).toBe(true);
            return getCachedComponentsMock.electricityPricePerCantonDimensions;
        });
        const newState2 = runReducer(newState, removeAction);
        const config = newState2.chartConfigs[0];
...
describe("add chart based on the same cube", () => {
    ...
    it("should correctly derive filters for a new chart", () => {
        ...
        const newStateAfterSameCube = runReducer(state, actionSameCube);
    ...
    it("should keep blocks in sync when adding a new chart based on new cube", () => {
        const newStateAfterSameCube = runReducer(state, actionNewCube);
        expect(newStateAfterSameCube.chartConfigs.map((c) => c.key)).toEqual(newStateAfterSameCube.layout.blocks.map((b) => b.key));
    });
});
...
describe("handleChartFieldChanged", () => {
    it("should not reset symbol layer when it's being updated", () => {
        getCachedComponents.mockImplementation(() => getCachedComponentsMock.geoAndNumerical);
        const state = configStateMock.map;
        const newState = produce(state, (state) => handleChartFieldChanged(state, {
            type: "CHART_FIELD_CHANGED",
            value: {
                locale: "en",
                field: "symbolLayer",
                componentId: "mapDataset(VISUALIZE.ADMIN_COMPONENT_ID_SEPARATOR)symbolLayerIri",
            },
        }));
        expect(newState.chartConfigs[0].fields.symbolLayer.color).toBeDefined();
    });
});
describe("CHART_FIELD_DELETED", () => {
    ...
    it("should correctly clear calculation config when segment field is deleted", () => {
        ...
        expect((_a = chartConfig.interactiveFiltersConfig) === null || _a === void 0 ? void 0 : _a.calculation).toEqual({
            active: false,
            type: "identity",
        });
...
describe("colorMapping", () => {
    it("should correctly reset color mapping for maps", () => {
        ...
        expect(getChartConfig(newState).fields.areaLayer.color.colorMapping).toEqual({
            red: "red",
            green: "green",
            blue: "blue",
        });
    ...
    it("should correctly reset color mapping for grouped column chart", () => {
        ...
        const newState = produce(state, (state) => {
            return updateColorMapping(state, {
                ...
                value: {
                    ...
                    dimensionId: "https://energy.ld.admin.ch/sfoe/bfe_ogd84_einmalverguetung_fuer_photovoltaikanlagen/Kanton",
                    values: [
                        {
                            value: "https://ld.admin.ch/canton/1",
        ...
        expect(getChartConfig(newState).fields.color.colorMapping).toEqual({
    ...
    it("should correctly shuffle color mapping for grouped column chart", () => {
        ...
        expect(getChartConfig(newState).fields.color.colorMapping).not.toEqual({
            "https://ld.admin.ch/canton/1": "#1f77b4",
            "https://ld.admin.ch/canton/2": "#ff7f0e",
            "https://ld.admin.ch/canton/3": "#17becf",
            "https://ld.admin.ch/canton/4": "#1f77b4",
    ...
    it("should use dimension colors if present", () => {
        ...
        handleChartFieldChanged(state, {
            ...
            value: {
                locale: "en",
                field: "segment",
                componentId: "mapDataset(VISUALIZE.ADMIN_COMPONENT_ID_SEPARATOR)newAreaLayerColorIri",
            },
        });
        const chartConfig = state.chartConfigs[0];
        expect(((_a = chartConfig.fields.segment) === null || _a === void 0 ? void 0 : _a.componentId) ===
        ...
        expect(chartConfig.fields.color.type === "single"
            ? chartConfig.fields.color.color
            : chartConfig.fields.color.colorMapping).toEqual(chartConfig.fields.color.type === "single"
            ? "rgb(255, 153, 0)"
...
describe("retainChartConfigWhenSwitchingChartType", () => {
    ...
    const dimensions = dataSetMetadata.dimensions.map((d) => ({
        ...d,
    ...
    const measures = dataSetMetadata.measures.map((m) => ({
        ...m,
        id: stringifyComponentId({
            unversionedCubeIri: "foo",
    ...
    const deriveNewChartConfig = (oldConfig, newChartType) => {
        const newConfig = createDraft(getChartConfigAdjustedToChartType({
    ...
    const runChecks = (config, checks) => {
        ...
        let newConfig;
        for (const { newChartType, checks: { comparisonChecks, oldConfigChecks, newConfigChecks }, } of checks) {
    ...
    it("should retain appropriate x & y fields and discard the others", async () => {
        runChecks(await migrateChartConfig(covid19ColumnChartConfig, {
            migrationProps: {
                meta: {},
                dataSet: "foo",
                dataSource: {
...
describe("handleChartOptionChanged", () => {
    it("should set required scale properties", () => {
        ...
        expect(state.chartConfigs[0].fields.areaLayer.color.nbClass).toBeTruthy();
    });
    it("should reset previous color filters", () => {
        ...
        expect(Object.keys(state.chartConfigs[0].cubes[0].filters)).not.toContain("areaLayerColorIri");
        handleChartFieldUpdated(state, {
            type: "CHART_FIELD_UPDATED",
            value: {
                locale: "en",
        ...
        expect((_a = state.chartConfigs[0].fields.areaLayer) === null || _a === void 0 ? void 0 : _a.color.componentId).toBeUndefined();
    });
});
describe("ensureDashboardLayoutIsCorrect", () => {
    it("should ensure dashboard layout is correct", () => {
        const state = configStateMock.map;
        const newState = produce(state, (state) => {
            assert(state.state === "CONFIGURING_CHART", "State should be CONFIGURING_CHART");
            state.layout.type = "dashboard";
            assert(state.layout.type === "dashboard", "Layout type should be dashboard");
        ...
        expect(newState.layout.layouts.lg.length).toBe(2);
    });
});
