import produce, { createDraft, current, isDraft, setAutoFreeze, } from "immer";
...
import { enableLayouting, isAreaConfig, isColorInConfig, isMapConfig, isSegmentInConfig, isTableConfig, } from "@/config-types";
import { getChartConfig, makeMultiFilter } from "@/config-utils";
import { mapValueIrisToColor } from "@/configurator/components/ui-helpers";
import { addDatasetInConfig, getPreviousState, getStateWithCurrentDataSource, hasChartConfigs, isConfiguring, isLayouting, removeDatasetInConfig, } from "@/configurator/configurator-state";
import { getInitialConfiguringConfigBasedOnCube, SELECTING_DATASET_STATE, } from "@/configurator/configurator-state/initial";
...
import { isGeoDimension, isJoinByComponent } from "@/domain/data";
import { getOriginalDimension, getOriginalIds, isJoinByCube, } from "@/graphql/join";
 ...
 */
export const deriveFiltersFromFields = produce((draft, options) => {
    const { dimensions, possibleFilters } = options;
    const getCubeDimensions = (dimensions, cubeIri) => {
        return dimensions.filter((d) => d.cubeIri === cubeIri || isJoinByCube(d.cubeIri));
    };
    if (draft.chartType === "table") {
        ...
        const isHidden = (dim) => hiddenFieldIds.has(dim.id);
        const isGrouped = (dim) => groupedDimensionIds.has(dim.id);
        draft.cubes.forEach((cube) => {
            ...
            cubeDimensions.forEach((dim) => {
                applyTableDimensionToFilters({
                    ...
                    possibleFilter: possibleFilters === null || possibleFilters === void 0 ? void 0 : possibleFilters.find((f) => f.id === dim.id),
                });
            });
        });
    }
        ...
        const isField = (dim) => fieldDimensionIds.has(dim.id);
        draft.cubes.forEach((cube) => {
            const cubeDimensions = getCubeDimensions(dimensions, cube.iri);
            const sortedCubeDimensions = sortBy(cubeDimensions, (d) => (isGeoDimension(d) ? -1 : 1), (d) => (d.hierarchy ? -1 : 1));
...
export const applyTableDimensionToFilters = (props) => {
    var _a, _b, _c;
    const { filters, dimension, isHidden, isGrouped, possibleFilter, cubeIri } = props;
    const originalIri = isJoinByComponent(dimension)
        ? (_a = dimension.originalIds.find((c) => c.cubeIri === cubeIri)) === null || _a === void 0 ? void 0 : _a.dimensionId
        : dimension.id;
    ...
    if (currentFilter) {
        switch (currentFilter.type) {
            ...
            case "multi":
                if (shouldBecomeSingleFilter && dimension.isKeyDimension) {
                    filters[originalIri] = {
                        type: "single",
                        value: (_b = Object.keys(currentFilter.values)[0]) !== null && _b !== void 0 ? _b : dimension.values[0].value,
                    };
                }
                break;
            case "range":
                break;
            default:
                const _exhaustiveCheck = currentFilter;
        ...
        filters[originalIri] = {
            ...
            value: (_c = possibleFilter === null || possibleFilter === void 0 ? void 0 : possibleFilter.value) !== null && _c !== void 0 ? _c : dimension.values[0].value,
        };
    }
};
export const applyNonTableDimensionToFilters = (props) => {
    ...
    if (currentFilter) {
            ...
            case "multi":
                if (!isField) {
                    ...
                    const filterValue = hierarchyTopMost
                        ...
                        : ((_b = Object.keys(currentFilter.values)[0]) !== null && _b !== void 0 ? _b : dimension.values[0].value);
                    filters[originalIri] = {
                        type: "single",
                        value: filterValue,
                    };
        ...
        const filterValue = hierarchyTopMost
            ? hierarchyTopMost.value
            : ((_c = possibleFilter === null || possibleFilter === void 0 ? void 0 : possibleFilter.value) !== null && _c !== void 0 ? _c : (_d = dimension.values[0]) === null || _d === void 0 ? void 0 : _d.value);
        if (filterValue) {
...
export const transitionStepNext = (draft, options) => {
    const { chartConfig } = options !== null && options !== void 0 ? options : {};
    switch (draft.state) {
...
const transitionStepPrevious = (draft, to) => {
    const stepTo = to !== null && to !== void 0 ? to : getPreviousState(draft);
...
export const handleChartFieldChanged = (draft, action) => {
    var _a, _b, _c;
    if (!isConfiguring(draft)) {
        return draft;
    }
    const chartConfig = getChartConfig(draft);
    const { locale, field, componentId, selectedValues: actionSelectedValues, } = action.value;
    ...
    const dimensions = (_a = dataCubesComponents === null || dataCubesComponents === void 0 ? void 0 : dataCubesComponents.dimensions) !== null && _a !== void 0 ? _a : [];
    const measures = (_b = dataCubesComponents === null || dataCubesComponents === void 0 ? void 0 : dataCubesComponents.measures) !== null && _b !== void 0 ? _b : [];
    const components = [...dimensions, ...measures];
    const component = components.find((d) => d.id === componentId);
    const selectedValues = (_c = actionSelectedValues !== null && actionSelectedValues !== void 0 ? actionSelectedValues : component === null || component === void 0 ? void 0 : component.values) !== null && _c !== void 0 ? _c : [];
    if (f) {
    ...
    sideEffect === null || sideEffect === void 0 ? void 0 : sideEffect(componentId, {
        chartConfig,
        dimensions,
        measures,
        initializing: !f,
    ...
    const componentIds = chartConfig.interactiveFiltersConfig.dataFilters.componentIds.filter((d) => d !== componentId);
    const active = componentIds.length > 0;
    chartConfig.interactiveFiltersConfig.dataFilters = {
        active,
        componentIds,
        defaultValueOverrides: Object.fromEntries(Object.entries(chartConfig.interactiveFiltersConfig.dataFilters.defaultValueOverrides).filter(([k]) => k !== componentId)),
        filterTypes: Object.fromEntries(Object.entries(chartConfig.interactiveFiltersConfig.dataFilters.filterTypes).filter(([k]) => k !== componentId)),
    };
...
export const handleChartFieldDeleted = (draft, action) => {
    var _a;
    if (!isConfiguring(draft)) {
        return draft;
    }
    ...
    sideEffect === null || sideEffect === void 0 ? void 0 : sideEffect({ chartConfig });
    return draft;
};
export const handleChartFieldUpdated = (draft, action) => {
    var _a, _b;
    if (isConfiguring(draft)) {
        ...
        const measures = (_b = dataCubesComponents === null || dataCubesComponents === void 0 ? void 0 : dataCubesComponents.measures) !== null && _b !== void 0 ? _b : [];
        if (field) {
            const sideEffect = getChartFieldOptionChangeSideEffect(chartConfig, field, path);
            sideEffect === null || sideEffect === void 0 ? void 0 : sideEffect(value, {
                chartConfig,
                dimensions,
                measures,
                field,
...
export const updateColorMapping = (draft, action) => {
    if (isConfiguring(draft)) {
        const { field, colorConfigPath, colorMapping: oldColorMapping, values, random, customPalette, } = action.value;
        const chartConfig = getChartConfig(draft);
        ...
        if (isTableConfig(chartConfig)) {
            const fieldValue = get(chartConfig, path);
            if (fieldValue) {
...
const handleInteractiveFilterChanged = (draft, action) => {
    if (isConfiguring(draft)) {
        const chartConfig = getChartConfig(draft);
...
export const setRangeFilter = (draft, action) => {
    const { dimension, from, to } = action.value;
    const chartConfig = getChartConfig(draft);
    const adjustFilter = (cubeIri, dimensionId) => {
...
const handleAddNewChartConfig = (draft, chartConfig) => {
    ...
    switch (action.type) {
        ...
        case "CHART_TYPE_CHANGED":
            if (isConfiguring(draft)) {
                ...
                const dimensions = dataCubesComponents === null || dataCubesComponents === void 0 ? void 0 : dataCubesComponents.dimensions;
                const measures = dataCubesComponents === null || dataCubesComponents === void 0 ? void 0 : dataCubesComponents.measures;
                if (dimensions && measures) {
                    const newConfig = deriveFiltersFromFields(getChartConfigAdjustedToChartType({
        ...
        case "COLOR_FIELD_SET":
            if (isConfiguring(draft)) {
                const chartConfig = getChartConfig(draft);
                setWith(chartConfig, `fields.color.paletteId`, action.value.paletteId, Object);
                setWith(chartConfig, `fields.color.${action.value.type === "single" ? "color" : "colorMapping"}`, action.value.type === "single"
                    ? action.value.color
                    : action.value.colorMapping, Object);
            }
            return draft;
        case "CHART_PALETTE_CHANGED":
            if (isConfiguring(draft)) {
                const chartConfig = getChartConfig(draft);
                const commonPath = `fields["${action.value.field}"].${action.value.colorConfigPath ? `${action.value.colorConfigPath}.` : ""}`;
                setWith(chartConfig, `${commonPath}palette`, action.value.paletteId, Object);
                setWith(chartConfig, `${commonPath}colorMapping`, action.value.colorMapping, Object);
            }
            return draft;
        case "CHART_PALETTE_RESET":
            if (isConfiguring(draft)) {
                const chartConfig = getChartConfig(draft);
                setWith(chartConfig, `fields["${action.value.field}"].${action.value.colorConfigPath
                    ? `${action.value.colorConfigPath}.`
                    : ""}colorMapping`, action.value.colorMapping, Object);
            }
            return draft;
        case "CHART_COLOR_CHANGED":
            if (isConfiguring(draft)) {
                ...
                setWith(chartConfig, `fields["${action.value.field}"].${action.value.colorConfigPath
                    ? `${action.value.colorConfigPath}.`
                    : ""}colorMapping["${action.value.value}"]`, action.value.color, Object);
            }
            return draft;
        case "CHART_META_CHANGE":
            if (isConfiguring(draft)) {
                const chartConfig = getChartConfig(draft);
                setWith(chartConfig, `meta.${action.value.path}`, action.value.value, Object);
            }
            return draft;
        ...
        case "CUSTOM_LAYER_UPDATE":
            if (isConfiguring(draft)) {
                ...
                if (isMapConfig(chartConfig)) {
                    ...
                    const i = chartConfig.baseLayer.customLayers.findIndex((l) => l.type === layer.type && l.id === layer.id);
                    if (i !== -1) {
            ...
            if (isConfiguring(draft)) {
                ...
                if (isMapConfig(chartConfig)) {
                    ...
                    chartConfig.baseLayer.customLayers =
                        chartConfig.baseLayer.customLayers.filter((layer) => !(layer.type === type && layer.id === id));
                }
            }
            return draft;
        ...
        case "CHART_CONFIG_REPLACED":
            if (isConfiguring(draft)) {
                const chartConfig = getChartConfig(draft);
                const index = draft.chartConfigs.findIndex((d) => d.key === chartConfig.key);
                draft.chartConfigs[index] = deriveFiltersFromFields(action.value.chartConfig, { dimensions: action.value.dataCubesComponents.dimensions });
            }
            return draft;
            ...
            if (isConfiguring(draft)) {
                ...
                for (const filter of filters) {
                    ...
                    if (cube) {
                        ...
                        console.warn(`Could not set filter, no cube in chart config was found with iri ${cubeIri}`);
                    }
                }
            }
            ...
            if (isConfiguring(draft)) {
                ...
                for (const filter of filters) {
                    ...
                    if (cube) {
                        ...
                        const newIFConfig = toggleInteractiveFilterDataDimension(chartConfig.interactiveFiltersConfig, dimensionId, false);
        ...
        case "CHART_CONFIG_FILTER_SET_MULTI":
            if (isConfiguring(draft)) {
                ...
                for (const resolvedCubeIri of resolvedCubeIris) {
                    const cube = chartConfig.cubes.find((cube) => {
                        var _a;
                        return cube.iri === resolvedCubeIri ||
                            ((_a = cube.joinBy) === null || _a === void 0 ? void 0 : _a.includes(resolvedCubeIri));
                    });
                    if (cube) {
        ...
        case "CHART_CONFIG_ADD":
            if (isConfiguring(draft) || isLayouting(draft)) {
                const chartConfig = (_a = createDraft(action.value.chartConfig)) !== null && _a !== void 0 ? _a : getChartConfig(draft);
                const dataCubesComponents = getCachedComponents({
        ...
        case "CHART_CONFIG_REMOVE":
            if (isConfiguring(draft) || isLayouting(draft)) {
                const index = draft.chartConfigs.findIndex((d) => d.key === action.value.chartKey);
                const removedKey = draft.chartConfigs[index].key;
                draft.chartConfigs.splice(index, 1);
                draft.layout.blocks = draft.layout.blocks.filter((block) => block.key !== removedKey);
                if (removedKey === draft.activeChartKey) {
                    draft.activeChartKey = draft.chartConfigs[Math.max(index - 1, 0)].key;
                }
            }
            else {
                console.warn("Ignoring CHART_CONFIG_REMOVE as state is incompatible with action");
            }
            ensureDashboardLayoutIsCorrect(draft);
            return draft;
        ...
        case "CHART_ANNOTATION_REMOVE":
            if (isConfiguring(draft)) {
                const chartConfig = getChartConfig(draft);
                chartConfig.annotations = chartConfig.annotations.filter((a) => a.key !== action.value.key);
        ...
        case "LIMIT_SET":
            if (isConfiguring(draft)) {
                ...
                if (!chartConfig.limits[measureId]) {
                    ...
                    const limitToAddIndex = chartConfig.limits[measureId].findIndex((d) => {
                        if (d.related.length !== limit.related.length) {
                            return false;
                        }
                        return d.related.every((r) => limit.related.some((nr) => r.dimensionId === nr.dimensionId && r.value === nr.value));
                    });
        ...
        case "LIMIT_REMOVE":
            if (isConfiguring(draft)) {
                ...
                const chartConfig = getChartConfig(draft);
                const limits = (_b = chartConfig.limits[measureId]) !== null && _b !== void 0 ? _b : [];
                ...
                if (limits.length === 1) {
                    ...
                    chartConfig.limits[measureId] = limits.filter((_, index) => index !== limitToRemoveIndex);
                }
            }
            return draft;
        ...
        case "LAYOUT_META_CHANGE":
            if (draft.state === "LAYOUTING") {
                setWith(draft, `layout.meta.${action.value.path}`, action.value.value, Object);
            }
            return draft;
        ...
        case "DASHBOARD_TIME_RANGE_FILTER_REMOVE":
            if (isLayouting(draft)) {
                setWith(draft, "dashboardFilters.timeRange", {
                    active: false,
                    timeUnit: "",
                    presets: {
                        from: "",
        ...
        case "DASHBOARD_DATA_FILTER_REMOVE":
            if (isLayouting(draft) && draft.dashboardFilters) {
                ...
                const newFilters = {
                    ...
                    dataFilters: {
                        ...draft.dashboardFilters.dataFilters,
                        componentIds: draft.dashboardFilters.dataFilters.componentIds.filter((d) => d !== dimensionId),
                    },
                };
                draft.dashboardFilters = newFilters;
            }
...
const withLogging = (reducer) => {
    return (state, action) => {
        ...
        console.log(`Action: ${action.type}`, action, isDraft(state) ? current(state) : state);
        return res;
    };
};
export const reducer = reducerLogging ? withLogging(reducer_) : reducer_;
export const ensureDashboardLayoutIsCorrect = (draft) => {
    if (hasChartConfigs(draft) &&
        ...
        for (const [breakpoint, _breakpointLayouts] of Object.entries(layouts)) {
            ...
            const keysToRemove = breakpointLayoutKeys.filter((key) => !blocks.find((block) => block.key === key));
...
const getPreferredEmptyCellCoords = ({ layouts, cols, }) => {
