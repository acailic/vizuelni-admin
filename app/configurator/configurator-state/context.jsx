...
export const ConfiguratorStateProvider = (props) => {
    return (<ConfiguratorStateProviderInternal 
    // Ensure that the state is reset by using the `chartId` as `key`
    key={props.chartId} {...props}/>);
};
export const useConfiguratorState = (predicate) => {
    const ctx = useContext(ConfiguratorStateContext);
    if (ctx === undefined) {
        throw Error("You need an <ConfiguratorStateProvider> to useConfiguratorState");
    }
...
export const useReadOnlyConfiguratorState = (predicate) => {
    const ctx = useContext(ConfiguratorStateContext);
    if (ctx === undefined) {
        throw Error("You need an <ConfiguratorStateProvider> to useConfiguratorState");
    }
...
const preparePublishingState = (state, chartConfigs, activeChartKey) => {
    return {
        ...state,
        chartConfigs: [
                ...
                return {
                    ...
                    cubes: chartConfig.cubes.map((cube) => {
                        return {
                            ...
                            filters: Object.fromEntries(Object.entries(cube.filters).map(([k, v], i) => {
        ...
        activeChartKey: activeChartKey !== null && activeChartKey !== void 0 ? activeChartKey : state.chartConfigs[0].key,
    };
};
const handlePublishSuccess = async (chartId, push) => {
    /**
...
async function initializeChartState(chartId, query, client, dataSource, locale) {
    if (chartId === "new") {
        if (query.copy && typeof query.copy === "string") {
            ...
            return initChartStateFromChartEdit(client, query.edit, typeof query.state === "string" ? query.state : undefined);
        }
...
export async function publishState(user, key, state, 
    ...
    switch (state.layout.type) {
        case "singleURLs":
            ...
            return allSequential(reversedChartKeys, async (chartKey, i) => {
                ...
                }, state.chartConfigs.filter((d) => d.key === chartKey), chartKey);
...
const allSequential = async (input, cb) => {
    const res = [];
...
const ConfiguratorStateProviderInternal = (props) => {
    const { chartId, initialState, allowDefaultRedirect = true, children, } = props;
    ...
    useEffect(() => {
        ...
        const initialize = async () => {
            try {
                const newChartState = await initializeChartState(chartId, query, client, dataSource, locale);
                if (!newChartState && allowDefaultRedirect && chartId !== "published") {
                    replace(`/create/new`);
                }
                stateToInitialize = newChartState !== null && newChartState !== void 0 ? newChartState : stateToInitialize;
            }
            finally {
    ...
    useEffect(() => {
        try {
            switch (state.state) {
                ...
                case "LAYOUTING":
                    const savedChartId = chartId === "new"
                ...
                case "PUBLISHING":
                    (async () => {
                        try {
                            ...
                            return publishState(user, key, state, async (result, i, total) => {
    ...
    return (<ConfiguratorStateContext.Provider value={stateAndDispatch}>
      {children}
