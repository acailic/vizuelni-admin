...
const useTimeline = () => {
    ...
    if (!timeline) {
        throw Error("useTimeline must be called inside a TimelineContext.Provider!");
    }
...
export const TimeSlider = (props) => {
    var _a;
    const { componentId, showPlayButton, type: animationType, duration: animationDuration, filters, dimensions, } = props;
    ...
    const timelineProps = useMemo(() => {
        ...
        const uniqueValues = Array.from(new Set(chartState.allData
            .map((d) => d[dimension.id])
            .filter(truthy)));
        if (temporal) {
            const msValues = uniqueValues.map((d) => parseDate(d).getTime());
            const formatValue = (d) => timeFormatUnit(new Date(d), timeUnit);
            return {
                type: "interval",
                msValues,
                formatValue,
    ...
    if ((_a = state.dashboardFilters) === null || _a === void 0 ? void 0 : _a.timeRange.active) {
        return null;
    }
    return (<TimelineContext.Provider value={timeline}>
      <Root showPlayButton={showPlayButton}/>
    </TimelineContext.Provider>);
};
const Root = ({ showPlayButton }) => {
    const timeline = useTimeline();
    const [min, max] = timeline.formattedExtent;
    return (<Box {...DISABLE_SCREENSHOT_ATTR} sx={{
            display: "flex",
            alignItems: "center",
            gap: 5,
            mb: 5,
      ...
      <div style={{ position: "relative", width: "100%" }}>
        <Slider />
        <Typography variant="caption" sx={{ position: "absolute", left: 0, mt: 2 }}>
          {min}
        </Typography>
        <Typography variant="caption" sx={{ position: "absolute", right: 0, mt: 2 }}>
          {max}
        </Typography>
      </div>
    </Box>);
...
const Slider = () => {
    ...
    const marks = useMemo(() => {
        var _a;
        return (_a = timeline.domain.map((d) => ({ value: d }))) !== null && _a !== void 0 ? _a : [];
    }, [timeline.domain]);
    ...
    useEffect(() => {
        ...
        if (timeline.type === "interval" &&
            timeSlider.type === "interval" &&
            ((_a = timeSlider.value) === null || _a === void 0 ? void 0 : _a.getTime()) !== timeline.value) {
            setTimeSlider({
                type: "interval",
                value: new Date(timeline.value),
            });
        }
        if (timeline.type === "ordinal" &&
            timeSlider.type === "ordinal" &&
            (timeSlider === null || timeSlider === void 0 ? void 0 : timeSlider.value) !== timeline.value) {
            setTimeSlider({
                type: "ordinal",
    ...
    return (<GenericSlider name="time-slider" renderTextInput={false} min={0} max={1} step={null} marks={marks} value={timeline.progress} onChange={onChange} onClick={onClick} valueLabelDisplay="on" valueLabelFormat={timeline.formattedValue} className={clsx(classes.root, timeline.playing && classes.playing)}/>);
};
