  ...
  describe('rendering with basic props', () => {
    ...
    it('should render with title', async () => {
      const { container } = render(
        <BarChart
          data={mockData}
          config={{ xAxis: 'category', yAxis: 'value', title: 'Test Chart Title' }}
          height={400}
        />
      );
    ...
    it('should render multiple series with different colors', async () => {
      const { container } = render(
        <BarChart
          ...
          config={{ xAxis: 'category', yAxis: ['series1', 'series2', 'series3'] }}
          height={400}
        />
      );
  ...
  describe('custom colors', () => {
    it('should use custom color from config', async () => {
      ...
      const hasCustomColor = Array.from(rects).some(
        (rect) => rect.getAttribute('fill') === '#ff0000' || rect.getAttribute('fill')?.includes('url(#bar-gradient-0')
      );
