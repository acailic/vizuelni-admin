 ...
 */
export const flag = function flag(...args) {
    if (args.length === 1) {
...
const removeDeprecatedFlags = () => {
    ...
    const deprecatedFlags = storeFlags.filter((name) => !FLAGS.some((flagDef) => flagDef.name === name));
...
const resetFlags = () => {
    listFlagNames().forEach((name) => store.remove(name));
};
/**
...
const initFromSearchParams = (locationSearch) => {
...
export const isVercelPreviewHost = (host) => {
    return !!/visualization\-tool.*ixt1\.vercel\.app/.exec(host);
};
const setDefaultFlag = (name, value) => {
    const flagValue = flag(name);
    if (flagValue === null) {
        flag(name, value);
    }
};
const initFromHost = (host) => {
    const shouldSetDefaultFlags = host.includes("localhost") ||
