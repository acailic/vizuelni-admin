 ...
 */
export function transformEducationData(rawData, locale = 'sr') {
 ...
 */
function normalizeEducationLevel(level, locale) {
 ...
 * ]
 */
export function transformEducationDataSimple(rawData) {
    if (!rawData || rawData.length === 0) {
        return [];
    }
    const yearColumn = detectColumn(rawData[0], [
 ...
 * ]
 */
export function transformEducationLevelDistribution(rawData, targetYear, locale = 'sr') {
    if (!rawData || rawData.length === 0) {
        return [];
    }
    const yearColumn = detectColumn(rawData[0], [
    ...
    if (targetYear && yearColumn) {
        filteredData = rawData.filter(row => String(row[yearColumn]).includes(targetYear));
    }
    ...
    if (filteredData.length === 0 && yearColumn) {
        ...
        filteredData = rawData.filter(row => String(row[yearColumn]) === mostRecentYear);
    }
    // Group by level
    const levelTotals = {};
 ...
 */
function detectColumn(row, possibleNames) {
 ...
 */
export function transformBudgetData(rawData, locale = 'sr') {
    if (!rawData || rawData.length === 0) {
        return [];
    }
    const yearColumn = detectColumn(rawData[0], [
