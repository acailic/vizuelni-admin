...
export const Examples = ({ headline, example1Headline, example1Description, example2Headline, example2Description, }) => {
    ...
    useEffect(() => {
        const run = async () => {
            (await Promise.all([getExampleState1(), getExampleState2()])).forEach((state, i) => {
      ...
      <ContentWrapper sx={{ py: 20 }}>
        <div style={{ width: "100%" }}>
          ...
          <Box sx={(t) => ({
            ...
            <Example queryKey="example1" configuratorState={state1} headline={example1Headline} description={example1Description}/>
            <Example queryKey="example2" configuratorState={state2} headline={example2Headline} description={example2Description}/>
          </Box>
        </div>
      </ContentWrapper>
    </Box>);
};
const Example = ({ queryKey, configuratorState, headline, description, }) => {
    ...
    return data ? (<ConfiguratorStateProvider chartId="published" initialState={data}>
      <ExampleCard headline={headline} description={description}>
        <ChartPublished embedParams={{ removeBorder: true }} shouldShrink={false}/>
      </ExampleCard>
    </ConfiguratorStateProvider>) : error ? (<Box sx={{ mb: 6 }}>
      <LoadingDataError />
    </Box>) : (<Skeleton variant="rectangular" height={400}/>);
};
const ExampleCard = ({ children, headline, description, }) => {
    return (<Card sx={{
            display: "grid",
            gridTemplateRows: "subgrid",
            gridRow: "span 2",
