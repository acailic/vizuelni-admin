...
import type { DatasetLoadOptions, Observation, ParsedDataset } from '../types';
import {
  createLoadError,
...
function coerceRows(
  ...
  return limitedRows.map((row) =>
    Object.fromEntries(
      columns.map((column) => [
        column,
        coerceObservationValue(row[column] ?? null),
      ])
    )
  );
}
...
export function parseDatasetContent(
  ...
  const format = detectResourceFormat(
    options.format,
    options.resourceUrl,
    options.contentType
  );
  const parsed =
    format === 'csv' ? parseCsvRows(content) : parseJsonRows(content);
  const observations = coerceRows(
    parsed.rows,
    parsed.columns,
    options.rowLimit
  );
...
export async function loadDatasetFromUrl(
  ...
  const fetchInit = {
    ...options.fetchInit,
    ...(options.rowLimit && !options.fetchInit?.cache
  ...
  try {
    ...
    if (format === 'csv' && options.rowLimit && response.body) {
      ...
      while (
        totalBytes < CSV_SAMPLE_BYTE_LIMIT &&
        newlineCount < options.rowLimit + 1
      ) {
