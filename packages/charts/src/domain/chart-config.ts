...
import type { GeoLevel as SharedGeoLevel } from '@vizualni/shared-kernel';
...
export type GeoLevel = SharedGeoLevel;
...
export interface ChartOptions {
  ...
  treemapTiling?: 'squarify' | 'binary';
  sankeySourceField?: string;
  sankeyTargetField?: string;
  heatmapXField?: string;
  heatmapYField?: string;
  gaugeMin?: number;
  gaugeMax?: number;
  gaugeThresholds?: Array<{ value: number; color: string; label?: string }>;
  pyramidMaleField?: string;
  pyramidFemaleField?: string;
  /** Disable the "Absolute vs Percentage" calculation toggle for charts that are already rates/percentages */
  disableCalculationToggle?: boolean;
}
...
// Chart renderer data row (framework-agnostic)
...
export const chartOptionsSchema = baseChartOptionsSchema.extend({
  ...
  sankeySourceField: z.string().trim().min(1).optional(),
  sankeyTargetField: z.string().trim().min(1).optional(),
  heatmapXField: z.string().trim().min(1).optional(),
  heatmapYField: z.string().trim().min(1).optional(),
  ...
  gaugeThresholds: z
    ...
    .min(1)
    .optional(),
  pyramidMaleField: z.string().trim().min(1).optional(),
  pyramidFemaleField: z.string().trim().min(1).optional(),
  disableCalculationToggle: z.boolean().optional(),
});
...
export function getDefaultOptions(
  ...
  switch (type) {
    ...
    case 'radar':
      return { showLegend: true, showGrid: true, fillOpacity: 0.35, radarShape: 'polygon' };
    case 'treemap':
      return { showLegend: false, showLabels: true, treemapTiling: 'squarify' };
    case 'funnel':
      return { showLegend: false, showLabels: true };
    case 'sankey':
      return { showLegend: false };
    case 'heatmap':
      return { showLegend: false, showLabels: true };
    case 'population-pyramid':
      return { showLegend: true, showGrid: true, grouping: 'grouped' };
    case 'waterfall':
      return { showLegend: false, showLabels: true };
    case 'gauge':
      return {
        ...
        gaugeMax: 100,
        gaugeThresholds: [
          { value: 33, color: '#C6363C', label: 'Low' },
