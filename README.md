...
The root is intentionally kept to a small set of active entrypoints:

- `src/`: the main Next.js application and shared frontend code
- `packages/`: workspace packages that power charts, data handling, and reusable libraries
- `scripts/`: active automation for builds, deploys, docs, and quality checks
- `tests/`: end-to-end, integration, and unit coverage
- `docs/`: long-form product, architecture, and VitePress documentation
- `templates/`: starter and deployment templates
- `public/`, `prisma/`, `.github/`: runtime assets, database schema, and CI

Legacy or exploratory files that used to live in the root have been moved into clearer homes:

- `scripts/legacy/`: old one-off utility scripts kept only for reference
- `docs/archive/`: archived documentation drafts and superseded doc entrypoints
- `templates/stackblitz/package.project.json`: the project-specific StackBlitz package variant

If a new file does not define a top-level runtime, workspace, or shared config concern, it should usually live under one of those directories instead of the repository root.
...
- [x] Geographic visualizations (municipal maps, election results)
- [x] Real-time dashboards
- [ ] Export to PDF/PowerPoint
- [x] Comparison tools (year-over-year, municipality-to-municipality)
- [x] Demo Gallery — World-class visualization discovery interface
  - [x] Dynamic stats display with filtering context
  - [x] Skeleton loading states with timeout handling
  - [x] Retry mechanism for failed previews
  - [x] Chart type icons and metadata badges
  - [x] Accessible category filtering with mobile support
  - [x] Population pyramid with correct y-axis ordering
