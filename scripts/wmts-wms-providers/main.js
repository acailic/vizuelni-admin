...
async function fetchAllowList() {
    ...
    const results = await Promise.all(urls.map((url) => fetch(url).then((x) => x.json())));
    return new Set(results
        .flat()
        .filter((x) => x.trim() !== "")
        .map((x) => normalizeUrl(x.trim())));
}
function filterProviders(providers, allowList) {
    try {
        const filteredProviders = providers.filter((provider) => provider && allowList.has(normalizeUrl(provider)));
...
async function main() {
    const initialProvidersPath = path.join(__dirname, "../../", "./app/charts/map/initial-wms-wmts-providers.json");
    const providersPath = path.join(__dirname, "../../", "./app/charts/map/wms-wmts-providers.json");
    ...
    console.log(`Filtered ${filteredProviders.length} providers from the JSON file`);
