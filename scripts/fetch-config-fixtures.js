...
const pullConfigs = async ({ baseUrl, env, }) => {
    const fixturesDir = path.join(__dirname, "..", "app", "test", "__fixtures", env);
    const cyFixuresDir = path.join(__dirname, "..", "cypress", "fixtures", env);
    console.log(`Pull configs from ${baseUrl}`);
    const configs = await Promise.all(intConfigs.map(async ({ chartId, slug }) => {
    ...
    await Promise.all(configs.map(async ({ config, slug }) => {
